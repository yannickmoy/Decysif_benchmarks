;; produced by z3_gnatprove.drv ;;
(set-info :smt-lib-version 2.5)
(set-info :source |VC generated by SPARK 2014|)
(set-info :category industrial)
(set-info :status unknown)
;;; generated by SMT-LIB2 driver
;;; SMT-LIB2 driver: bit-vectors, common part
;;; SMT-LIB2: integer arithmetic
(declare-sort us_private 0)

(declare-const us_null_ext__ us_private)

(declare-datatypes ()
  ((int__ref (int__refqtmk (int__content Int)))))

(declare-datatypes ()
  ((bool__ref (bool__refqtmk (bool__content Bool)))))

(declare-sort integer 0)

;; "integer'int"
(declare-fun integerqtint (integer) Int)

;; "integer'axiom"
(assert
  (forall ((i integer))
    (and
      (<= (- 2147483648) (integerqtint i))
      (<= (integerqtint i) 2147483647))))

;; "in_range"
(define-fun in_range ((x Int)) Bool
  (and (<= (- 2147483648) x) (<= x 2147483647)))

;; "to_rep"
(define-fun to_rep ((x integer)) Int
  (integerqtint x))

;; "of_rep"
(declare-fun of_rep (Int) integer)

;; "inversion_axiom"
(assert
  (forall ((x integer)) (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)) )))

;; "range_axiom"
(assert
  (forall ((x integer)) (! (in_range (to_rep x)) :pattern ((to_rep x)) )))

;; "coerce_axiom"
(assert
  (forall ((x Int))
    (! (=> (in_range x) (= (to_rep (of_rep x)) x)) :pattern ((to_rep
                                                               (of_rep x))) )))

;; "in_range"
(define-fun in_range1 ((x Int)) Bool
  (and (<= 1 x) (<= x 2147483647)))

(declare-datatypes ()
  ((map__ref (map__refqtmk (map__content (Array Int integer))))))

(declare-datatypes ()
  ((t (tqtmk (rec__first integer)(rec__last integer)))))

;; "mk"
(declare-fun mk (Int
  Int) t)

;; "mk_def"
(assert
  (forall ((f Int) (l Int))
    (! (=>
         (in_range f)
         (=>
           (in_range l)
           (and
             (= (to_rep (rec__first (mk f l))) f)
             (= (to_rep (rec__last (mk f l))) l)))) :pattern ((mk f l)) )))

;; "dynamic_property"
(define-fun dynamic_property ((range_first Int) (range_last Int) (low Int) (high Int)) Bool
  (and
    (in_range low)
    (and
      (in_range high)
      (=> (<= low high) (and (in_range1 low) (in_range1 high))))))

(declare-datatypes ()
  ((us_t (us_tqtmk (elts (Array Int integer))(rt t)))))

;; "to_array"
(define-fun to_array ((a us_t)) (Array Int integer)
  (elts a))

;; "of_array"
(define-fun of_array ((a (Array Int integer)) (f Int) (l Int)) us_t
  (us_tqtmk a (mk f l)))

;; "first"
(define-fun first ((a us_t)) Int
  (to_rep (rec__first (rt a))))

;; "last"
(define-fun last ((a us_t)) Int
  (to_rep (rec__last (rt a))))

;; "length"
(define-fun length ((a us_t)) Int
  (ite (<= (first a) (last a)) (+ (- (last a) (first a)) 1) 0))

(declare-const value__size Int)

(declare-const object__size Int)

(declare-const component__size Int)

(declare-const alignment Int)

;; "value__size_axiom"
(assert (<= 0 value__size))

;; "object__size_axiom"
(assert (<= 0 object__size))

;; "component__size_axiom"
(assert (<= 0 component__size))

;; "alignment_axiom"
(assert (<= 0 alignment))

(declare-datatypes ()
  ((int_array__ref (int_array__refqtmk (int_array__content us_t)))))

(declare-const s1 us_t)

(declare-const s2 us_t)

(declare-const r__first integer)

(declare-const r__last integer)

;; "dynamic_invariant"
(define-fun dynamic_invariant ((temp___expr_189 us_t) (temp___is_init_185 Bool) (temp___skip_constant_186 Bool) (temp___do_toplevel_187 Bool) (temp___do_typ_inv_188 Bool)) Bool
  (and
    (=>
      (not (= temp___skip_constant_186 true))
      (dynamic_property
        1
        2147483647
        (first temp___expr_189)
        (last temp___expr_189)))
    (=>
      (= temp___is_init_185 true)
      (=> (= temp___do_toplevel_187 true) (= (first temp___expr_189) 1)))))

;; "dynamic_invariant"
(define-fun dynamic_invariant1 ((temp___expr_268 us_t) (temp___is_init_264 Bool) (temp___skip_constant_265 Bool) (temp___do_toplevel_266 Bool) (temp___do_typ_inv_267 Bool)) Bool
  (and
    (=>
      (not (= temp___skip_constant_265 true))
      (and
        (and
          (dynamic_property
            1
            (+ (length s1) (length s2))
            (first temp___expr_268)
            (last temp___expr_268))
          (= (first temp___expr_268) 1))
        (= (last temp___expr_268) (+ (length s1) (length s2)))))
    (=> (= temp___is_init_264 true) (= (first temp___expr_268) 1))))

;; "default_initial_assumption"
(define-fun default_initial_assumption ((temp___expr_271 us_t) (temp___skip_top_level_272 Bool)) Bool
  (and
    (= (first temp___expr_271) 1)
    (= (last temp___expr_271) (+ (length s1) (length s2)))))

;; "temp_____aggregate_def_617"
(declare-fun temp_____aggregate_def_617 (Int
  Int
  Int) us_t)

(declare-const rliteral integer)

;; "rliteral_axiom"
(assert (= (integerqtint rliteral) 0))

(declare-const r (Array Int integer))

;; "r'unused"
(define-fun rqtunused () map__ref
  (map__refqtmk r))

;; "Assume"
(assert (dynamic_invariant s1 true false true true))

;; "Assume"
(assert (dynamic_invariant s2 true false true true))

;; "Assume"
(assert
  (and
    (and
      (< (length s1) (- 2147483647 (length s2)))
      (forall ((l Int))
        (=>
          (and (<= 2 l) (<= l (last s1)))
          (<= (to_rep (select (to_array s1) (- l 1))) (to_rep
                                                        (select (to_array s1) l))))))
    (forall ((l Int))
      (=>
        (and (<= 2 l) (<= l (last s2)))
        (<= (to_rep (select (to_array s2) (- l 1))) (to_rep
                                                      (select (to_array s2) l)))))))

(declare-const j1 Int)

;; "j1'unused"
(define-fun j1qtunused () int__ref
  (int__refqtmk j1))

;; "H"
(assert (= j1 1))

;; "Assume"
(assert (in_range1 j1))

(declare-const j2 Int)

;; "j2'unused"
(define-fun j2qtunused () int__ref
  (int__refqtmk j2))

;; "H"
(assert (= j2 1))

;; "Assume"
(assert (in_range1 j2))

(declare-const j Int)

;; "j'unused"
(define-fun jqtunused () int__ref
  (int__refqtmk j))

;; "H"
(assert (= j 1))

;; "Assume"
(assert (in_range1 j))

;; "Assume"
(assert
  (default_initial_assumption
    (of_array r (to_rep r__first) (to_rep r__last))
    false))

;; "Assume"
(assert
  (dynamic_invariant1
    (of_array r (to_rep r__first) (to_rep r__last))
    false
    false
    true
    true))

;; "o"
(define-fun o () Int
  (length s2))

;; "length'result'unused"
(define-fun lengthqtresultqtunused () Int
  o)

;; "o"
(define-fun o1 () Int
  (length s1))

;; "length'result'unused"
(define-fun lengthqtresultqtunused1 () Int
  o1)

;; "o"
(define-fun o2 () Int
  (+ o1 o))

;; "infix +'result'unused"
(define-fun infix_plqtresultqtunused () Int
  o2)

(declare-const temp___621 us_t)

;; "H"
(assert (= (first temp___621) 1))

;; "H"
(assert (= (last temp___621) o2))

;; "H"
(assert true)

;; "H"
(assert true)

;; "H"
(assert
  (forall ((temp___620 Int))
    (! (=>
         (and
           (<= (first temp___621) temp___620)
           (<= temp___620 (last temp___621)))
         (= (select (to_array temp___621) temp___620) rliteral)) :pattern ((select 
    (to_array
      temp___621) temp___620)) )))

;; "H"
(assert (= temp___621 (temp_____aggregate_def_617 0 1 o2)))

;; "temp_____aggregate_def_617'result'unused"
(define-fun temp_____aggregate_def_617qtresultqtunused () us_t
  temp___621)

;; "Assume"
(assert (= (first temp___621) 1))

;; "Assume"
(assert (= (last temp___621) (+ (length s1) (length s2))))

;; "Assert"
(assert (= (first temp___621) 1))

;; "temp___625"
(define-fun temp___625 () (Array Int integer)
  (to_array temp___621))

;; "to_array'result'unused"
(define-fun to_arrayqtresultqtunused () (Array Int integer)
  temp___625)

;; "Assert"
(assert (= (first temp___621) 1))

;; "Assert"
(assert
  (ite (<= (first temp___621) (last temp___621))
    (and
      (<= 1 (+ (length s1) (length s2)))
      (= (- (last temp___621) (first temp___621)) (- (+ (length s1) (length
                                                                    s2)) 1)))
    (< (+ (length s1) (length s2)) 1)))

;; "o"
(define-fun o3 () Int
  (last temp___621))

;; "last'result'unused"
(define-fun lastqtresultqtunused () Int
  o3)

;; "o"
(define-fun o4 () Int
  (first temp___621))

;; "first'result'unused"
(define-fun firstqtresultqtunused () Int
  o4)

;; "temp___627"
(define-fun temp___627 () us_t
  temp___621)

;; "of_array'result'unused"
(define-fun of_arrayqtresultqtunused () us_t
  temp___627)

(declare-const o5 Int)

;; "Ensures"
(assert (= (to_rep r__last) o5))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused () Int
  o5)

(declare-const o6 Int)

;; "Ensures"
(assert (= (to_rep r__first) o6))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused1 () Int
  o6)

;; "map__content'result'unused"
(define-fun map__contentqtresultqtunused () (Array Int integer)
  r)

;; "temp___628"
(define-fun temp___628 () us_t
  (of_array r o6 o5))

;; "of_array'result'unused"
(define-fun of_arrayqtresultqtunused1 () us_t
  temp___628)

;; "Assert"
(assert
  (ite (<= (first temp___627) (last temp___627))
    (and (<= o6 o5) (= (- (last temp___627) (first temp___627)) (- o5 o6)))
    (< o5 o6)))

;; "o"
(define-fun o7 () (Array Int integer)
  (to_array temp___627))

;; "to_array'result'unused"
(define-fun to_arrayqtresultqtunused1 () (Array Int integer)
  o7)

(declare-const r1 (Array Int integer))

;; "r'unused"
(define-fun rqtunused1 () map__ref
  (map__refqtmk r1))

;; "H"
(assert (= r1 o7))

(declare-const j11 Int)

(declare-const j21 Int)

(declare-const j3 Int)

(declare-const r2 (Array Int integer))

;; "r'unused"
(define-fun rqtunused2 () map__ref
  (map__refqtmk r2))

;; "j'unused"
(define-fun jqtunused1 () int__ref
  (int__refqtmk j3))

;; "j2'unused"
(define-fun j2qtunused1 () int__ref
  (int__refqtmk j21))

;; "j1'unused"
(define-fun j1qtunused1 () int__ref
  (int__refqtmk j11))

(declare-const o8 Int)

;; "H"
(assert (= o8 (last s1)))

(declare-const lastqtresultqtunused1 Int)

;; "H"
(assert (= lastqtresultqtunused1 o8))

(declare-const int__contentqtresultqtunused Int)

;; "H"
(assert (= int__contentqtresultqtunused j1))

(declare-const o9 Bool)

;; "H"
(assert (= o9 (ite (<= j1 o8) true false)))

(declare-const infix_lseqqtresultqtunused Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused o9))

(declare-const int__contentqtresultqtunused1 Int)

;; "H"
(assert (= int__contentqtresultqtunused1 j1))

(declare-const o10 Int)

;; "H"
(assert (= o10 (first s1)))

(declare-const firstqtresultqtunused1 Int)

;; "H"
(assert (= firstqtresultqtunused1 o10))

(declare-const o11 Bool)

;; "H"
(assert (= o11 (ite (<= o10 j1) true false)))

(declare-const infix_lseqqtresultqtunused1 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused1 o11))

(declare-const o12 Bool)

;; "H"
(assert (= o12 (and o11 o9)))

(declare-const andbqtresultqtunused Bool)

;; "H"
(assert (= andbqtresultqtunused o12))

(declare-const o13 Bool)

(declare-const o14 Int)

(declare-const lastqtresultqtunused2 Int)

(declare-const int__contentqtresultqtunused2 Int)

(declare-const o15 Bool)

(declare-const infix_lseqqtresultqtunused2 Bool)

(declare-const int__contentqtresultqtunused3 Int)

(declare-const o16 Int)

(declare-const firstqtresultqtunused2 Int)

(declare-const o17 Bool)

(declare-const infix_lseqqtresultqtunused3 Bool)

(declare-const andbqtresultqtunused1 Bool)

;; "H"
(assert
  (ite (= o12 true)
    (and
      (= o14 (last s2))
      (and
        (= lastqtresultqtunused2 o14)
        (and
          (= int__contentqtresultqtunused2 j2)
          (and
            (= o15 (ite (<= j2 o14) true false))
            (and
              (= infix_lseqqtresultqtunused2 o15)
              (and
                (= int__contentqtresultqtunused3 j2)
                (and
                  (= o16 (first s2))
                  (and
                    (= firstqtresultqtunused2 o16)
                    (and
                      (= o17 (ite (<= o16 j2) true false))
                      (and
                        (= infix_lseqqtresultqtunused3 o17)
                        (and
                          (= o13 (and o17 o15))
                          (= andbqtresultqtunused1 o13))))))))))))
    (= o13 false)))

(declare-const o18 Int)

(declare-const to_repqtresultqtunused2 Int)

(declare-const o19 Int)

(declare-const to_repqtresultqtunused3 Int)

(declare-const map__contentqtresultqtunused1 (Array Int integer))

(declare-const temp___loop_entry_645 us_t)

(declare-const of_arrayqtresultqtunused2 us_t)

(declare-const temp___loop_entry_644qtunused Int)

(declare-const int__contentqtresultqtunused4 Int)

(declare-const temp___loop_entry_643qtunused Int)

(declare-const int__contentqtresultqtunused5 Int)

(declare-const temp___loop_entry_642qtunused Int)

(declare-const int__contentqtresultqtunused6 Int)

(declare-const int__contentqtresultqtunused7 Int)

(declare-const int__contentqtresultqtunused8 Int)

(declare-const o20 Int)

(declare-const infix_plqtresultqtunused1 Int)

(declare-const range_check_qtresultqtunused Int)

(declare-const o21 Int)

(declare-const infix_mnqtresultqtunused Int)

(declare-const int__contentqtresultqtunused9 Int)

(declare-const infix_eqqtresultqtunused Bool)

(declare-const j12 Int)

(declare-const j22 Int)

(declare-const j4 Int)

(declare-const r3 (Array Int integer))

(declare-const rqtunused3 map__ref)

(declare-const jqtunused2 int__ref)

(declare-const j2qtunused2 int__ref)

(declare-const j1qtunused2 int__ref)

(declare-const loop_var___43231qtunused Int)

(declare-const int__contentqtresultqtunused10 Int)

(declare-const loop_var___43245qtunused Int)

(declare-const int__contentqtresultqtunused11 Int)

(declare-const int__contentqtresultqtunused12 Int)

(declare-const o22 Int)

(declare-const infix_plqtresultqtunused2 Int)

(declare-const range_check_qtresultqtunused1 Int)

(declare-const o23 Bool)

(declare-const o24 Int)

(declare-const lastqtresultqtunused3 Int)

(declare-const int__contentqtresultqtunused13 Int)

(declare-const o25 Bool)

(declare-const infix_lseqqtresultqtunused4 Bool)

(declare-const int__contentqtresultqtunused14 Int)

(declare-const o26 Int)

(declare-const firstqtresultqtunused3 Int)

(declare-const o27 Bool)

(declare-const infix_lseqqtresultqtunused5 Bool)

(declare-const o28 Bool)

(declare-const andbqtresultqtunused2 Bool)

(declare-const o29 Int)

(declare-const lastqtresultqtunused4 Int)

(declare-const int__contentqtresultqtunused15 Int)

(declare-const o30 Bool)

(declare-const infix_lseqqtresultqtunused6 Bool)

(declare-const int__contentqtresultqtunused16 Int)

(declare-const o31 Int)

(declare-const firstqtresultqtunused4 Int)

(declare-const o32 Bool)

(declare-const infix_lseqqtresultqtunused7 Bool)

(declare-const andbqtresultqtunused3 Bool)

(declare-const int__contentqtresultqtunused17 Int)

(declare-const o33 (Array Int integer))

(declare-const to_arrayqtresultqtunused2 (Array Int integer))

(declare-const o34 integer)

(declare-const getqtresultqtunused integer)

(declare-const o35 Int)

(declare-const to_repqtresultqtunused4 Int)

(declare-const int__contentqtresultqtunused18 Int)

(declare-const o36 (Array Int integer))

(declare-const to_arrayqtresultqtunused3 (Array Int integer))

(declare-const o37 integer)

(declare-const getqtresultqtunused1 integer)

(declare-const o38 Int)

(declare-const to_repqtresultqtunused5 Int)

(declare-const o39 Bool)

(declare-const infix_lsqtresultqtunused Bool)

(declare-const spark__branch Bool)

(declare-const spark__branchqtunused bool__ref)

(declare-const bool__contentqtresultqtunused Bool)

(declare-const o40 Int)

(declare-const to_repqtresultqtunused6 Int)

(declare-const o41 Int)

(declare-const to_repqtresultqtunused7 Int)

(declare-const map__contentqtresultqtunused2 (Array Int integer))

(declare-const temp___636 us_t)

(declare-const of_arrayqtresultqtunused3 us_t)

(declare-const o42 t)

(declare-const rtqtresultqtunused t)

(declare-const int__contentqtresultqtunused19 Int)

(declare-const o43 (Array Int integer))

(declare-const to_arrayqtresultqtunused4 (Array Int integer))

(declare-const o44 integer)

(declare-const getqtresultqtunused2 integer)

(declare-const o45 Int)

(declare-const to_repqtresultqtunused8 Int)

(declare-const o46 integer)

(declare-const of_repqtresultqtunused integer)

(declare-const int__contentqtresultqtunused20 Int)

(declare-const o47 (Array Int integer))

(declare-const to_arrayqtresultqtunused5 (Array Int integer))

(declare-const o48 (Array Int integer))

(declare-const setqtresultqtunused (Array Int integer))

(declare-const temp___638 us_t)

(declare-const us_tqtmkqtresultqtunused us_t)

(declare-const o49 (Array Int integer))

(declare-const to_arrayqtresultqtunused6 (Array Int integer))

(declare-const int__contentqtresultqtunused21 Int)

(declare-const o50 Int)

(declare-const infix_plqtresultqtunused3 Int)

(declare-const range_check_qtresultqtunused2 Int)

(declare-const o51 Int)

(declare-const to_repqtresultqtunused9 Int)

(declare-const o52 Int)

(declare-const to_repqtresultqtunused10 Int)

(declare-const map__contentqtresultqtunused3 (Array Int integer))

(declare-const temp___630 us_t)

(declare-const of_arrayqtresultqtunused4 us_t)

(declare-const o53 t)

(declare-const rtqtresultqtunused1 t)

(declare-const int__contentqtresultqtunused22 Int)

(declare-const o54 (Array Int integer))

(declare-const to_arrayqtresultqtunused7 (Array Int integer))

(declare-const o55 integer)

(declare-const getqtresultqtunused3 integer)

(declare-const o56 Int)

(declare-const to_repqtresultqtunused11 Int)

(declare-const o57 integer)

(declare-const of_repqtresultqtunused1 integer)

(declare-const int__contentqtresultqtunused23 Int)

(declare-const o58 (Array Int integer))

(declare-const to_arrayqtresultqtunused8 (Array Int integer))

(declare-const o59 (Array Int integer))

(declare-const setqtresultqtunused1 (Array Int integer))

(declare-const temp___632 us_t)

(declare-const us_tqtmkqtresultqtunused1 us_t)

(declare-const o60 (Array Int integer))

(declare-const to_arrayqtresultqtunused9 (Array Int integer))

(declare-const int__contentqtresultqtunused24 Int)

(declare-const o61 Int)

(declare-const infix_plqtresultqtunused4 Int)

(declare-const range_check_qtresultqtunused3 Int)

;; "H"
(assert
  (ite (= o13 true)
    (and
      (= (to_rep r__last) o18)
      (and
        (= to_repqtresultqtunused2 o18)
        (and
          (= (to_rep r__first) o19)
          (and
            (= to_repqtresultqtunused3 o19)
            (and
              (= map__contentqtresultqtunused1 r1)
              (and
                (= temp___loop_entry_645 (of_array r1 o19 o18))
                (and
                  (= of_arrayqtresultqtunused2 temp___loop_entry_645)
                  (and
                    (= temp___loop_entry_644qtunused j)
                    (and
                      (= int__contentqtresultqtunused4 j)
                      (and
                        (= temp___loop_entry_643qtunused j2)
                        (and
                          (= int__contentqtresultqtunused5 j2)
                          (and
                            (= temp___loop_entry_642qtunused j1)
                            (and
                              (= int__contentqtresultqtunused6 j1)
                              (and
                                (= int__contentqtresultqtunused7 j2)
                                (and
                                  (= int__contentqtresultqtunused8 j1)
                                  (and
                                    (= o20 (+ j1 j2))
                                    (and
                                      (= infix_plqtresultqtunused1 o20)
                                      (and
                                        (in_range o20)
                                        (and
                                          (= range_check_qtresultqtunused 
                                          o20)
                                          (and
                                            (= o21 (- o20 1))
                                            (and
                                              (= infix_mnqtresultqtunused 
                                              o21)
                                              (and
                                                (= int__contentqtresultqtunused9 
                                                j)
                                                (and
                                                  (= infix_eqqtresultqtunused 
                                                  (ite (= j o21)
                                                    true
                                                    false))
                                                  (and
                                                    (= rqtunused3 (map__refqtmk
                                                                    r3))
                                                    (and
                                                      (= jqtunused2 (int__refqtmk
                                                                    j4))
                                                      (and
                                                        (= j2qtunused2 
                                                        (int__refqtmk
                                                          j22))
                                                        (and
                                                          (= j1qtunused2 
                                                          (int__refqtmk
                                                            j12))
                                                          (and
                                                            (and
                                                              (= j4 (- (+ 
                                                              j12 j22) 1))
                                                              (and
                                                                (=>
                                                                  (< 1 
                                                                  j4)
                                                                  (<= 
                                                                  (to_rep
                                                                    (select 
                                                                    r3 (- 
                                                                    j4 1))) 
                                                                  (to_rep
                                                                    (select 
                                                                    (to_array
                                                                    s1) 
                                                                    j12))))
                                                                (and
                                                                  (=>
                                                                    (< 1 
                                                                    j4)
                                                                    (<= 
                                                                    (to_rep
                                                                    (select 
                                                                    r3 (- 
                                                                    j4 1))) 
                                                                    (to_rep
                                                                    (select 
                                                                    (to_array
                                                                    s2) 
                                                                    j22))))
                                                                  (forall 
                                                                    ((l Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 2 l)
                                                                    (<= l (- 
                                                                    j4 1)))
                                                                    (<= 
                                                                    (to_rep
                                                                    (select 
                                                                    r3 (- l 1))) 
                                                                    (to_rep
                                                                    (select 
                                                                    r3 l))))))))
                                                            (and
                                                              (= loop_var___43231qtunused 
                                                              j12)
                                                              (and
                                                                (= int__contentqtresultqtunused10 
                                                                j12)
                                                                (and
                                                                  (= 
                                                                  loop_var___43245qtunused 
                                                                  j22)
                                                                  (and
                                                                    (= 
                                                                    int__contentqtresultqtunused11 
                                                                    j22)
                                                                    (and
                                                                    (= (and 
                                                                    (ite 
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (in_range1
                                                                    j12)
                                                                    (in_range1
                                                                    j22))
                                                                    (in_range1
                                                                    j4))
                                                                    (dynamic_invariant1
                                                                    (of_array
                                                                    r3
                                                                    (to_rep
                                                                    r__first)
                                                                    (to_rep
                                                                    r__last))
                                                                    false
                                                                    true
                                                                    true
                                                                    true))
                                                                    true
                                                                    false) 
                                                                    (ite 
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    (first
                                                                    s1) 
                                                                    j12)
                                                                    (<= 
                                                                    j12 
                                                                    (last
                                                                    s1)))
                                                                    (and
                                                                    (<= 
                                                                    (first
                                                                    s2) 
                                                                    j22)
                                                                    (<= 
                                                                    j22 
                                                                    (last
                                                                    s2))))
                                                                    true
                                                                    false)) true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    (first
                                                                    s2) 
                                                                    j22)
                                                                    (<= 
                                                                    j22 
                                                                    (last
                                                                    s2)))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused17 
                                                                    j22)
                                                                    (and
                                                                    (= 
                                                                    o33 
                                                                    (to_array
                                                                    s2))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused2 
                                                                    o33)
                                                                    (and
                                                                    (= 
                                                                    o34 (select 
                                                                    o33 
                                                                    j22))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused 
                                                                    o34)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o34) 
                                                                    o35)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused4 
                                                                    o35)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    (first
                                                                    s1) 
                                                                    j12)
                                                                    (<= 
                                                                    j12 
                                                                    (last
                                                                    s1)))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused18 
                                                                    j12)
                                                                    (and
                                                                    (= 
                                                                    o36 
                                                                    (to_array
                                                                    s1))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused3 
                                                                    o36)
                                                                    (and
                                                                    (= 
                                                                    o37 (select 
                                                                    o36 
                                                                    j12))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused1 
                                                                    o37)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o37) 
                                                                    o38)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused5 
                                                                    o38)
                                                                    (and
                                                                    (= 
                                                                    o39 
                                                                    (ite (< 
                                                                    o38 
                                                                    o35)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lsqtresultqtunused 
                                                                    o39)
                                                                    (and
                                                                    (= 
                                                                    spark__branchqtunused 
                                                                    (bool__refqtmk
                                                                    spark__branch))
                                                                    (and
                                                                    (= 
                                                                    spark__branch 
                                                                    o39)
                                                                    (and
                                                                    (= 
                                                                    bool__contentqtresultqtunused 
                                                                    spark__branch)
                                                                    (ite (= 
                                                                    spark__branch true)
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    r__last) 
                                                                    o51)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused9 
                                                                    o51)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    r__first) 
                                                                    o52)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused10 
                                                                    o52)
                                                                    (and
                                                                    (= 
                                                                    map__contentqtresultqtunused3 
                                                                    r3)
                                                                    (and
                                                                    (= 
                                                                    temp___630 
                                                                    (of_array
                                                                    r3
                                                                    o52
                                                                    o51))
                                                                    (and
                                                                    (= 
                                                                    of_arrayqtresultqtunused4 
                                                                    temp___630)
                                                                    (and
                                                                    (= 
                                                                    o53 
                                                                    (rt
                                                                    temp___630))
                                                                    (and
                                                                    (= 
                                                                    rtqtresultqtunused1 
                                                                    o53)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    (first
                                                                    s1) 
                                                                    j12)
                                                                    (<= 
                                                                    j12 
                                                                    (last
                                                                    s1)))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused22 
                                                                    j12)
                                                                    (and
                                                                    (= 
                                                                    o54 
                                                                    (to_array
                                                                    s1))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused7 
                                                                    o54)
                                                                    (and
                                                                    (= 
                                                                    o55 (select 
                                                                    o54 
                                                                    j12))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused3 
                                                                    o55)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o55) 
                                                                    o56)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused11 
                                                                    o56)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o57) 
                                                                    o56)
                                                                    (and
                                                                    (= 
                                                                    of_repqtresultqtunused1 
                                                                    o57)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    o52 
                                                                    j4)
                                                                    (<= 
                                                                    j4 
                                                                    o51))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused23 
                                                                    j4)
                                                                    (and
                                                                    (= 
                                                                    o58 
                                                                    r3)
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused8 
                                                                    o58)
                                                                    (and
                                                                    (= 
                                                                    o59 (store 
                                                                    o58 
                                                                    j4 
                                                                    o57))
                                                                    (and
                                                                    (= 
                                                                    setqtresultqtunused1 
                                                                    o59)
                                                                    (and
                                                                    (= 
                                                                    temp___632 
                                                                    (us_tqtmk
                                                                    o59
                                                                    o53))
                                                                    (and
                                                                    (= 
                                                                    us_tqtmkqtresultqtunused1 
                                                                    temp___632)
                                                                    (and
                                                                    (= 
                                                                    (first
                                                                    temp___632) 1)
                                                                    (and
                                                                    (= 
                                                                    o60 
                                                                    (to_array
                                                                    temp___632))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused9 
                                                                    o60)
                                                                    (and
                                                                    (= 
                                                                    r2 
                                                                    o60)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused24 
                                                                    j12)
                                                                    (and
                                                                    (= 
                                                                    o61 (+ 
                                                                    j12 1))
                                                                    (and
                                                                    (= 
                                                                    infix_plqtresultqtunused4 
                                                                    o61)
                                                                    (and
                                                                    (in_range
                                                                    o61)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused3 
                                                                    o61)
                                                                    (= 
                                                                    j11 
                                                                    o61)))))))))))))))))))))))))))))))))))))
                                                                    (= 
                                                                    j21 
                                                                    j22))
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    r__last) 
                                                                    o40)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused6 
                                                                    o40)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    r__first) 
                                                                    o41)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused7 
                                                                    o41)
                                                                    (and
                                                                    (= 
                                                                    map__contentqtresultqtunused2 
                                                                    r3)
                                                                    (and
                                                                    (= 
                                                                    temp___636 
                                                                    (of_array
                                                                    r3
                                                                    o41
                                                                    o40))
                                                                    (and
                                                                    (= 
                                                                    of_arrayqtresultqtunused3 
                                                                    temp___636)
                                                                    (and
                                                                    (= 
                                                                    o42 
                                                                    (rt
                                                                    temp___636))
                                                                    (and
                                                                    (= 
                                                                    rtqtresultqtunused 
                                                                    o42)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    (first
                                                                    s2) 
                                                                    j22)
                                                                    (<= 
                                                                    j22 
                                                                    (last
                                                                    s2)))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused19 
                                                                    j22)
                                                                    (and
                                                                    (= 
                                                                    o43 
                                                                    (to_array
                                                                    s2))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused4 
                                                                    o43)
                                                                    (and
                                                                    (= 
                                                                    o44 (select 
                                                                    o43 
                                                                    j22))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused2 
                                                                    o44)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o44) 
                                                                    o45)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused8 
                                                                    o45)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o46) 
                                                                    o45)
                                                                    (and
                                                                    (= 
                                                                    of_repqtresultqtunused 
                                                                    o46)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    o41 
                                                                    j4)
                                                                    (<= 
                                                                    j4 
                                                                    o40))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused20 
                                                                    j4)
                                                                    (and
                                                                    (= 
                                                                    o47 
                                                                    r3)
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused5 
                                                                    o47)
                                                                    (and
                                                                    (= 
                                                                    o48 (store 
                                                                    o47 
                                                                    j4 
                                                                    o46))
                                                                    (and
                                                                    (= 
                                                                    setqtresultqtunused 
                                                                    o48)
                                                                    (and
                                                                    (= 
                                                                    temp___638 
                                                                    (us_tqtmk
                                                                    o48
                                                                    o42))
                                                                    (and
                                                                    (= 
                                                                    us_tqtmkqtresultqtunused 
                                                                    temp___638)
                                                                    (and
                                                                    (= 
                                                                    (first
                                                                    temp___638) 1)
                                                                    (and
                                                                    (= 
                                                                    o49 
                                                                    (to_array
                                                                    temp___638))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused6 
                                                                    o49)
                                                                    (and
                                                                    (= 
                                                                    r2 
                                                                    o49)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused21 
                                                                    j22)
                                                                    (and
                                                                    (= 
                                                                    o50 (+ 
                                                                    j22 1))
                                                                    (and
                                                                    (= 
                                                                    infix_plqtresultqtunused3 
                                                                    o50)
                                                                    (and
                                                                    (in_range
                                                                    o50)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused2 
                                                                    o50)
                                                                    (= 
                                                                    j21 
                                                                    o50)))))))))))))))))))))))))))))))))))))
                                                                    (= 
                                                                    j11 
                                                                    j12))))))))))))))))))))))))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused12 
                                                                    j4)
                                                                    (and
                                                                    (= 
                                                                    o22 (+ 
                                                                    j4 1))
                                                                    (and
                                                                    (= 
                                                                    infix_plqtresultqtunused2 
                                                                    o22)
                                                                    (and
                                                                    (in_range
                                                                    o22)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused1 
                                                                    o22)
                                                                    (and
                                                                    (= 
                                                                    j3 
                                                                    o22)
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    o24 
                                                                    (last
                                                                    s1))
                                                                    (and
                                                                    (= 
                                                                    lastqtresultqtunused3 
                                                                    o24)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused13 
                                                                    j11)
                                                                    (and
                                                                    (= 
                                                                    o25 
                                                                    (ite (<= 
                                                                    j11 
                                                                    o24)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lseqqtresultqtunused4 
                                                                    o25)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused14 
                                                                    j11)
                                                                    (and
                                                                    (= 
                                                                    o26 
                                                                    (first
                                                                    s1))
                                                                    (and
                                                                    (= 
                                                                    firstqtresultqtunused3 
                                                                    o26)
                                                                    (and
                                                                    (= 
                                                                    o27 
                                                                    (ite (<= 
                                                                    o26 
                                                                    j11)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lseqqtresultqtunused5 
                                                                    o27)
                                                                    (and
                                                                    (= 
                                                                    o28 (and 
                                                                    o27 
                                                                    o25))
                                                                    (and
                                                                    (= 
                                                                    andbqtresultqtunused2 
                                                                    o28)
                                                                    (ite (= 
                                                                    o28 true)
                                                                    (and
                                                                    (= 
                                                                    o29 
                                                                    (last
                                                                    s2))
                                                                    (and
                                                                    (= 
                                                                    lastqtresultqtunused4 
                                                                    o29)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused15 
                                                                    j21)
                                                                    (and
                                                                    (= 
                                                                    o30 
                                                                    (ite (<= 
                                                                    j21 
                                                                    o29)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lseqqtresultqtunused6 
                                                                    o30)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused16 
                                                                    j21)
                                                                    (and
                                                                    (= 
                                                                    o31 
                                                                    (first
                                                                    s2))
                                                                    (and
                                                                    (= 
                                                                    firstqtresultqtunused4 
                                                                    o31)
                                                                    (and
                                                                    (= 
                                                                    o32 
                                                                    (ite (<= 
                                                                    o31 
                                                                    j21)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lseqqtresultqtunused7 
                                                                    o32)
                                                                    (and
                                                                    (= 
                                                                    o23 (and 
                                                                    o32 
                                                                    o30))
                                                                    (= 
                                                                    andbqtresultqtunused3 
                                                                    o23))))))))))))
                                                                    (= 
                                                                    o23 false))))))))))))))
                                                                    (not
                                                                    (= 
                                                                    o23 true)))))))))))))))))))))))))))))))))))))))))))
    (and (and (and (= j11 j1) (= j21 j2)) (= j3 j)) (= r2 r1))))

(declare-const j13 Int)

(declare-const j5 Int)

(declare-const r4 (Array Int integer))

;; "r'unused"
(define-fun rqtunused4 () map__ref
  (map__refqtmk r4))

;; "j'unused"
(define-fun jqtunused3 () int__ref
  (int__refqtmk j5))

;; "j1'unused"
(define-fun j1qtunused3 () int__ref
  (int__refqtmk j13))

(declare-const o62 Int)

;; "H"
(assert (= o62 (last s1)))

(declare-const lastqtresultqtunused5 Int)

;; "H"
(assert (= lastqtresultqtunused5 o62))

(declare-const int__contentqtresultqtunused25 Int)

;; "H"
(assert (= int__contentqtresultqtunused25 j11))

(declare-const o63 Bool)

;; "H"
(assert (= o63 (ite (<= j11 o62) true false)))

(declare-const infix_lseqqtresultqtunused8 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused8 o63))

(declare-const int__contentqtresultqtunused26 Int)

;; "H"
(assert (= int__contentqtresultqtunused26 j11))

(declare-const o64 Int)

;; "H"
(assert (= o64 (first s1)))

(declare-const firstqtresultqtunused5 Int)

;; "H"
(assert (= firstqtresultqtunused5 o64))

(declare-const o65 Bool)

;; "H"
(assert (= o65 (ite (<= o64 j11) true false)))

(declare-const infix_lseqqtresultqtunused9 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused9 o65))

(declare-const o66 Bool)

;; "H"
(assert (= o66 (and o65 o63)))

(declare-const andbqtresultqtunused4 Bool)

;; "H"
(assert (= andbqtresultqtunused4 o66))

(declare-const o67 Int)

(declare-const to_repqtresultqtunused12 Int)

(declare-const o68 Int)

(declare-const to_repqtresultqtunused13 Int)

(declare-const map__contentqtresultqtunused4 (Array Int integer))

(declare-const temp___loop_entry_673qtunused us_t)

(declare-const of_arrayqtresultqtunused5 us_t)

(declare-const temp___loop_entry_672qtunused Int)

(declare-const int__contentqtresultqtunused27 Int)

(declare-const temp___loop_entry_671qtunused Int)

(declare-const int__contentqtresultqtunused28 Int)

(declare-const int__contentqtresultqtunused29 Int)

(declare-const int__contentqtresultqtunused30 Int)

(declare-const o69 Int)

(declare-const infix_plqtresultqtunused5 Int)

(declare-const range_check_qtresultqtunused4 Int)

(declare-const o70 Int)

(declare-const infix_mnqtresultqtunused1 Int)

(declare-const int__contentqtresultqtunused31 Int)

(declare-const infix_eqqtresultqtunused1 Bool)

(declare-const j14 Int)

(declare-const j6 Int)

(declare-const r5 (Array Int integer))

(declare-const rqtunused5 map__ref)

(declare-const jqtunused4 int__ref)

(declare-const j1qtunused4 int__ref)

(declare-const loop_var___46961qtunused Int)

(declare-const int__contentqtresultqtunused32 Int)

(declare-const o71 Int)

(declare-const to_repqtresultqtunused14 Int)

(declare-const o72 Int)

(declare-const to_repqtresultqtunused15 Int)

(declare-const map__contentqtresultqtunused5 (Array Int integer))

(declare-const temp___665 us_t)

(declare-const of_arrayqtresultqtunused6 us_t)

(declare-const o73 t)

(declare-const rtqtresultqtunused2 t)

(declare-const int__contentqtresultqtunused33 Int)

(declare-const o74 (Array Int integer))

(declare-const to_arrayqtresultqtunused10 (Array Int integer))

(declare-const o75 integer)

(declare-const getqtresultqtunused4 integer)

(declare-const o76 Int)

(declare-const to_repqtresultqtunused16 Int)

(declare-const o77 integer)

(declare-const of_repqtresultqtunused2 integer)

(declare-const int__contentqtresultqtunused34 Int)

(declare-const o78 (Array Int integer))

(declare-const to_arrayqtresultqtunused11 (Array Int integer))

(declare-const o79 (Array Int integer))

(declare-const setqtresultqtunused2 (Array Int integer))

(declare-const temp___667 us_t)

(declare-const us_tqtmkqtresultqtunused2 us_t)

(declare-const o80 (Array Int integer))

(declare-const to_arrayqtresultqtunused12 (Array Int integer))

(declare-const int__contentqtresultqtunused35 Int)

(declare-const o81 Int)

(declare-const infix_plqtresultqtunused6 Int)

(declare-const range_check_qtresultqtunused5 Int)

(declare-const int__contentqtresultqtunused36 Int)

(declare-const o82 Int)

(declare-const infix_plqtresultqtunused7 Int)

(declare-const range_check_qtresultqtunused6 Int)

(declare-const o83 Int)

(declare-const lastqtresultqtunused6 Int)

(declare-const int__contentqtresultqtunused37 Int)

(declare-const o84 Bool)

(declare-const infix_lseqqtresultqtunused10 Bool)

(declare-const int__contentqtresultqtunused38 Int)

(declare-const o85 Int)

(declare-const firstqtresultqtunused6 Int)

(declare-const o86 Bool)

(declare-const infix_lseqqtresultqtunused11 Bool)

(declare-const o87 Bool)

(declare-const andbqtresultqtunused5 Bool)

;; "H"
(assert
  (ite (= o66 true)
    (and
      (= (to_rep r__last) o67)
      (and
        (= to_repqtresultqtunused12 o67)
        (and
          (= (to_rep r__first) o68)
          (and
            (= to_repqtresultqtunused13 o68)
            (and
              (= map__contentqtresultqtunused4 r2)
              (and
                (= temp___loop_entry_673qtunused (of_array r2 o68 o67))
                (and
                  (= of_arrayqtresultqtunused5 (of_array r2 o68 o67))
                  (and
                    (= temp___loop_entry_672qtunused j3)
                    (and
                      (= int__contentqtresultqtunused27 j3)
                      (and
                        (= temp___loop_entry_671qtunused j11)
                        (and
                          (= int__contentqtresultqtunused28 j11)
                          (and
                            (= int__contentqtresultqtunused29 j21)
                            (and
                              (= int__contentqtresultqtunused30 j11)
                              (and
                                (= o69 (+ j11 j21))
                                (and
                                  (= infix_plqtresultqtunused5 o69)
                                  (and
                                    (in_range o69)
                                    (and
                                      (= range_check_qtresultqtunused4 
                                      o69)
                                      (and
                                        (= o70 (- o69 1))
                                        (and
                                          (= infix_mnqtresultqtunused1 
                                          o70)
                                          (and
                                            (= int__contentqtresultqtunused31 
                                            j3)
                                            (and
                                              (= infix_eqqtresultqtunused1 
                                              (ite (= j3 o70)
                                                true
                                                false))
                                              (and
                                                (= rqtunused5 (map__refqtmk
                                                                r5))
                                                (and
                                                  (= jqtunused4 (int__refqtmk
                                                                  j6))
                                                  (and
                                                    (= j1qtunused4 (int__refqtmk
                                                                    j14))
                                                    (and
                                                      (and
                                                        (= j6 (- (+ j14 
                                                        j21) 1))
                                                        (and
                                                          (forall ((l Int))
                                                            (=>
                                                              (and
                                                                (<= 2 l)
                                                                (<= l (- 
                                                                j6 1)))
                                                              (<= (to_rep
                                                                    (select 
                                                                    r5 (- l 1))) 
                                                              (to_rep
                                                                (select 
                                                                r5 l)))))
                                                          (=>
                                                            (< 1 j6)
                                                            (<= (to_rep
                                                                  (select 
                                                                  r5 (- 
                                                                  j6 1))) 
                                                            (to_rep
                                                              (select 
                                                              (to_array
                                                                s1) j14))))))
                                                      (and
                                                        (= loop_var___46961qtunused 
                                                        j14)
                                                        (and
                                                          (= int__contentqtresultqtunused32 
                                                          j14)
                                                          (and
                                                            (= (and (ite 
                                                                    (and
                                                                    (and
                                                                    (in_range1
                                                                    j14)
                                                                    (in_range1
                                                                    j6))
                                                                    (dynamic_invariant1
                                                                    (of_array
                                                                    r5
                                                                    (to_rep
                                                                    r__first)
                                                                    (to_rep
                                                                    r__last))
                                                                    false
                                                                    true
                                                                    true
                                                                    true))
                                                                    true
                                                                    false) 
                                                            (ite (and
                                                                   (<= 
                                                                   (first
                                                                    s1) 
                                                                   j14)
                                                                   (<= 
                                                                   j14 
                                                                   (last
                                                                    s1)))
                                                              true
                                                              false)) true)
                                                            (and
                                                              (= (to_rep
                                                                   r__last) 
                                                              o71)
                                                              (and
                                                                (= to_repqtresultqtunused14 
                                                                o71)
                                                                (and
                                                                  (= 
                                                                  (to_rep
                                                                    r__first) 
                                                                  o72)
                                                                  (and
                                                                    (= 
                                                                    to_repqtresultqtunused15 
                                                                    o72)
                                                                    (and
                                                                    (= 
                                                                    map__contentqtresultqtunused5 
                                                                    r5)
                                                                    (and
                                                                    (= 
                                                                    temp___665 
                                                                    (of_array
                                                                    r5
                                                                    o72
                                                                    o71))
                                                                    (and
                                                                    (= 
                                                                    of_arrayqtresultqtunused6 
                                                                    temp___665)
                                                                    (and
                                                                    (= 
                                                                    o73 
                                                                    (rt
                                                                    temp___665))
                                                                    (and
                                                                    (= 
                                                                    rtqtresultqtunused2 
                                                                    o73)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    (first
                                                                    s1) 
                                                                    j14)
                                                                    (<= 
                                                                    j14 
                                                                    (last
                                                                    s1)))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused33 
                                                                    j14)
                                                                    (and
                                                                    (= 
                                                                    o74 
                                                                    (to_array
                                                                    s1))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused10 
                                                                    o74)
                                                                    (and
                                                                    (= 
                                                                    o75 (select 
                                                                    o74 
                                                                    j14))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused4 
                                                                    o75)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o75) 
                                                                    o76)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused16 
                                                                    o76)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o77) 
                                                                    o76)
                                                                    (and
                                                                    (= 
                                                                    of_repqtresultqtunused2 
                                                                    o77)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    o72 
                                                                    j6)
                                                                    (<= 
                                                                    j6 
                                                                    o71))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused34 
                                                                    j6)
                                                                    (and
                                                                    (= 
                                                                    o78 
                                                                    r5)
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused11 
                                                                    o78)
                                                                    (and
                                                                    (= 
                                                                    o79 (store 
                                                                    o78 
                                                                    j6 
                                                                    o77))
                                                                    (and
                                                                    (= 
                                                                    setqtresultqtunused2 
                                                                    o79)
                                                                    (and
                                                                    (= 
                                                                    temp___667 
                                                                    (us_tqtmk
                                                                    o79
                                                                    o73))
                                                                    (and
                                                                    (= 
                                                                    us_tqtmkqtresultqtunused2 
                                                                    temp___667)
                                                                    (and
                                                                    (= 
                                                                    (first
                                                                    temp___667) 1)
                                                                    (and
                                                                    (= 
                                                                    o80 
                                                                    (to_array
                                                                    temp___667))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused12 
                                                                    o80)
                                                                    (and
                                                                    (= 
                                                                    r4 
                                                                    o80)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused35 
                                                                    j6)
                                                                    (and
                                                                    (= 
                                                                    o81 (+ 
                                                                    j6 1))
                                                                    (and
                                                                    (= 
                                                                    infix_plqtresultqtunused6 
                                                                    o81)
                                                                    (and
                                                                    (in_range
                                                                    o81)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused5 
                                                                    o81)
                                                                    (and
                                                                    (= 
                                                                    j5 
                                                                    o81)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused36 
                                                                    j14)
                                                                    (and
                                                                    (= 
                                                                    o82 (+ 
                                                                    j14 1))
                                                                    (and
                                                                    (= 
                                                                    infix_plqtresultqtunused7 
                                                                    o82)
                                                                    (and
                                                                    (in_range
                                                                    o82)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused6 
                                                                    o82)
                                                                    (and
                                                                    (= 
                                                                    j13 
                                                                    o82)
                                                                    (and
                                                                    (= 
                                                                    o83 
                                                                    (last
                                                                    s1))
                                                                    (and
                                                                    (= 
                                                                    lastqtresultqtunused6 
                                                                    o83)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused37 
                                                                    j13)
                                                                    (and
                                                                    (= 
                                                                    o84 
                                                                    (ite (<= 
                                                                    j13 
                                                                    o83)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lseqqtresultqtunused10 
                                                                    o84)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused38 
                                                                    j13)
                                                                    (and
                                                                    (= 
                                                                    o85 
                                                                    (first
                                                                    s1))
                                                                    (and
                                                                    (= 
                                                                    firstqtresultqtunused6 
                                                                    o85)
                                                                    (and
                                                                    (= 
                                                                    o86 
                                                                    (ite (<= 
                                                                    o85 
                                                                    j13)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lseqqtresultqtunused11 
                                                                    o86)
                                                                    (and
                                                                    (= 
                                                                    o87 (and 
                                                                    o86 
                                                                    o84))
                                                                    (and
                                                                    (= 
                                                                    andbqtresultqtunused5 
                                                                    o87)
                                                                    (not
                                                                    (= 
                                                                    o87 true)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
    (and (and (= j13 j11) (= j5 j3)) (= r4 r2))))

(declare-const r6 (Array Int integer))

;; "r'unused"
(define-fun rqtunused6 () map__ref
  (map__refqtmk r6))

(declare-const o88 Int)

;; "H"
(assert (= o88 (last s2)))

(declare-const lastqtresultqtunused7 Int)

;; "H"
(assert (= lastqtresultqtunused7 o88))

(declare-const int__contentqtresultqtunused39 Int)

;; "H"
(assert (= int__contentqtresultqtunused39 j21))

(declare-const o89 Bool)

;; "H"
(assert (= o89 (ite (<= j21 o88) true false)))

(declare-const infix_lseqqtresultqtunused12 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused12 o89))

(declare-const int__contentqtresultqtunused40 Int)

;; "H"
(assert (= int__contentqtresultqtunused40 j21))

(declare-const o90 Int)

;; "H"
(assert (= o90 (first s2)))

(declare-const firstqtresultqtunused7 Int)

;; "H"
(assert (= firstqtresultqtunused7 o90))

(declare-const o91 Bool)

;; "H"
(assert (= o91 (ite (<= o90 j21) true false)))

(declare-const infix_lseqqtresultqtunused13 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused13 o91))

(declare-const o92 Bool)

;; "H"
(assert (= o92 (and o91 o89)))

(declare-const andbqtresultqtunused6 Bool)

;; "H"
(assert (= andbqtresultqtunused6 o92))

(declare-const o93 Int)

(declare-const to_repqtresultqtunused17 Int)

(declare-const o94 Int)

(declare-const to_repqtresultqtunused18 Int)

(declare-const map__contentqtresultqtunused6 (Array Int integer))

(declare-const temp___loop_entry_697qtunused us_t)

(declare-const of_arrayqtresultqtunused7 us_t)

(declare-const temp___loop_entry_696qtunused Int)

(declare-const int__contentqtresultqtunused41 Int)

(declare-const temp___loop_entry_695qtunused Int)

(declare-const int__contentqtresultqtunused42 Int)

(declare-const int__contentqtresultqtunused43 Int)

(declare-const int__contentqtresultqtunused44 Int)

(declare-const o95 Int)

(declare-const infix_plqtresultqtunused8 Int)

(declare-const range_check_qtresultqtunused7 Int)

(declare-const o96 Int)

(declare-const infix_mnqtresultqtunused2 Int)

(declare-const int__contentqtresultqtunused45 Int)

(declare-const infix_eqqtresultqtunused2 Bool)

(declare-const j23 Int)

(declare-const j7 Int)

(declare-const r7 (Array Int integer))

(declare-const rqtunused7 map__ref)

(declare-const jqtunused5 int__ref)

(declare-const j2qtunused3 int__ref)

(declare-const loop_var___49540qtunused Int)

(declare-const int__contentqtresultqtunused46 Int)

(declare-const o97 Int)

(declare-const to_repqtresultqtunused19 Int)

(declare-const o98 Int)

(declare-const to_repqtresultqtunused20 Int)

(declare-const map__contentqtresultqtunused7 (Array Int integer))

(declare-const temp___689 us_t)

(declare-const of_arrayqtresultqtunused8 us_t)

(declare-const o99 t)

(declare-const rtqtresultqtunused3 t)

(declare-const int__contentqtresultqtunused47 Int)

(declare-const o100 (Array Int integer))

(declare-const to_arrayqtresultqtunused13 (Array Int integer))

(declare-const o101 integer)

(declare-const getqtresultqtunused5 integer)

(declare-const o102 Int)

(declare-const to_repqtresultqtunused21 Int)

(declare-const o103 integer)

(declare-const of_repqtresultqtunused3 integer)

(declare-const int__contentqtresultqtunused48 Int)

(declare-const o104 (Array Int integer))

(declare-const to_arrayqtresultqtunused14 (Array Int integer))

(declare-const o105 (Array Int integer))

(declare-const setqtresultqtunused3 (Array Int integer))

(declare-const temp___691 us_t)

(declare-const us_tqtmkqtresultqtunused3 us_t)

(declare-const o106 (Array Int integer))

(declare-const to_arrayqtresultqtunused15 (Array Int integer))

(declare-const int__contentqtresultqtunused49 Int)

(declare-const o107 Int)

(declare-const infix_plqtresultqtunused9 Int)

(declare-const range_check_qtresultqtunused8 Int)

(declare-const j8 Int)

(declare-const int__contentqtresultqtunused50 Int)

(declare-const o108 Int)

(declare-const infix_plqtresultqtunused10 Int)

(declare-const range_check_qtresultqtunused9 Int)

(declare-const j24 Int)

(declare-const j2qtunused4 int__ref)

(declare-const o109 Int)

(declare-const lastqtresultqtunused8 Int)

(declare-const int__contentqtresultqtunused51 Int)

(declare-const o110 Bool)

(declare-const infix_lseqqtresultqtunused14 Bool)

(declare-const int__contentqtresultqtunused52 Int)

(declare-const o111 Int)

(declare-const firstqtresultqtunused8 Int)

(declare-const o112 Bool)

(declare-const infix_lseqqtresultqtunused15 Bool)

(declare-const o113 Bool)

(declare-const andbqtresultqtunused7 Bool)

;; "H"
(assert
  (ite (= o92 true)
    (and
      (= (to_rep r__last) o93)
      (and
        (= to_repqtresultqtunused17 o93)
        (and
          (= (to_rep r__first) o94)
          (and
            (= to_repqtresultqtunused18 o94)
            (and
              (= map__contentqtresultqtunused6 r4)
              (and
                (= temp___loop_entry_697qtunused (of_array r4 o94 o93))
                (and
                  (= of_arrayqtresultqtunused7 (of_array r4 o94 o93))
                  (and
                    (= temp___loop_entry_696qtunused j5)
                    (and
                      (= int__contentqtresultqtunused41 j5)
                      (and
                        (= temp___loop_entry_695qtunused j21)
                        (and
                          (= int__contentqtresultqtunused42 j21)
                          (and
                            (= int__contentqtresultqtunused43 j21)
                            (and
                              (= int__contentqtresultqtunused44 j13)
                              (and
                                (= o95 (+ j13 j21))
                                (and
                                  (= infix_plqtresultqtunused8 o95)
                                  (and
                                    (in_range o95)
                                    (and
                                      (= range_check_qtresultqtunused7 
                                      o95)
                                      (and
                                        (= o96 (- o95 1))
                                        (and
                                          (= infix_mnqtresultqtunused2 
                                          o96)
                                          (and
                                            (= int__contentqtresultqtunused45 
                                            j5)
                                            (and
                                              (= infix_eqqtresultqtunused2 
                                              (ite (= j5 o96)
                                                true
                                                false))
                                              (and
                                                (= rqtunused7 (map__refqtmk
                                                                r7))
                                                (and
                                                  (= jqtunused5 (int__refqtmk
                                                                  j7))
                                                  (and
                                                    (= j2qtunused3 (int__refqtmk
                                                                    j23))
                                                    (and
                                                      (and
                                                        (= j7 (- (+ j13 
                                                        j23) 1))
                                                        (and
                                                          (forall ((l Int))
                                                            (=>
                                                              (and
                                                                (<= 2 l)
                                                                (<= l (- 
                                                                j7 1)))
                                                              (<= (to_rep
                                                                    (select 
                                                                    r7 (- l 1))) 
                                                              (to_rep
                                                                (select 
                                                                r7 l)))))
                                                          (=>
                                                            (< 1 j7)
                                                            (<= (to_rep
                                                                  (select 
                                                                  r7 (- 
                                                                  j7 1))) 
                                                            (to_rep
                                                              (select 
                                                              (to_array
                                                                s2) j23))))))
                                                      (and
                                                        (= loop_var___49540qtunused 
                                                        j23)
                                                        (and
                                                          (= int__contentqtresultqtunused46 
                                                          j23)
                                                          (and
                                                            (= (and (ite 
                                                                    (and
                                                                    (and
                                                                    (in_range1
                                                                    j23)
                                                                    (in_range1
                                                                    j7))
                                                                    (dynamic_invariant1
                                                                    (of_array
                                                                    r7
                                                                    (to_rep
                                                                    r__first)
                                                                    (to_rep
                                                                    r__last))
                                                                    false
                                                                    true
                                                                    true
                                                                    true))
                                                                    true
                                                                    false) 
                                                            (ite (and
                                                                   (<= 
                                                                   (first
                                                                    s2) 
                                                                   j23)
                                                                   (<= 
                                                                   j23 
                                                                   (last
                                                                    s2)))
                                                              true
                                                              false)) true)
                                                            (and
                                                              (= (to_rep
                                                                   r__last) 
                                                              o97)
                                                              (and
                                                                (= to_repqtresultqtunused19 
                                                                o97)
                                                                (and
                                                                  (= 
                                                                  (to_rep
                                                                    r__first) 
                                                                  o98)
                                                                  (and
                                                                    (= 
                                                                    to_repqtresultqtunused20 
                                                                    o98)
                                                                    (and
                                                                    (= 
                                                                    map__contentqtresultqtunused7 
                                                                    r7)
                                                                    (and
                                                                    (= 
                                                                    temp___689 
                                                                    (of_array
                                                                    r7
                                                                    o98
                                                                    o97))
                                                                    (and
                                                                    (= 
                                                                    of_arrayqtresultqtunused8 
                                                                    temp___689)
                                                                    (and
                                                                    (= 
                                                                    o99 
                                                                    (rt
                                                                    temp___689))
                                                                    (and
                                                                    (= 
                                                                    rtqtresultqtunused3 
                                                                    o99)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    (first
                                                                    s2) 
                                                                    j23)
                                                                    (<= 
                                                                    j23 
                                                                    (last
                                                                    s2)))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused47 
                                                                    j23)
                                                                    (and
                                                                    (= 
                                                                    o100 
                                                                    (to_array
                                                                    s2))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused13 
                                                                    o100)
                                                                    (and
                                                                    (= 
                                                                    o101 (select 
                                                                    o100 
                                                                    j23))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused5 
                                                                    o101)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o101) 
                                                                    o102)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused21 
                                                                    o102)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o103) 
                                                                    o102)
                                                                    (and
                                                                    (= 
                                                                    of_repqtresultqtunused3 
                                                                    o103)
                                                                    (and
                                                                    (and
                                                                    (<= 
                                                                    o98 
                                                                    j7)
                                                                    (<= 
                                                                    j7 
                                                                    o97))
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused48 
                                                                    j7)
                                                                    (and
                                                                    (= 
                                                                    o104 
                                                                    r7)
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused14 
                                                                    o104)
                                                                    (and
                                                                    (= 
                                                                    o105 (store 
                                                                    o104 
                                                                    j7 
                                                                    o103))
                                                                    (and
                                                                    (= 
                                                                    setqtresultqtunused3 
                                                                    o105)
                                                                    (and
                                                                    (= 
                                                                    temp___691 
                                                                    (us_tqtmk
                                                                    o105
                                                                    o99))
                                                                    (and
                                                                    (= 
                                                                    us_tqtmkqtresultqtunused3 
                                                                    temp___691)
                                                                    (and
                                                                    (= 
                                                                    (first
                                                                    temp___691) 1)
                                                                    (and
                                                                    (= 
                                                                    o106 
                                                                    (to_array
                                                                    temp___691))
                                                                    (and
                                                                    (= 
                                                                    to_arrayqtresultqtunused15 
                                                                    o106)
                                                                    (and
                                                                    (= 
                                                                    r6 
                                                                    o106)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused49 
                                                                    j7)
                                                                    (and
                                                                    (= 
                                                                    o107 (+ 
                                                                    j7 1))
                                                                    (and
                                                                    (= 
                                                                    infix_plqtresultqtunused9 
                                                                    o107)
                                                                    (and
                                                                    (in_range
                                                                    o107)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused8 
                                                                    o107)
                                                                    (and
                                                                    (= 
                                                                    j8 
                                                                    o107)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused50 
                                                                    j23)
                                                                    (and
                                                                    (= 
                                                                    o108 (+ 
                                                                    j23 1))
                                                                    (and
                                                                    (= 
                                                                    infix_plqtresultqtunused10 
                                                                    o108)
                                                                    (and
                                                                    (in_range
                                                                    o108)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused9 
                                                                    o108)
                                                                    (and
                                                                    (= 
                                                                    j2qtunused4 
                                                                    (int__refqtmk
                                                                    j24))
                                                                    (and
                                                                    (= 
                                                                    j24 
                                                                    o108)
                                                                    (and
                                                                    (= 
                                                                    o109 
                                                                    (last
                                                                    s2))
                                                                    (and
                                                                    (= 
                                                                    lastqtresultqtunused8 
                                                                    o109)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused51 
                                                                    j24)
                                                                    (and
                                                                    (= 
                                                                    o110 
                                                                    (ite (<= 
                                                                    j24 
                                                                    o109)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lseqqtresultqtunused14 
                                                                    o110)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused52 
                                                                    j24)
                                                                    (and
                                                                    (= 
                                                                    o111 
                                                                    (first
                                                                    s2))
                                                                    (and
                                                                    (= 
                                                                    firstqtresultqtunused8 
                                                                    o111)
                                                                    (and
                                                                    (= 
                                                                    o112 
                                                                    (ite (<= 
                                                                    o111 
                                                                    j24)
                                                                    true
                                                                    false))
                                                                    (and
                                                                    (= 
                                                                    infix_lseqqtresultqtunused15 
                                                                    o112)
                                                                    (and
                                                                    (= 
                                                                    o113 (and 
                                                                    o112 
                                                                    o110))
                                                                    (and
                                                                    (= 
                                                                    andbqtresultqtunused7 
                                                                    o113)
                                                                    (not
                                                                    (= 
                                                                    o113 true))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
    (= r6 r4)))

(declare-const o114 Int)

;; "Ensures"
(assert (= (to_rep r__last) o114))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused22 () Int
  o114)

(declare-const o115 Int)

;; "Ensures"
(assert (= (to_rep r__first) o115))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused23 () Int
  o115)

;; "map__content'result'unused"
(define-fun map__contentqtresultqtunused8 () (Array Int integer)
  r6)

;; "temp___611"
(define-fun temp___611 () us_t
  (of_array r6 o115 o114))

;; "of_array'result'unused"
(define-fun of_arrayqtresultqtunused9 () us_t
  temp___611)

;; "temp___612"
(define-fun temp___612 () (Array Int integer)
  r6)

;; "to_array'result'unused"
(define-fun to_arrayqtresultqtunused16 () (Array Int integer)
  temp___612)

;; "o"
(define-fun o116 () Int
  o114)

;; "last'result'unused"
(define-fun lastqtresultqtunused9 () Int
  o116)

;; "o"
(define-fun o117 () Int
  o115)

;; "first'result'unused"
(define-fun firstqtresultqtunused9 () Int
  o117)

;; "o"
(define-fun o118 () us_t
  (of_array temp___612 o117 o116))

;; "of_array'result'unused"
(define-fun of_arrayqtresultqtunused10 () us_t
  o118)

(declare-const ghc_sort__merge__result us_t)

;; "ghc_sort__merge__result'unused"
(define-fun ghc_sort__merge__resultqtunused () int_array__ref
  (int_array__refqtmk ghc_sort__merge__result))

;; "H"
(assert (= ghc_sort__merge__result o118))

;; "result'unused"
(define-fun resultqtunused () us_t
  ghc_sort__merge__result)

;; "int_array__content'result'unused"
(define-fun int_array__contentqtresultqtunused () us_t
  ghc_sort__merge__result)

(declare-const l Int)

;; "H"
(assert (<= 2 l))

;; "H"
(assert (<= l (last ghc_sort__merge__result)))

;; Goal "def'vc"
;; File "ghc_sort.ads", line 36, characters 0-0
(assert
  (not
  (<= (to_rep (select (to_array ghc_sort__merge__result) (- l 1))) (to_rep
                                                                    (select 
                                                                    (to_array
                                                                    ghc_sort__merge__result) 
                                                                    l)))))

(check-sat)
