;; produced by z3_gnatprove.drv ;;
(set-info :smt-lib-version 2.5)
(set-info :source |VC generated by SPARK 2014|)
(set-info :category industrial)
(set-info :status unknown)
;;; generated by SMT-LIB2 driver
;;; SMT-LIB2 driver: bit-vectors, common part
;;; SMT-LIB2: integer arithmetic
(declare-sort us_private 0)

(declare-const us_null_ext__ us_private)

(declare-datatypes ()
  ((int__ref (int__refqtmk (int__content Int)))))

(declare-const two_power_size_minus_one Int)

(declare-datatypes ()
  ((t__ref (t__refqtmk (t__content (_ BitVec 8))))))

(declare-sort integer 0)

;; "integer'int"
(declare-fun integerqtint (integer) Int)

;; "integer'axiom"
(assert
  (forall ((i integer))
    (and
      (<= (- 2147483648) (integerqtint i))
      (<= (integerqtint i) 2147483647))))

;; "in_range"
(define-fun in_range ((x Int)) Bool
  (and (<= (- 2147483648) x) (<= x 2147483647)))

;; "in_range"
(define-fun in_range1 ((x Int)) Bool
  (and (<= 0 x) (<= x 2147483647)))

;; "in_range"
(define-fun in_range2 ((x Int)) Bool
  (and (<= 1 x) (<= x 2147483647)))

(declare-sort character 0)

;; "in_range"
(define-fun in_range3 ((x Int)) Bool
  (and (<= 0 x) (<= x 255)))

(declare-datatypes ()
  ((character__init_wrapper
   (character__init_wrapperqtmk (rec__value character)(us_attr__init Bool)))))

(declare-const dummy character__init_wrapper)

;; "dummy__def"
(assert (= (us_attr__init dummy) false))

;; "to_rep"
(declare-fun to_rep (character) Int)

;; "of_rep"
(declare-fun of_rep (Int) character)

;; "inversion_axiom"
(assert
  (forall ((x character))
    (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)) )))

;; "range_axiom"
(assert
  (forall ((x character)) (! (in_range3 (to_rep x)) :pattern ((to_rep x)) )))

;; "coerce_axiom"
(assert
  (forall ((x Int))
    (! (=> (in_range3 x) (= (to_rep (of_rep x)) x)) :pattern ((to_rep
                                                                (of_rep x))) )))

(declare-datatypes ()
  ((map__ref
   (map__refqtmk (map__content (Array Int character__init_wrapper))))))

;; "to_rep"
(define-fun to_rep1 ((x integer)) Int
  (integerqtint x))

;; "of_rep"
(declare-fun of_rep1 (Int) integer)

;; "inversion_axiom"
(assert
  (forall ((x integer))
    (! (= (of_rep1 (to_rep1 x)) x) :pattern ((to_rep1 x)) )))

;; "range_axiom"
(assert
  (forall ((x integer)) (! (in_range (to_rep1 x)) :pattern ((to_rep1 x)) )))

;; "coerce_axiom"
(assert
  (forall ((x Int))
    (! (=> (in_range x) (= (to_rep1 (of_rep1 x)) x)) :pattern ((to_rep1
                                                                 (of_rep1 x))) )))

;; "dynamic_property"
(define-fun dynamic_property ((range_first Int) (range_last Int) (low Int) (high Int)) Bool
  (and
    (in_range low)
    (and
      (in_range high)
      (=> (<= low high) (and (in_range2 low) (in_range2 high))))))

(declare-const value__size Int)

(declare-const object__size Int)

(declare-const component__size Int)

(declare-const alignment Int)

;; "value__size_axiom"
(assert (<= 0 value__size))

;; "object__size_axiom"
(assert (<= 0 object__size))

;; "component__size_axiom"
(assert (<= 0 component__size))

;; "alignment_axiom"
(assert (<= 0 alignment))

;; "in_range"
(define-fun in_range4 ((x Int)) Bool
  (and (<= 48 x) (<= x 57)))

;; "char_of_byte_10"
(define-fun char_of_byte_10 ((param__i (_ BitVec 8))) Int
  (ite (= param__i #x00)
    48
    (ite (= param__i #x01)
      49
      (ite (= param__i #x02)
        50
        (ite (= param__i #x03)
          51
          (ite (= param__i #x04)
            52
            (ite (= param__i #x05)
              53
              (ite (= param__i #x06)
                54
                (ite (= param__i #x07) 55 (ite (= param__i #x08) 56 57))))))))))

(declare-const v (_ BitVec 8))

(declare-const s__first integer)

(declare-const s__last integer)

(declare-const attr__ATTRIBUTE_MODULUS (_ BitVec 8))

(declare-datatypes ()
  ((t (tqtmk (rec__first integer)(rec__last integer)))))

;; "mk"
(declare-fun mk (Int
  Int) t)

;; "mk_def"
(assert
  (forall ((f Int) (l Int))
    (! (=>
         (in_range f)
         (=>
           (in_range l)
           (and
             (= (to_rep1 (rec__first (mk f l))) f)
             (= (to_rep1 (rec__last (mk f l))) l)))) :pattern ((mk f l)) )))

(declare-datatypes ()
  ((us_t (us_tqtmk (elts (Array Int character__init_wrapper))(rt t)))))

;; "to_array"
(define-fun to_array ((a us_t)) (Array Int character__init_wrapper)
  (elts a))

;; "of_array"
(define-fun of_array ((a (Array Int character__init_wrapper)) (f Int) (l Int)) us_t
  (us_tqtmk a (mk f l)))

;; "first"
(define-fun first ((a us_t)) Int
  (to_rep1 (rec__first (rt a))))

;; "last"
(define-fun last ((a us_t)) Int
  (to_rep1 (rec__last (rt a))))

(declare-const value__size1 Int)

(declare-const object__size1 Int)

(declare-const component__size1 Int)

(declare-const alignment1 Int)

;; "value__size_axiom"
(assert (<= 0 value__size1))

;; "object__size_axiom"
(assert (<= 0 object__size1))

;; "component__size_axiom"
(assert (<= 0 component__size1))

;; "alignment_axiom"
(assert (<= 0 alignment1))

(declare-const s (Array Int character__init_wrapper))

(declare-const last1 Int)

;; "last'unused"
(define-fun lastqtunused () int__ref
  (int__refqtmk last1))

;; "s'unused"
(define-fun sqtunused () map__ref
  (map__refqtmk s))

;; "s'unused"
(define-fun sqtunused1 () map__ref
  (map__refqtmk s))

;; "last'unused"
(define-fun lastqtunused1 () int__ref
  (int__refqtmk last1))

;; "Assume"
(assert true)

;; "Assume"
(assert (dynamic_property 1 2147483647 (to_rep1 s__first) (to_rep1 s__last)))

;; "Assume"
(assert (in_range1 last1))

;; "Assume"
(assert
  (and
    (and
      (< (to_rep1 s__last) 2147483647)
      (and (<= (to_rep1 s__first) last1) (<= last1 (to_rep1 s__last))))
    (<= (to_rep1 s__first) (- last1 2))))

(declare-const o Int)

;; "Ensures"
(assert (= (to_rep1 s__last) o))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused () Int
  o)

(declare-const o1 Int)

;; "Ensures"
(assert (= (to_rep1 s__first) o1))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused1 () Int
  o1)

;; "map__content'result'unused"
(define-fun map__contentqtresultqtunused () (Array Int character__init_wrapper)
  s)

;; "temp___700"
(define-fun temp___700 () us_t
  (of_array s o1 o))

;; "of_array'result'unused"
(define-fun of_arrayqtresultqtunused () us_t
  temp___700)

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused () Int
  last1)

(declare-const o2 Int)

;; "Ensures"
(assert (= (to_rep1 s__last) o2))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused2 () Int
  o2)

(declare-const o3 Int)

;; "Ensures"
(assert (= (to_rep1 s__first) o3))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused3 () Int
  o3)

;; "map__content'result'unused"
(define-fun map__contentqtresultqtunused1 () (Array Int character__init_wrapper)
  s)

;; "temp___699"
(define-fun temp___699 () us_t
  (of_array s o3 o2))

;; "of_array'result'unused"
(define-fun of_arrayqtresultqtunused1 () us_t
  temp___699)

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused1 () Int
  last1)

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused2 () Int
  last1)

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused3 () Int
  last1)

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused4 () Int
  last1)

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused5 () Int
  last1)

(declare-const w (_ BitVec 8))

;; "w'unused"
(define-fun wqtunused () t__ref
  (t__refqtmk w))

;; "H"
(assert (= w v))

;; "Assume"
(assert true)

(declare-const s1 (Array Int character__init_wrapper))

(declare-const last2 Int)

;; "last'unused"
(define-fun lastqtunused2 () int__ref
  (int__refqtmk last2))

;; "s'unused"
(define-fun sqtunused2 () map__ref
  (map__refqtmk s1))

;; "H"
(assert
  (or
    (exists ((o4 Int))
      (and
        (= (to_rep1 s__last) o4)
        (exists ((o5 Int))
          (and
            (= (to_rep1 s__first) o5)
            (exists ((o6 Int))
              (and
                (= (to_rep1 s__last) o6)
                (exists ((o7 Int))
                  (and
                    (= (to_rep1 s__first) o7)
                    (let ((o8 (bvurem w #x0A)))
                      (let ((o9 (char_of_byte_10 o8)))
                        (and
                          (and
                            (in_range4 o9)
                            (= o9 (ite (= o8 #x00)
                                    48
                                    (ite (= o8 #x01)
                                      49
                                      (ite (= o8 #x02)
                                        50
                                        (ite (= o8 #x03)
                                          51
                                          (ite (= o8 #x04)
                                            52
                                            (ite (= o8 #x05)
                                              53
                                              (ite (= o8 #x06)
                                                54
                                                (ite (= o8 #x07)
                                                  55
                                                  (ite (= o8 #x08) 56 57)))))))))))
                          (exists ((o10 character))
                            (and
                              (= (to_rep o10) o9)
                              (and
                                (and (<= o7 last1) (<= last1 o6))
                                (exists ((s2 (Array Int character__init_wrapper)))
                                  (and
                                    (= s2 (to_array
                                            (us_tqtmk
                                              (store s last1 (character__init_wrapperqtmk
                                                               o10
                                                               true))
                                              (rt (of_array s o7 o6)))))
                                    (let ((o11 (- last1 1)))
                                      (and
                                        (in_range1 o11)
                                        (exists ((last3 Int))
                                          (and
                                            (= last3 o11)
                                            (exists ((w1 (_ BitVec 8)))
                                              (and
                                                (= w1 (bvudiv w #x0A))
                                                (and
                                                  (not (= w1 #x00))
                                                  (exists ((o12 Int))
                                                    (and
                                                      (= (to_rep1 s__last) o12)
                                                      (exists ((o13 Int))
                                                        (and
                                                          (= (to_rep1
                                                               s__first) o13)
                                                          (let ((o14 (bvurem w1 #x0A)))
                                                            (let ((o15 
                                                              (char_of_byte_10
                                                                o14)))
                                                              (and
                                                                (and
                                                                  (in_range4
                                                                    o15)
                                                                  (= o15 
                                                                  (ite (= o14 #x00)
                                                                    48
                                                                    (ite (= o14 #x01)
                                                                    49
                                                                    (ite (= o14 #x02)
                                                                    50
                                                                    (ite (= o14 #x03)
                                                                    51
                                                                    (ite (= o14 #x04)
                                                                    52
                                                                    (ite (= o14 #x05)
                                                                    53
                                                                    (ite (= o14 #x06)
                                                                    54
                                                                    (ite (= o14 #x07)
                                                                    55
                                                                    (ite (= o14 #x08)
                                                                    56
                                                                    57)))))))))))
                                                                (exists 
                                                                  ((o16 character))
                                                                  (and
                                                                    (= 
                                                                    (to_rep
                                                                    o16) o15)
                                                                    (and
                                                                    (and
                                                                    (<= o13 last3)
                                                                    (<= last3 o12))
                                                                    (exists 
                                                                    ((s3 (Array Int character__init_wrapper)))
                                                                    (and
                                                                    (= s3 
                                                                    (to_array
                                                                    (us_tqtmk
                                                                    (store s2 last3 
                                                                    (character__init_wrapperqtmk
                                                                    o16
                                                                    true))
                                                                    (rt
                                                                    (of_array
                                                                    s2
                                                                    o13
                                                                    o12)))))
                                                                    (let ((o17 (- last3 1)))
                                                                    (and
                                                                    (in_range1
                                                                    o17)
                                                                    (exists 
                                                                    ((last4 Int))
                                                                    (and
                                                                    (= last4 o17)
                                                                    (exists 
                                                                    ((w2 (_ BitVec 8)))
                                                                    (and
                                                                    (= w2 (bvudiv w1 #x0A))
                                                                    (and
                                                                    (not
                                                                    (= w2 #x00))
                                                                    (exists 
                                                                    ((o18 Int))
                                                                    (and
                                                                    (= 
                                                                    (to_rep1
                                                                    s__last) o18)
                                                                    (exists 
                                                                    ((o19 Int))
                                                                    (and
                                                                    (= 
                                                                    (to_rep1
                                                                    s__first) o19)
                                                                    (let ((o20 (bvurem w2 #x0A)))
                                                                    (let ((o21 
                                                                    (char_of_byte_10
                                                                    o20)))
                                                                    (and
                                                                    (and
                                                                    (in_range4
                                                                    o21)
                                                                    (= o21 
                                                                    (ite (= o20 #x00)
                                                                    48
                                                                    (ite (= o20 #x01)
                                                                    49
                                                                    (ite (= o20 #x02)
                                                                    50
                                                                    (ite (= o20 #x03)
                                                                    51
                                                                    (ite (= o20 #x04)
                                                                    52
                                                                    (ite (= o20 #x05)
                                                                    53
                                                                    (ite (= o20 #x06)
                                                                    54
                                                                    (ite (= o20 #x07)
                                                                    55
                                                                    (ite (= o20 #x08)
                                                                    56
                                                                    57)))))))))))
                                                                    (exists 
                                                                    ((o22 character))
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o22) o21)
                                                                    (and
                                                                    (and
                                                                    (<= o19 last4)
                                                                    (<= last4 o18))
                                                                    (and
                                                                    (= 
                                                                    s1 
                                                                    (to_array
                                                                    (us_tqtmk
                                                                    (store s3 last4 
                                                                    (character__init_wrapperqtmk
                                                                    o22
                                                                    true))
                                                                    (rt
                                                                    (of_array
                                                                    s3
                                                                    o19
                                                                    o18)))))
                                                                    (let ((o23 (- last4 1)))
                                                                    (and
                                                                    (in_range1
                                                                    o23)
                                                                    (and
                                                                    (= 
                                                                    last2 o23)
                                                                    (exists 
                                                                    ((w3 (_ BitVec 8)))
                                                                    (and
                                                                    (= w3 (bvudiv w2 #x0A))
                                                                    (not
                                                                    (= w3 #x00))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
    (exists ((o4 Int))
      (and
        (= (to_rep1 s__last) o4)
        (exists ((o5 Int))
          (and
            (= (to_rep1 s__first) o5)
            (exists ((o6 Int))
              (and
                (= (to_rep1 s__last) o6)
                (exists ((o7 Int))
                  (and
                    (= (to_rep1 s__first) o7)
                    (let ((o8 (bvurem w #x0A)))
                      (let ((o9 (char_of_byte_10 o8)))
                        (and
                          (and
                            (in_range4 o9)
                            (= o9 (ite (= o8 #x00)
                                    48
                                    (ite (= o8 #x01)
                                      49
                                      (ite (= o8 #x02)
                                        50
                                        (ite (= o8 #x03)
                                          51
                                          (ite (= o8 #x04)
                                            52
                                            (ite (= o8 #x05)
                                              53
                                              (ite (= o8 #x06)
                                                54
                                                (ite (= o8 #x07)
                                                  55
                                                  (ite (= o8 #x08) 56 57)))))))))))
                          (exists ((o10 character))
                            (and
                              (= (to_rep o10) o9)
                              (and
                                (and (<= o7 last1) (<= last1 o6))
                                (exists ((s2 (Array Int character__init_wrapper)))
                                  (and
                                    (= s2 (to_array
                                            (us_tqtmk
                                              (store s last1 (character__init_wrapperqtmk
                                                               o10
                                                               true))
                                              (rt (of_array s o7 o6)))))
                                    (let ((o11 (- last1 1)))
                                      (and
                                        (in_range1 o11)
                                        (exists ((last3 Int))
                                          (and
                                            (= last3 o11)
                                            (exists ((w1 (_ BitVec 8)))
                                              (and
                                                (= w1 (bvudiv w #x0A))
                                                (ite (= w1 #x00)
                                                  (and
                                                    (= s1 s2)
                                                    (= last2 last3))
                                                  (exists ((o12 Int))
                                                    (and
                                                      (= (to_rep1 s__last) o12)
                                                      (exists ((o13 Int))
                                                        (and
                                                          (= (to_rep1
                                                               s__first) o13)
                                                          (let ((o14 (bvurem w1 #x0A)))
                                                            (let ((o15 
                                                              (char_of_byte_10
                                                                o14)))
                                                              (and
                                                                (and
                                                                  (in_range4
                                                                    o15)
                                                                  (= o15 
                                                                  (ite (= o14 #x00)
                                                                    48
                                                                    (ite (= o14 #x01)
                                                                    49
                                                                    (ite (= o14 #x02)
                                                                    50
                                                                    (ite (= o14 #x03)
                                                                    51
                                                                    (ite (= o14 #x04)
                                                                    52
                                                                    (ite (= o14 #x05)
                                                                    53
                                                                    (ite (= o14 #x06)
                                                                    54
                                                                    (ite (= o14 #x07)
                                                                    55
                                                                    (ite (= o14 #x08)
                                                                    56
                                                                    57)))))))))))
                                                                (exists 
                                                                  ((o16 character))
                                                                  (and
                                                                    (= 
                                                                    (to_rep
                                                                    o16) o15)
                                                                    (and
                                                                    (and
                                                                    (<= o13 last3)
                                                                    (<= last3 o12))
                                                                    (exists 
                                                                    ((s3 (Array Int character__init_wrapper)))
                                                                    (and
                                                                    (= s3 
                                                                    (to_array
                                                                    (us_tqtmk
                                                                    (store s2 last3 
                                                                    (character__init_wrapperqtmk
                                                                    o16
                                                                    true))
                                                                    (rt
                                                                    (of_array
                                                                    s2
                                                                    o13
                                                                    o12)))))
                                                                    (let ((o17 (- last3 1)))
                                                                    (and
                                                                    (in_range1
                                                                    o17)
                                                                    (exists 
                                                                    ((last4 Int))
                                                                    (and
                                                                    (= last4 o17)
                                                                    (exists 
                                                                    ((w2 (_ BitVec 8)))
                                                                    (and
                                                                    (= w2 (bvudiv w1 #x0A))
                                                                    (ite (= w2 #x00)
                                                                    (and
                                                                    (= 
                                                                    s1 s3)
                                                                    (= 
                                                                    last2 last4))
                                                                    (exists 
                                                                    ((o18 Int))
                                                                    (and
                                                                    (= 
                                                                    (to_rep1
                                                                    s__last) o18)
                                                                    (exists 
                                                                    ((o19 Int))
                                                                    (and
                                                                    (= 
                                                                    (to_rep1
                                                                    s__first) o19)
                                                                    (let ((o20 (bvurem w2 #x0A)))
                                                                    (let ((o21 
                                                                    (char_of_byte_10
                                                                    o20)))
                                                                    (and
                                                                    (and
                                                                    (in_range4
                                                                    o21)
                                                                    (= o21 
                                                                    (ite (= o20 #x00)
                                                                    48
                                                                    (ite (= o20 #x01)
                                                                    49
                                                                    (ite (= o20 #x02)
                                                                    50
                                                                    (ite (= o20 #x03)
                                                                    51
                                                                    (ite (= o20 #x04)
                                                                    52
                                                                    (ite (= o20 #x05)
                                                                    53
                                                                    (ite (= o20 #x06)
                                                                    54
                                                                    (ite (= o20 #x07)
                                                                    55
                                                                    (ite (= o20 #x08)
                                                                    56
                                                                    57)))))))))))
                                                                    (exists 
                                                                    ((o22 character))
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o22) o21)
                                                                    (and
                                                                    (and
                                                                    (<= o19 last4)
                                                                    (<= last4 o18))
                                                                    (and
                                                                    (= 
                                                                    s1 
                                                                    (to_array
                                                                    (us_tqtmk
                                                                    (store s3 last4 
                                                                    (character__init_wrapperqtmk
                                                                    o22
                                                                    true))
                                                                    (rt
                                                                    (of_array
                                                                    s3
                                                                    o19
                                                                    o18)))))
                                                                    (let ((o23 (- last4 1)))
                                                                    (and
                                                                    (in_range1
                                                                    o23)
                                                                    (and
                                                                    (= 
                                                                    last2 o23)
                                                                    (exists 
                                                                    ((w3 (_ BitVec 8)))
                                                                    (and
                                                                    (= w3 (bvudiv w2 #x0A))
                                                                    (= w3 #x00)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))

;; "temp___719"
(define-fun temp___719 () us_t
  (let ((temp___717 (+ last2 1))) (of_array s1 temp___717 last1)))

(declare-const temp___720 Int)

;; "H"
(assert (<= (first temp___719) temp___720))

;; "H"
(assert (<= temp___720 (last temp___719)))

;; Goal "def'vc"
;; File "ipv4_parsing.adb", line 153, characters 0-0
(assert
  (not (= (us_attr__init (select (to_array temp___719) temp___720)) true)))

(check-sat)
