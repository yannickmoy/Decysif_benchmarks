;; produced by z3_gnatprove.drv ;;
(set-info :smt-lib-version 2.5)
(set-info :source |VC generated by SPARK 2014|)
(set-info :category industrial)
(set-info :status unknown)
;;; generated by SMT-LIB2 driver
;;; SMT-LIB2 driver: bit-vectors, common part
;;; SMT-LIB2: integer arithmetic
(declare-sort us_private 0)

(declare-const us_null_ext__ us_private)

(declare-datatypes ()
  ((int__ref (int__refqtmk (int__content Int)))))

(declare-datatypes ()
  ((bool__ref (bool__refqtmk (bool__content Bool)))))

(declare-sort extended_index_type 0)

;; "extended_index_type'int"
(declare-fun extended_index_typeqtint (extended_index_type) Int)

;; "extended_index_type'axiom"
(assert
  (forall ((i extended_index_type))
    (and
      (<= 0 (extended_index_typeqtint i))
      (<= (extended_index_typeqtint i) 100))))

;; "in_range"
(define-fun in_range ((x Int)) Bool
  (and (<= 0 x) (<= x 100)))

;; "to_rep"
(define-fun to_rep ((x extended_index_type)) Int
  (extended_index_typeqtint x))

;; "of_rep"
(declare-fun of_rep (Int) extended_index_type)

;; "inversion_axiom"
(assert
  (forall ((x extended_index_type))
    (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)) )))

;; "range_axiom"
(assert
  (forall ((x extended_index_type))
    (! (in_range (to_rep x)) :pattern ((to_rep x)) )))

;; "coerce_axiom"
(assert
  (forall ((x Int))
    (! (=> (in_range x) (= (to_rep (of_rep x)) x)) :pattern ((to_rep
                                                               (of_rep x))) )))

(declare-sort position_type 0)

;; "in_range"
(define-fun in_range1 ((x Int)) Bool
  (and (<= 0 x) (<= x 2)))

;; "to_rep"
(declare-fun to_rep1 (position_type) Int)

;; "of_rep"
(declare-fun of_rep1 (Int) position_type)

;; "inversion_axiom"
(assert
  (forall ((x position_type))
    (! (= (of_rep1 (to_rep1 x)) x) :pattern ((to_rep1 x)) )))

;; "range_axiom"
(assert
  (forall ((x position_type))
    (! (in_range1 (to_rep1 x)) :pattern ((to_rep1 x)) )))

;; "coerce_axiom"
(assert
  (forall ((x Int))
    (! (=> (in_range1 x) (= (to_rep1 (of_rep1 x)) x)) :pattern ((to_rep1
                                                                  (of_rep1 x))) )))

(declare-datatypes ()
  ((us_split_fields
   (us_split_fieldsqtmk
   (rec__binary_trees__cell__left extended_index_type)(rec__binary_trees__cell__right extended_index_type)(rec__binary_trees__cell__parent extended_index_type)(rec__binary_trees__cell__position position_type)))))

(declare-datatypes ()
  ((us_rep (us_repqtmk (us_split_fields1 us_split_fields)))))

;; "bool_eq"
(define-fun bool_eq ((a us_rep) (b us_rep)) Bool
  (ite (and
         (= (to_rep (rec__binary_trees__cell__left (us_split_fields1 a))) 
         (to_rep
           (rec__binary_trees__cell__left (us_split_fields1 b))))
         (and
           (and
             (= (to_rep
                  (rec__binary_trees__cell__right (us_split_fields1 a))) 
             (to_rep
               (rec__binary_trees__cell__right (us_split_fields1 b))))
             (= (to_rep
                  (rec__binary_trees__cell__parent (us_split_fields1 a))) 
             (to_rep
               (rec__binary_trees__cell__parent (us_split_fields1 b)))))
           (= (to_rep1
                (rec__binary_trees__cell__position (us_split_fields1 a))) 
           (to_rep1
             (rec__binary_trees__cell__position (us_split_fields1 b))))))
    true
    false))

(declare-const value__size Int)

(declare-const object__size Int)

(declare-const alignment Int)

;; "value__size_axiom"
(assert (<= 0 value__size))

;; "object__size_axiom"
(assert (<= 0 object__size))

;; "alignment_axiom"
(assert (<= 0 alignment))

(declare-const binary_trees__cell__left__first__bit Int)

(declare-const binary_trees__cell__left__last__bit Int)

(declare-const binary_trees__cell__left__position Int)

;; "binary_trees__cell__left__first__bit_axiom"
(assert (<= 0 binary_trees__cell__left__first__bit))

;; "binary_trees__cell__left__last__bit_axiom"
(assert
  (< binary_trees__cell__left__first__bit binary_trees__cell__left__last__bit))

;; "binary_trees__cell__left__position_axiom"
(assert (<= 0 binary_trees__cell__left__position))

(declare-const binary_trees__cell__right__first__bit Int)

(declare-const binary_trees__cell__right__last__bit Int)

(declare-const binary_trees__cell__right__position Int)

;; "binary_trees__cell__right__first__bit_axiom"
(assert (<= 0 binary_trees__cell__right__first__bit))

;; "binary_trees__cell__right__last__bit_axiom"
(assert
  (< binary_trees__cell__right__first__bit binary_trees__cell__right__last__bit))

;; "binary_trees__cell__right__position_axiom"
(assert (<= 0 binary_trees__cell__right__position))

(declare-const binary_trees__cell__parent__first__bit Int)

(declare-const binary_trees__cell__parent__last__bit Int)

(declare-const binary_trees__cell__parent__position Int)

;; "binary_trees__cell__parent__first__bit_axiom"
(assert (<= 0 binary_trees__cell__parent__first__bit))

;; "binary_trees__cell__parent__last__bit_axiom"
(assert
  (< binary_trees__cell__parent__first__bit binary_trees__cell__parent__last__bit))

;; "binary_trees__cell__parent__position_axiom"
(assert (<= 0 binary_trees__cell__parent__position))

(declare-const binary_trees__cell__position__first__bit Int)

(declare-const binary_trees__cell__position__last__bit Int)

(declare-const binary_trees__cell__position__position Int)

;; "binary_trees__cell__position__first__bit_axiom"
(assert (<= 0 binary_trees__cell__position__first__bit))

;; "binary_trees__cell__position__last__bit_axiom"
(assert
  (< binary_trees__cell__position__first__bit binary_trees__cell__position__last__bit))

;; "binary_trees__cell__position__position_axiom"
(assert (<= 0 binary_trees__cell__position__position))

(declare-const dummy us_rep)

(declare-const value__size1 Int)

(declare-const object__size1 Int)

(declare-const component__size Int)

(declare-const alignment1 Int)

;; "value__size_axiom"
(assert (<= 0 value__size1))

;; "object__size_axiom"
(assert (<= 0 object__size1))

;; "component__size_axiom"
(assert (<= 0 component__size))

;; "alignment_axiom"
(assert (<= 0 alignment1))

(declare-datatypes ()
  ((us_split_fields2
   (us_split_fieldsqtmk1
   (rec__binary_trees__forest__s extended_index_type)(rec__binary_trees__forest__c (Array Int us_rep))))))

(declare-datatypes ()
  ((us_split_fields__ref
   (us_split_fields__refqtmk (us_split_fields__content us_split_fields2)))))

(declare-datatypes ()
  ((us_rep1 (us_repqtmk1 (us_split_fields3 us_split_fields2)))))

(declare-const value__size2 Int)

(declare-const object__size2 Int)

(declare-const alignment2 Int)

;; "value__size_axiom"
(assert (<= 0 value__size2))

;; "object__size_axiom"
(assert (<= 0 object__size2))

;; "alignment_axiom"
(assert (<= 0 alignment2))

(declare-const binary_trees__forest__s__first__bit Int)

(declare-const binary_trees__forest__s__last__bit Int)

(declare-const binary_trees__forest__s__position Int)

;; "binary_trees__forest__s__first__bit_axiom"
(assert (<= 0 binary_trees__forest__s__first__bit))

;; "binary_trees__forest__s__last__bit_axiom"
(assert
  (< binary_trees__forest__s__first__bit binary_trees__forest__s__last__bit))

;; "binary_trees__forest__s__position_axiom"
(assert (<= 0 binary_trees__forest__s__position))

(declare-const binary_trees__forest__c__first__bit Int)

(declare-const binary_trees__forest__c__last__bit Int)

(declare-const binary_trees__forest__c__position Int)

;; "binary_trees__forest__c__first__bit_axiom"
(assert (<= 0 binary_trees__forest__c__first__bit))

;; "binary_trees__forest__c__last__bit_axiom"
(assert
  (< binary_trees__forest__c__first__bit binary_trees__forest__c__last__bit))

;; "binary_trees__forest__c__position_axiom"
(assert (<= 0 binary_trees__forest__c__position))

(declare-const dummy1 us_rep1)

(declare-sort us_rep2 0)

(declare-const value__size3 Int)

(declare-const object__size3 Int)

(declare-const alignment3 Int)

;; "value__size_axiom"
(assert (<= 0 value__size3))

;; "object__size_axiom"
(assert (<= 0 object__size3))

;; "alignment_axiom"
(assert (<= 0 alignment3))

(declare-const dummy2 us_rep2)

(declare-const value__size4 Int)

(declare-const object__size4 Int)

(declare-const alignment4 Int)

;; "value__size_axiom"
(assert (<= 0 value__size4))

;; "object__size_axiom"
(assert (<= 0 object__size4))

;; "alignment_axiom"
(assert (<= 0 alignment4))

(declare-const dummy3 Int)

;; "last"
(declare-fun last (us_rep2) Int)

;; "in_range"
(define-fun in_range2 ((x Int)) Bool
  (and (<= 0 x) (<= x 2147483647)))

;; "length"
(define-fun length ((param__container us_rep2)) Int
  (- (+ (last param__container) 1) 1))

;; "get"
(declare-fun get (us_rep2
  Int) Int)

;; "oeq"
(define-fun oeq ((param__left us_rep2) (param__right us_rep2)) Bool
  (and (= (length param__left) (length param__right)) (ite (forall ((n Int))
                                                             (=>
                                                               (and
                                                                 (<= 1 n)
                                                                 (<= n 
                                                                 (last
                                                                   param__left)))
                                                               (= (get
                                                                    param__left
                                                                    n) 
                                                               (get
                                                                 param__right
                                                                 n))))
                                                        true
                                                        false)))

(declare-datatypes ()
  ((us_split_fields4
   (us_split_fieldsqtmk2
   (rec__tree_model__path_type__a us_rep2)(rec__tree_model__path_type__k Bool)))))

(declare-datatypes ()
  ((us_rep3 (us_repqtmk2 (us_split_fields5 us_split_fields4)))))

(declare-const value__size5 Int)

(declare-const object__size5 Int)

(declare-const alignment5 Int)

;; "value__size_axiom"
(assert (<= 0 value__size5))

;; "object__size_axiom"
(assert (<= 0 object__size5))

;; "alignment_axiom"
(assert (<= 0 alignment5))

(declare-const tree_model__path_type__a__first__bit Int)

(declare-const tree_model__path_type__a__last__bit Int)

(declare-const tree_model__path_type__a__position Int)

;; "tree_model__path_type__a__first__bit_axiom"
(assert (<= 0 tree_model__path_type__a__first__bit))

;; "tree_model__path_type__a__last__bit_axiom"
(assert
  (< tree_model__path_type__a__first__bit tree_model__path_type__a__last__bit))

;; "tree_model__path_type__a__position_axiom"
(assert (<= 0 tree_model__path_type__a__position))

(declare-const tree_model__path_type__k__first__bit Int)

(declare-const tree_model__path_type__k__last__bit Int)

(declare-const tree_model__path_type__k__position Int)

;; "tree_model__path_type__k__first__bit_axiom"
(assert (<= 0 tree_model__path_type__k__first__bit))

;; "tree_model__path_type__k__last__bit_axiom"
(assert
  (< tree_model__path_type__k__first__bit tree_model__path_type__k__last__bit))

;; "tree_model__path_type__k__position_axiom"
(assert (<= 0 tree_model__path_type__k__position))

(declare-const dummy4 us_rep3)

;; "model"
(declare-fun model1 (us_rep1
  Int) (Array Int us_rep3))

;; "valid_root"
(declare-fun valid_root (us_rep1
  Int) Bool)

;; "parent"
(declare-fun parent (us_rep1
  Int) Int)

;; "position"
(define-fun position ((param__f us_rep1) (param__i Int)) Int
  (to_rep1
    (rec__binary_trees__cell__position
      (us_split_fields1
        (let ((temp___173 (rec__binary_trees__forest__c
                            (us_split_fields3 param__f))))
          (select temp___173 param__i))))))

;; "is_add"
(declare-fun is_add (us_rep2
  Int
  us_rep2) Bool)

;; "in_range"
(define-fun in_range3 ((x Int)) Bool
  (and (<= 1 x) (<= x 100)))

(declare-const value__size6 Int)

(declare-const object__size6 Int)

(declare-const component__size1 Int)

(declare-const alignment6 Int)

;; "value__size_axiom"
(assert (<= 0 value__size6))

;; "object__size_axiom"
(assert (<= 0 object__size6))

;; "component__size_axiom"
(assert (<= 0 component__size1))

;; "alignment_axiom"
(assert (<= 0 alignment6))

;; "dynamic_invariant"
(define-fun dynamic_invariant ((temp___expr_414 (Array Int us_rep3)) (temp___is_init_410 Bool) (temp___skip_constant_411 Bool) (temp___do_toplevel_412 Bool) (temp___do_typ_inv_413 Bool)) Bool
  (forall ((temp___415 Int))
    (=>
      (and (<= 1 temp___415) (<= temp___415 100))
      (< (length
           (rec__tree_model__path_type__a
             (us_split_fields5 (select temp___expr_414 temp___415)))) 100))))

;; "dynamic_invariant"
(define-fun dynamic_invariant1 ((temp___expr_319 us_rep1) (temp___is_init_315 Bool) (temp___skip_constant_316 Bool) (temp___do_toplevel_317 Bool) (temp___do_typ_inv_318 Bool)) Bool
  true)

(declare-const rliteral extended_index_type)

;; "rliteral_axiom"
(assert (= (extended_index_typeqtint rliteral) 0))

;; "tree_structure"
(define-fun tree_structure ((param__f us_rep1)) Bool
  (and (and (and (and (and (and (and (and (ite (forall ((i Int))
                                                 (=>
                                                   (and
                                                     (<= (+ (to_rep
                                                              (rec__binary_trees__forest__s
                                                                (us_split_fields3
                                                                  param__f))) 1) i)
                                                     (<= i 100))
                                                   (= (bool_eq
                                                        (let ((temp___191 
                                                          (rec__binary_trees__forest__c
                                                            (us_split_fields3
                                                              param__f))))
                                                          (select temp___191 i))
                                                        (us_repqtmk
                                                          (us_split_fieldsqtmk
                                                            rliteral
                                                            rliteral
                                                            rliteral
                                                            (of_rep1 2)))) true)))
                                            true
                                            false) (ite (forall ((i Int))
                                                          (=>
                                                            (and
                                                              (<= 1 i)
                                                              (<= i 100))
                                                            (let ((temp___196 
                                                              (to_rep
                                                                (rec__binary_trees__cell__parent
                                                                  (us_split_fields1
                                                                    (let ((temp___194 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___194 i)))))))
                                                              (and
                                                                (<= 0 temp___196)
                                                                (<= temp___196 
                                                                (to_rep
                                                                  (rec__binary_trees__forest__s
                                                                    (us_split_fields3
                                                                    param__f))))))))
                                                     true
                                                     false)) (ite (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (let ((temp___199 
                                                                    (to_rep
                                                                    (rec__binary_trees__cell__left
                                                                    (us_split_fields1
                                                                    (let ((temp___197 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___197 i)))))))
                                                                    (and
                                                                    (<= 0 temp___199)
                                                                    (<= temp___199 
                                                                    (to_rep
                                                                    (rec__binary_trees__forest__s
                                                                    (us_split_fields3
                                                                    param__f))))))))
                                                               true
                                                               false)) 
  (ite (forall ((i Int))
         (=>
           (and (<= 1 i) (<= i 100))
           (let ((temp___202 (to_rep
                               (rec__binary_trees__cell__right
                                 (us_split_fields1
                                   (let ((temp___200 (rec__binary_trees__forest__c
                                                       (us_split_fields3
                                                         param__f))))
                                     (select temp___200 i)))))))
             (and
               (<= 0 temp___202)
               (<= temp___202 (to_rep
                                (rec__binary_trees__forest__s
                                  (us_split_fields3 param__f))))))))
    true
    false)) (ite (forall ((i Int))
                   (=>
                     (and (<= 1 i) (<= i 100))
                     (=>
                       (= (to_rep1
                            (rec__binary_trees__cell__position
                              (us_split_fields1
                                (let ((temp___204 (rec__binary_trees__forest__c
                                                    (us_split_fields3
                                                      param__f))))
                                  (select temp___204 i))))) 2)
                       (= (to_rep
                            (rec__binary_trees__cell__parent
                              (us_split_fields1
                                (let ((temp___203 (rec__binary_trees__forest__c
                                                    (us_split_fields3
                                                      param__f))))
                                  (select temp___203 i))))) 0))))
              true
              false)) (ite (forall ((i Int))
                             (=>
                               (and (<= 1 i) (<= i 100))
                               (=>
                                 (not
                                   (= (to_rep
                                        (rec__binary_trees__cell__left
                                          (us_split_fields1
                                            (let ((temp___209 (rec__binary_trees__forest__c
                                                                (us_split_fields3
                                                                  param__f))))
                                              (select temp___209 i))))) 0))
                                 (and
                                   (= (to_rep1
                                        (rec__binary_trees__cell__position
                                          (us_split_fields1
                                            (let ((temp___205 (rec__binary_trees__forest__c
                                                                (us_split_fields3
                                                                  param__f))))
                                              (select temp___205 (to_rep
                                                                   (rec__binary_trees__cell__left
                                                                    (us_split_fields1
                                                                    (let ((temp___206 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___206 i)))))))))) 0)
                                   (= (to_rep
                                        (rec__binary_trees__cell__parent
                                          (us_split_fields1
                                            (let ((temp___207 (rec__binary_trees__forest__c
                                                                (us_split_fields3
                                                                  param__f))))
                                              (select temp___207 (to_rep
                                                                   (rec__binary_trees__cell__left
                                                                    (us_split_fields1
                                                                    (let ((temp___208 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___208 i)))))))))) i)))))
                        true
                        false)) (ite (forall ((i Int))
                                       (=>
                                         (and (<= 1 i) (<= i 100))
                                         (=>
                                           (not
                                             (= (to_rep
                                                  (rec__binary_trees__cell__right
                                                    (us_split_fields1
                                                      (let ((temp___214 
                                                        (rec__binary_trees__forest__c
                                                          (us_split_fields3
                                                            param__f))))
                                                        (select temp___214 i))))) 0))
                                           (and
                                             (= (to_rep1
                                                  (rec__binary_trees__cell__position
                                                    (us_split_fields1
                                                      (let ((temp___210 
                                                        (rec__binary_trees__forest__c
                                                          (us_split_fields3
                                                            param__f))))
                                                        (select temp___210 
                                                        (to_rep
                                                          (rec__binary_trees__cell__right
                                                            (us_split_fields1
                                                              (let ((temp___211 
                                                                (rec__binary_trees__forest__c
                                                                  (us_split_fields3
                                                                    param__f))))
                                                                (select temp___211 i)))))))))) 1)
                                             (= (to_rep
                                                  (rec__binary_trees__cell__parent
                                                    (us_split_fields1
                                                      (let ((temp___212 
                                                        (rec__binary_trees__forest__c
                                                          (us_split_fields3
                                                            param__f))))
                                                        (select temp___212 
                                                        (to_rep
                                                          (rec__binary_trees__cell__right
                                                            (us_split_fields1
                                                              (let ((temp___213 
                                                                (rec__binary_trees__forest__c
                                                                  (us_split_fields3
                                                                    param__f))))
                                                                (select temp___213 i)))))))))) i)))))
                                  true
                                  false)) (ite (forall ((i Int))
                                                 (=>
                                                   (and (<= 1 i) (<= i 100))
                                                   (=>
                                                     (and
                                                       (not
                                                         (= (to_rep
                                                              (rec__binary_trees__cell__parent
                                                                (us_split_fields1
                                                                  (let ((temp___217 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___217 i))))) 0))
                                                       (= (to_rep1
                                                            (rec__binary_trees__cell__position
                                                              (us_split_fields1
                                                                (let ((temp___218 
                                                                  (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                  (select temp___218 i))))) 0))
                                                     (= (to_rep
                                                          (rec__binary_trees__cell__left
                                                            (us_split_fields1
                                                              (let ((temp___215 
                                                                (rec__binary_trees__forest__c
                                                                  (us_split_fields3
                                                                    param__f))))
                                                                (select temp___215 
                                                                (to_rep
                                                                  (rec__binary_trees__cell__parent
                                                                    (us_split_fields1
                                                                    (let ((temp___216 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___216 i)))))))))) i))))
                                            true
                                            false)) (ite (forall ((i Int))
                                                           (=>
                                                             (and
                                                               (<= 1 i)
                                                               (<= i 100))
                                                             (=>
                                                               (and
                                                                 (not
                                                                   (= 
                                                                   (to_rep
                                                                    (rec__binary_trees__cell__parent
                                                                    (us_split_fields1
                                                                    (let ((temp___221 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___221 i))))) 0))
                                                                 (= (to_rep1
                                                                    (rec__binary_trees__cell__position
                                                                    (us_split_fields1
                                                                    (let ((temp___222 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___222 i))))) 1))
                                                               (= (to_rep
                                                                    (rec__binary_trees__cell__right
                                                                    (us_split_fields1
                                                                    (let ((temp___219 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___219 
                                                                    (to_rep
                                                                    (rec__binary_trees__cell__parent
                                                                    (us_split_fields1
                                                                    (let ((temp___220 
                                                                    (rec__binary_trees__forest__c
                                                                    (us_split_fields3
                                                                    param__f))))
                                                                    (select temp___220 i)))))))))) i))))
                                                      true
                                                      false)))

;; "type_invariant"
(define-fun type_invariant ((temp___327 us_rep1)) Bool
  (= (tree_structure temp___327) true))

;; "oeq"
(declare-fun oeq1 ((Array Int us_rep3)
  (Array Int us_rep3)) Bool)

;; "ole"
(define-fun ole ((param__left us_rep2) (param__right us_rep2)) Bool
  (and (<= (length param__left) (length param__right)) (ite (forall ((n Int))
                                                              (=>
                                                                (and
                                                                  (<= 1 n)
                                                                  (<= n 
                                                                  (last
                                                                    param__left)))
                                                                (= (get
                                                                    param__left
                                                                    n) 
                                                                (get
                                                                  param__right
                                                                  n))))
                                                         true
                                                         false)))

;; "is_concat"
(declare-fun is_concat (us_rep2
  us_rep2
  us_rep2) Bool)

(declare-const root Int)

(declare-const v Int)

(declare-const r266b Int)

;; "in_range"
(define-fun in_range4 ((x Int)) Bool
  (and (<= 0 x) (<= x 1)))

;; "size"
(define-fun size ((param__f us_rep1)) Int
  (to_rep (rec__binary_trees__forest__s (us_split_fields3 param__f))))

;; "olt"
(define-fun olt ((param__left us_rep2) (param__right us_rep2)) Bool
  (and (< (length param__left) (length param__right)) (ite (forall ((n Int))
                                                             (=>
                                                               (and
                                                                 (<= 1 n)
                                                                 (<= n 
                                                                 (last
                                                                   param__left)))
                                                               (= (get
                                                                    param__left
                                                                    n) 
                                                               (get
                                                                 param__right
                                                                 n))))
                                                        true
                                                        false)))

;; "valid_root__post_axiom"
(assert
  (forall ((param__f us_rep1))
    (forall ((param__i Int))
      (! (=>
           (and
             (and
               (dynamic_invariant1 param__f true false true true)
               (in_range3 param__i))
             (type_invariant param__f))
           (=>
             (< (size param__f) param__i)
             (not (= (valid_root param__f param__i) true)))) :pattern (
      (valid_root
        param__f
        param__i)) ))))

;; "valid_root__def_axiom"
(assert
  (forall ((param__f us_rep1))
    (forall ((param__i Int))
      (! (=
           (= (valid_root param__f param__i) true)
           (and
             (<= param__i (to_rep
                            (rec__binary_trees__forest__s
                              (us_split_fields3 param__f))))
             (= (to_rep1
                  (rec__binary_trees__cell__position
                    (us_split_fields1
                      (let ((temp___332 (rec__binary_trees__forest__c
                                          (us_split_fields3 param__f))))
                        (select temp___332 param__i))))) 2))) :pattern (
      (valid_root
        param__f
        param__i)) ))))

;; "parent__post_axiom"
(assert
  (forall ((param__f us_rep1))
    (forall ((param__i Int))
      (! (=>
           (and
             (and
               (dynamic_invariant1 param__f true false true true)
               (in_range3 param__i))
             (type_invariant param__f))
           (let ((result (parent param__f param__i)))
             (and
               (and
                 (=> (= (valid_root param__f param__i) true) (= result 0))
                 (=> (= (size param__f) 0) (= result 0)))
               (in_range result)))) :pattern ((parent param__f param__i)) ))))

;; "parent__def_axiom"
(assert
  (forall ((param__f us_rep1))
    (forall ((param__i Int))
      (! (= (parent param__f param__i) (to_rep
                                         (rec__binary_trees__cell__parent
                                           (us_split_fields1
                                             (let ((temp___335 (rec__binary_trees__forest__c
                                                                 (us_split_fields3
                                                                   param__f))))
                                               (select temp___335 param__i)))))) :pattern (
      (parent
        param__f
        param__i)) ))))

;; "last__post_axiom"
(assert
  (forall ((param__container us_rep2))
    (! (in_range2 (last param__container)) :pattern ((last param__container)) )))

;; "get__post_axiom"
(assert
  (forall ((param__container us_rep2))
    (forall ((param__position Int))
      (! (=>
           (and
             (in_range2 param__position)
             (and
               (<= 1 param__position)
               (<= param__position (last param__container))))
           (in_range4 (get param__container param__position))) :pattern (
      (get
        param__container
        param__position)) ))))

;; "is_add__post_axiom"
(assert true)

;; "is_add__def_axiom"
(assert
  (forall ((param__s1 us_rep2) (param__s2 us_rep2))
    (forall ((param__d Int))
      (! (=
           (= (is_add param__s1 param__d param__s2) true)
           (and
             (and
               (= (- (length param__s2) 1) (length param__s1))
               (= (olt param__s1 param__s2) true))
             (= (get param__s2 (last param__s2)) param__d))) :pattern (
      (is_add
        param__s1
        param__d
        param__s2)) ))))

;; "model__post_axiom"
(assert
  (forall ((param__f us_rep1))
    (forall ((param__root Int))
      (! (=>
           (and
             (and
               (and
                 (dynamic_invariant1 param__f true false true true)
                 (in_range3 param__root))
               (type_invariant param__f))
             (= (valid_root param__f param__root) true))
           (let ((result (model1 param__f param__root)))
             (and
               (and
                 (and
                   (and
                     (and
                       (= (rec__tree_model__path_type__k
                            (us_split_fields5 (select result param__root))) true)
                       (= (last
                            (rec__tree_model__path_type__a
                              (us_split_fields5 (select result param__root)))) 0))
                     (forall ((i Int))
                       (=>
                         (and (<= 1 i) (<= i 100))
                         (=>
                           (not (= i param__root))
                           (ite (and
                                  (not (= (parent param__f i) 0))
                                  (= (rec__tree_model__path_type__k
                                       (us_split_fields5
                                         (select result (parent param__f i)))) true))
                             (= (rec__tree_model__path_type__k
                                  (us_split_fields5 (select result i))) true)
                             (not
                               (= (rec__tree_model__path_type__k
                                    (us_split_fields5 (select result i))) true)))))))
                   (forall ((i Int))
                     (=>
                       (and (<= 1 i) (<= i 100))
                       (ite (and
                              (= (rec__tree_model__path_type__k
                                   (us_split_fields5 (select result i))) true)
                              (not (= i param__root)))
                         (= (is_add
                              (rec__tree_model__path_type__a
                                (us_split_fields5
                                  (select result (parent param__f i))))
                              (position param__f i)
                              (rec__tree_model__path_type__a
                                (us_split_fields5 (select result i)))) true)
                         (= (last
                              (rec__tree_model__path_type__a
                                (us_split_fields5 (select result i)))) 0)))))
                 (forall ((i Int))
                   (=>
                     (and (<= 1 i) (<= i 100))
                     (=>
                       (= (rec__tree_model__path_type__k
                            (us_split_fields5 (select result i))) true)
                       (forall ((j Int))
                         (=>
                           (and (<= 1 j) (<= j 100))
                           (=>
                             (and
                               (= (rec__tree_model__path_type__k
                                    (us_split_fields5 (select result j))) true)
                               (= (oeq
                                    (rec__tree_model__path_type__a
                                      (us_split_fields5 (select result i)))
                                    (rec__tree_model__path_type__a
                                      (us_split_fields5 (select result j)))) true))
                             (= j i))))))))
               (dynamic_invariant result true false true true)))) :pattern (
      (model1
        param__f
        param__root)) ))))

;; "oeq__post_axiom"
(assert true)

;; "oeq__def_axiom"
(assert
  (forall ((param__m1 (Array Int us_rep3)) (param__m2 (Array Int us_rep3)))
    (! (=
         (= (oeq1 param__m1 param__m2) true)
         (forall ((i Int))
           (=>
             (and (<= 1 i) (<= i 100))
             (and
               (= (oeq
                    (rec__tree_model__path_type__a
                      (us_split_fields5 (select param__m1 i)))
                    (rec__tree_model__path_type__a
                      (us_split_fields5 (select param__m2 i)))) true)
               (=
                 (= (rec__tree_model__path_type__k
                      (us_split_fields5 (select param__m1 i))) true)
                 (= (rec__tree_model__path_type__k
                      (us_split_fields5 (select param__m2 i))) true)))))) :pattern (
    (oeq1
      param__m1
      param__m2)) )))

;; "is_concat__post_axiom"
(assert true)

;; "is_concat__def_axiom"
(assert
  (forall ((param__q us_rep2) (param__v us_rep2) (param__p us_rep2))
    (! (=
         (= (is_concat param__q param__v param__p) true)
         (and
           (and
             (and
               (= (- (length param__p) (length param__v)) (length param__q))
               (forall ((i Int))
                 (=>
                   (and (<= 1 i) (<= i (last param__q)))
                   (= (get param__p i) (get param__q i)))))
             (forall ((i Int))
               (=>
                 (and (<= 1 i) (<= i (last param__v)))
                 (= (get param__p (+ i (last param__q))) (get param__v i)))))
           (forall ((i Int))
             (=>
               (and (<= (+ (last param__q) 1) i) (<= i (last param__p)))
               (= (get param__v (- i (last param__q))) (get param__p i)))))) :pattern (
    (is_concat
      param__q
      param__v
      param__p)) )))

(declare-const spark__branch Bool)

(declare-const f__split_fields us_split_fields2)

(declare-const f_old__split_fields us_split_fields2)

;; "f_old__split_fields'unused"
(define-fun f_old__split_fieldsqtunused () us_split_fields__ref
  (us_split_fields__refqtmk f_old__split_fields))

;; "f__split_fields'unused"
(define-fun f__split_fieldsqtunused () us_split_fields__ref
  (us_split_fields__refqtmk f__split_fields))

;; "Assume"
(assert (in_range3 root))

;; "Assume"
(assert (in_range v))

;; "Assume"
(assert
  (dynamic_invariant1 (us_repqtmk1 f_old__split_fields) true false true true))

;; "Assume"
(assert
  (dynamic_invariant1 (us_repqtmk1 f__split_fields) true false true true))

;; "Assume"
(assert (in_range3 root))

;; "Assume"
(assert (in_range v))

;; "Assume"
(assert
  (and
    (type_invariant (us_repqtmk1 f_old__split_fields))
    (type_invariant (us_repqtmk1 f__split_fields))))

;; "Assume"
(assert
  (and
    (and
      (and
        (and
          (and
            (and
              (and
                (and
                  (and
                    (and
                      (and
                        (and
                          (= (to_rep
                               (rec__binary_trees__forest__s f__split_fields)) 
                          (to_rep
                            (rec__binary_trees__forest__s
                              f_old__split_fields)))
                          (<= root (to_rep
                                     (rec__binary_trees__forest__s
                                       f__split_fields))))
                        (= (to_rep1
                             (rec__binary_trees__cell__position
                               (us_split_fields1
                                 (select (rec__binary_trees__forest__c
                                           f_old__split_fields) root)))) 2))
                      (= (to_rep1
                           (rec__binary_trees__cell__position
                             (us_split_fields1
                               (select (rec__binary_trees__forest__c
                                         f__split_fields) root)))) 2))
                    (not (= v root)))
                  (and
                    (<= 1 v)
                    (<= v (to_rep
                            (rec__binary_trees__forest__s f__split_fields)))))
                (= (to_rep1
                     (rec__binary_trees__cell__position
                       (us_split_fields1
                         (select (rec__binary_trees__forest__c
                                   f_old__split_fields) v)))) 2))
              (not
                (= (to_rep
                     (rec__binary_trees__cell__parent
                       (us_split_fields1
                         (select (rec__binary_trees__forest__c
                                   f__split_fields) v)))) 0)))
            (= (rec__tree_model__path_type__k
                 (us_split_fields5
                   (select (model1 (us_repqtmk1 f_old__split_fields) root) 
                   (to_rep
                     (rec__binary_trees__cell__parent
                       (us_split_fields1
                         (select (rec__binary_trees__forest__c
                                   f__split_fields) v))))))) true))
          (forall ((i Int))
            (=>
              (and (<= 1 i) (<= i 100))
              (=>
                (not (= i v))
                (= (to_rep
                     (rec__binary_trees__cell__parent
                       (us_split_fields1
                         (select (rec__binary_trees__forest__c
                                   f__split_fields) i)))) (to_rep
                                                            (rec__binary_trees__cell__parent
                                                              (us_split_fields1
                                                                (select 
                                                                (rec__binary_trees__forest__c
                                                                  f_old__split_fields) i)))))))))
        (forall ((i Int))
          (=>
            (and (<= 1 i) (<= i 100))
            (=>
              (not (= i v))
              (= (to_rep1
                   (rec__binary_trees__cell__position
                     (us_split_fields1
                       (select (rec__binary_trees__forest__c f__split_fields) i)))) 
              (to_rep1
                (rec__binary_trees__cell__position
                  (us_split_fields1
                    (select (rec__binary_trees__forest__c
                              f_old__split_fields) i)))))))))
      (forall ((j Int))
        (=>
          (and (<= 1 j) (<= j 100))
          (=>
            (not
              (= (rec__tree_model__path_type__k
                   (us_split_fields5
                     (select (model1 (us_repqtmk1 f_old__split_fields) root) j))) true))
            (= (to_rep
                 (rec__binary_trees__cell__left
                   (us_split_fields1
                     (select (rec__binary_trees__forest__c f__split_fields) j)))) 
            (to_rep
              (rec__binary_trees__cell__left
                (us_split_fields1
                  (select (rec__binary_trees__forest__c f_old__split_fields) j)))))))))
    (forall ((j Int))
      (=>
        (and (<= 1 j) (<= j 100))
        (=>
          (not
            (= (rec__tree_model__path_type__k
                 (us_split_fields5
                   (select (model1 (us_repqtmk1 f_old__split_fields) root) j))) true))
          (= (to_rep
               (rec__binary_trees__cell__right
                 (us_split_fields1
                   (select (rec__binary_trees__forest__c f__split_fields) j)))) 
          (to_rep
            (rec__binary_trees__cell__right
              (us_split_fields1
                (select (rec__binary_trees__forest__c f_old__split_fields) j))))))))))

(declare-const n Int)

(declare-const nqtunused int__ref)

;; "H"
(assert (= nqtunused (int__refqtmk n)))

;; "H"
(assert (= n 1))

(declare-const int__contentqtresultqtunused Int)

;; "H"
(assert (= int__contentqtresultqtunused n))

(declare-const o Bool)

;; "H"
(assert (= o (ite (<= n 100) true false)))

(declare-const infix_lseqqtresultqtunused Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused o))

(declare-const int__contentqtresultqtunused1 Int)

;; "H"
(assert (= int__contentqtresultqtunused1 n))

(declare-const o1 Bool)

;; "H"
(assert (= o1 (ite (<= 1 n) true false)))

(declare-const infix_lseqqtresultqtunused1 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused1 o1))

(declare-const o2 Bool)

;; "H"
(assert (= o2 (and o1 o)))

(declare-const andbqtresultqtunused Bool)

;; "H"
(assert (= andbqtresultqtunused o2))

(declare-const usfqtunused Bool)

(declare-const anyqtresultqtunused Bool)

(declare-const n1 Int)

(declare-const nqtunused1 int__ref)

(declare-const int__contentqtresultqtunused2 Int)

(declare-const infix_eqqtresultqtunused Bool)

(declare-const usf Int)

(declare-const anyqtresultqtunused1 Int)

(declare-const iqtunused Int)

(declare-const o3 Bool)

(declare-const infix_lseqqtresultqtunused2 Bool)

(declare-const o4 Bool)

(declare-const infix_lseqqtresultqtunused3 Bool)

(declare-const andbqtresultqtunused1 Bool)

;; "H"
(assert
  (=>
    (= o2 true)
    (and
      (and
        (= anyqtresultqtunused1 usf)
        (and
          (= iqtunused usf)
          (and
            (= o3 (ite (<= usf 100) true false))
            (and
              (= infix_lseqqtresultqtunused2 o3)
              (and
                (= o4 (ite (<= 1 usf) true false))
                (and
                  (= infix_lseqqtresultqtunused3 o4)
                  (= andbqtresultqtunused1 (and o4 o3))))))))
      (and
        (= usfqtunused (ite (forall ((i Int))
                              (=>
                                (and (<= 1 i) (<= i 100))
                                (=>
                                  (and
                                    (and
                                      (< 0 (length
                                             (rec__tree_model__path_type__a
                                               (us_split_fields5
                                                 (select (model1
                                                           (us_repqtmk1
                                                             f__split_fields)
                                                           root) v)))))
                                      (= (rec__tree_model__path_type__k
                                           (us_split_fields5
                                             (select (model1
                                                       (us_repqtmk1
                                                         f__split_fields)
                                                       root) i))) true))
                                    (<= (last
                                          (rec__tree_model__path_type__a
                                            (us_split_fields5
                                              (select (model1
                                                        (us_repqtmk1
                                                          f__split_fields)
                                                        root) i)))) (- 
                                    n 1)))
                                  (ite (= (ole
                                            (rec__tree_model__path_type__a
                                              (us_split_fields5
                                                (select (model1
                                                          (us_repqtmk1
                                                            f__split_fields)
                                                          root) v)))
                                            (rec__tree_model__path_type__a
                                              (us_split_fields5
                                                (select (model1
                                                          (us_repqtmk1
                                                            f__split_fields)
                                                          root) i)))) true)
                                    (= (rec__tree_model__path_type__k
                                         (us_split_fields5
                                           (select (model1
                                                     (us_repqtmk1
                                                       f_old__split_fields)
                                                     v) i))) true)
                                    (= (rec__tree_model__path_type__k
                                         (us_split_fields5
                                           (select (model1
                                                     (us_repqtmk1
                                                       f_old__split_fields)
                                                     root) i))) true)))))
                         true
                         false))
        (and
          (= anyqtresultqtunused (ite (forall ((i Int))
                                        (=>
                                          (and (<= 1 i) (<= i 100))
                                          (=>
                                            (and
                                              (and
                                                (< 0 (length
                                                       (rec__tree_model__path_type__a
                                                         (us_split_fields5
                                                           (select (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) 
                                                           v)))))
                                                (= (rec__tree_model__path_type__k
                                                     (us_split_fields5
                                                       (select (model1
                                                                 (us_repqtmk1
                                                                   f__split_fields)
                                                                 root) i))) true))
                                              (<= (last
                                                    (rec__tree_model__path_type__a
                                                      (us_split_fields5
                                                        (select (model1
                                                                  (us_repqtmk1
                                                                    f__split_fields)
                                                                  root) i)))) (- 
                                              n 1)))
                                            (ite (= (ole
                                                      (rec__tree_model__path_type__a
                                                        (us_split_fields5
                                                          (select (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) 
                                                          v)))
                                                      (rec__tree_model__path_type__a
                                                        (us_split_fields5
                                                          (select (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))) true)
                                              (= (rec__tree_model__path_type__k
                                                   (us_split_fields5
                                                     (select (model1
                                                               (us_repqtmk1
                                                                 f_old__split_fields)
                                                               v) i))) true)
                                              (= (rec__tree_model__path_type__k
                                                   (us_split_fields5
                                                     (select (model1
                                                               (us_repqtmk1
                                                                 f_old__split_fields)
                                                               root) i))) true)))))
                                   true
                                   false))
          (and
            (= nqtunused1 (int__refqtmk n1))
            (and
              (and
                (forall ((i Int))
                  (=>
                    (and (<= 1 i) (<= i 100))
                    (=>
                      (and
                        (and
                          (< 0 (length
                                 (rec__tree_model__path_type__a
                                   (us_split_fields5
                                     (select (model1
                                               (us_repqtmk1 f__split_fields)
                                               root) v)))))
                          (= (rec__tree_model__path_type__k
                               (us_split_fields5
                                 (select (model1
                                           (us_repqtmk1 f__split_fields)
                                           root) i))) true))
                        (<= (last
                              (rec__tree_model__path_type__a
                                (us_split_fields5
                                  (select (model1
                                            (us_repqtmk1 f__split_fields)
                                            root) i)))) (- n1 1)))
                      (ite (= (ole
                                (rec__tree_model__path_type__a
                                  (us_split_fields5
                                    (select (model1
                                              (us_repqtmk1 f__split_fields)
                                              root) v)))
                                (rec__tree_model__path_type__a
                                  (us_split_fields5
                                    (select (model1
                                              (us_repqtmk1 f__split_fields)
                                              root) i)))) true)
                        (= (rec__tree_model__path_type__k
                             (us_split_fields5
                               (select (model1
                                         (us_repqtmk1 f_old__split_fields)
                                         v) i))) true)
                        (= (rec__tree_model__path_type__k
                             (us_split_fields5
                               (select (model1
                                         (us_repqtmk1 f_old__split_fields)
                                         root) i))) true)))))
                (and
                  (forall ((i Int))
                    (=>
                      (and (<= 1 i) (<= i 100))
                      (=>
                        (and
                          (= (rec__tree_model__path_type__k
                               (us_split_fields5
                                 (select (model1
                                           (us_repqtmk1 f_old__split_fields)
                                           root) i))) true)
                          (<= (last
                                (rec__tree_model__path_type__a
                                  (us_split_fields5
                                    (select (model1
                                              (us_repqtmk1
                                                f_old__split_fields)
                                              root) i)))) (- n1 1)))
                        (= (rec__tree_model__path_type__k
                             (us_split_fields5
                               (select (model1
                                         (us_repqtmk1 f__split_fields)
                                         root) i))) true))))
                  (forall ((i Int))
                    (=>
                      (and (<= 1 i) (<= i 100))
                      (=>
                        (and
                          (and
                            (= (rec__tree_model__path_type__k
                                 (us_split_fields5
                                   (select (model1
                                             (us_repqtmk1
                                               f_old__split_fields)
                                             v) i))) true)
                            (<= (last
                                  (rec__tree_model__path_type__a
                                    (us_split_fields5
                                      (select (model1
                                                (us_repqtmk1
                                                  f_old__split_fields)
                                                v) i)))) (- n1 1)))
                          (= (rec__tree_model__path_type__k
                               (us_split_fields5
                                 (select (model1
                                           (us_repqtmk1 f__split_fields)
                                           root) (to_rep
                                                   (rec__binary_trees__cell__parent
                                                     (us_split_fields1
                                                       (select (rec__binary_trees__forest__c
                                                                 f__split_fields) 
                                                       v))))))) true))
                        (= (rec__tree_model__path_type__k
                             (us_split_fields5
                               (select (model1
                                         (us_repqtmk1 f__split_fields)
                                         root) i))) true))))))
              (and
                (and (<= 1 n1) (<= n1 100))
                (and
                  (= int__contentqtresultqtunused2 n1)
                  (and
                    (= infix_eqqtresultqtunused (ite (= n1 100) true false))
                    (= n1 100)))))))))))

(declare-const spark__branch1 Bool)

;; "spark__branch'unused"
(define-fun spark__branchqtunused () bool__ref
  (bool__refqtmk spark__branch1))

(declare-const n2 Int)

(declare-const nqtunused2 int__ref)

;; "H"
(assert (= nqtunused2 (int__refqtmk n2)))

;; "H"
(assert (= n2 1))

(declare-const int__contentqtresultqtunused3 Int)

;; "H"
(assert (= int__contentqtresultqtunused3 n2))

(declare-const o5 Bool)

;; "H"
(assert (= o5 (ite (<= n2 100) true false)))

(declare-const infix_lseqqtresultqtunused4 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused4 o5))

(declare-const int__contentqtresultqtunused4 Int)

;; "H"
(assert (= int__contentqtresultqtunused4 n2))

(declare-const o6 Bool)

;; "H"
(assert (= o6 (ite (<= 1 n2) true false)))

(declare-const infix_lseqqtresultqtunused5 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused5 o6))

(declare-const o7 Bool)

;; "H"
(assert (= o7 (and o6 o5)))

(declare-const andbqtresultqtunused2 Bool)

;; "H"
(assert (= andbqtresultqtunused2 o7))

(declare-const usfqtunused1 Bool)

(declare-const anyqtresultqtunused2 Bool)

(declare-const spark__branch2 Bool)

(declare-const n3 Int)

(declare-const nqtunused3 int__ref)

(declare-const int__contentqtresultqtunused5 Int)

(declare-const infix_eqqtresultqtunused1 Bool)

(declare-const ki Int)

(declare-const kiqtunused int__ref)

(declare-const int__contentqtresultqtunused6 Int)

(declare-const o8 Bool)

(declare-const infix_lseqqtresultqtunused6 Bool)

(declare-const int__contentqtresultqtunused7 Int)

(declare-const o9 Bool)

(declare-const infix_lseqqtresultqtunused7 Bool)

(declare-const o10 Bool)

(declare-const andbqtresultqtunused3 Bool)

(declare-const spark__branch3 Bool)

(declare-const spark__branchqtunused1 bool__ref)

(declare-const spark__branch4 Bool)

(declare-const spark__branchqtunused2 bool__ref)

(declare-const usfqtunused2 Bool)

(declare-const anyqtresultqtunused3 Bool)

(declare-const ki1 Int)

(declare-const kiqtunused1 int__ref)

(declare-const int__contentqtresultqtunused8 Int)

(declare-const infix_eqqtresultqtunused2 Bool)

(declare-const usf1 Int)

(declare-const anyqtresultqtunused4 Int)

(declare-const iqtunused1 Int)

(declare-const int__contentqtresultqtunused9 Int)

(declare-const o11 Bool)

(declare-const infix_lseqqtresultqtunused8 Bool)

(declare-const o12 Bool)

(declare-const infix_lseqqtresultqtunused9 Bool)

(declare-const andbqtresultqtunused4 Bool)

(declare-const o13 Bool)

(declare-const bool__contentqtresultqtunused Bool)

(declare-const us_split_fields__contentqtresultqtunused us_split_fields2)

(declare-const us_repqtmkqtresultqtunused us_rep1)

(declare-const us_split_fieldsqtresultqtunused us_split_fields2)

(declare-const temp___3254 (Array Int us_rep))

(declare-const rec__binary_trees__forest__cqtresultqtunused (Array Int us_rep))

(declare-const int__contentqtresultqtunused10 Int)

(declare-const o14 us_rep)

(declare-const getqtresultqtunused us_rep)

(declare-const o15 us_split_fields)

(declare-const us_split_fieldsqtresultqtunused1 us_split_fields)

(declare-const o16 position_type)

(declare-const rec__binary_trees__cell__positionqtresultqtunused position_type)

(declare-const o17 Int)

(declare-const to_repqtresultqtunused Int)

(declare-const range_check_qtresultqtunused Int)

(declare-const us_split_fields__contentqtresultqtunused1 us_split_fields2)

(declare-const temp___compl_3248 us_rep1)

(declare-const us_repqtmkqtresultqtunused1 us_rep1)

(declare-const temp___compl_3249 Int)

(declare-const us_split_fields__contentqtresultqtunused2 us_split_fields2)

(declare-const temp___3250 us_rep1)

(declare-const us_repqtmkqtresultqtunused2 us_rep1)

(declare-const temp___3252 (Array Int us_rep3))

(declare-const modelqtresultqtunused (Array Int us_rep3))

(declare-const o18 us_rep3)

(declare-const getqtresultqtunused1 us_rep3)

(declare-const o19 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused2 us_split_fields4)

(declare-const o20 us_rep2)

(declare-const rec__tree_model__path_type__aqtresultqtunused us_rep2)

(declare-const us_split_fields__contentqtresultqtunused3 us_split_fields2)

(declare-const temp___compl_3242 us_rep1)

(declare-const us_repqtmkqtresultqtunused3 us_rep1)

(declare-const temp___compl_3243 Int)

(declare-const us_split_fields__contentqtresultqtunused4 us_split_fields2)

(declare-const temp___3244 us_rep1)

(declare-const us_repqtmkqtresultqtunused4 us_rep1)

(declare-const temp___3246 (Array Int us_rep3))

(declare-const modelqtresultqtunused1 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused11 Int)

(declare-const o21 us_rep3)

(declare-const getqtresultqtunused2 us_rep3)

(declare-const o22 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused3 us_split_fields4)

(declare-const o23 us_rep2)

(declare-const rec__tree_model__path_type__aqtresultqtunused1 us_rep2)

(declare-const us_split_fields__contentqtresultqtunused5 us_split_fields2)

(declare-const temp___compl_3236 us_rep1)

(declare-const us_repqtmkqtresultqtunused5 us_rep1)

(declare-const o24 Int)

(declare-const range_check_qtresultqtunused1 Int)

(declare-const us_split_fields__contentqtresultqtunused6 us_split_fields2)

(declare-const temp___3238 us_rep1)

(declare-const us_repqtmkqtresultqtunused6 us_rep1)

(declare-const temp___3240 (Array Int us_rep3))

(declare-const modelqtresultqtunused2 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused12 Int)

(declare-const o25 us_rep3)

(declare-const getqtresultqtunused3 us_rep3)

(declare-const o26 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused4 us_split_fields4)

(declare-const o27 us_rep2)

(declare-const rec__tree_model__path_type__aqtresultqtunused2 us_rep2)

(declare-const us_split_fields__contentqtresultqtunused7 us_split_fields2)

(declare-const temp___compl_3228 us_rep1)

(declare-const us_repqtmkqtresultqtunused7 us_rep1)

(declare-const temp___compl_3229 Int)

(declare-const us_split_fields__contentqtresultqtunused8 us_split_fields2)

(declare-const temp___3230 us_rep1)

(declare-const us_repqtmkqtresultqtunused8 us_rep1)

(declare-const temp___3232 (Array Int us_rep3))

(declare-const modelqtresultqtunused3 (Array Int us_rep3))

(declare-const us_split_fields__contentqtresultqtunused9 us_split_fields2)

(declare-const us_repqtmkqtresultqtunused9 us_rep1)

(declare-const us_split_fieldsqtresultqtunused5 us_split_fields2)

(declare-const temp___3234 (Array Int us_rep))

(declare-const rec__binary_trees__forest__cqtresultqtunused1 (Array Int us_rep))

(declare-const int__contentqtresultqtunused13 Int)

(declare-const o28 us_rep)

(declare-const getqtresultqtunused4 us_rep)

(declare-const o29 us_split_fields)

(declare-const us_split_fieldsqtresultqtunused6 us_split_fields)

(declare-const o30 extended_index_type)

(declare-const rec__binary_trees__cell__parentqtresultqtunused extended_index_type)

(declare-const temp___3235 Int)

(declare-const to_repqtresultqtunused1 Int)

(declare-const o31 us_rep3)

(declare-const getqtresultqtunused5 us_rep3)

(declare-const o32 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused7 us_split_fields4)

(declare-const rec__tree_model__path_type__aqtresultqtunused3 us_rep2)

(declare-const us_split_fields__contentqtresultqtunused10 us_split_fields2)

(declare-const temp___compl_3220 us_rep1)

(declare-const us_repqtmkqtresultqtunused10 us_rep1)

(declare-const o33 Int)

(declare-const range_check_qtresultqtunused2 Int)

(declare-const us_split_fields__contentqtresultqtunused11 us_split_fields2)

(declare-const temp___3222 us_rep1)

(declare-const us_repqtmkqtresultqtunused11 us_rep1)

(declare-const temp___3224 (Array Int us_rep3))

(declare-const modelqtresultqtunused4 (Array Int us_rep3))

(declare-const us_split_fields__contentqtresultqtunused12 us_split_fields2)

(declare-const us_repqtmkqtresultqtunused12 us_rep1)

(declare-const us_split_fieldsqtresultqtunused8 us_split_fields2)

(declare-const temp___3226 (Array Int us_rep))

(declare-const rec__binary_trees__forest__cqtresultqtunused2 (Array Int us_rep))

(declare-const int__contentqtresultqtunused14 Int)

(declare-const o34 us_rep)

(declare-const getqtresultqtunused6 us_rep)

(declare-const o35 us_split_fields)

(declare-const us_split_fieldsqtresultqtunused9 us_split_fields)

(declare-const o36 extended_index_type)

(declare-const rec__binary_trees__cell__parentqtresultqtunused1 extended_index_type)

(declare-const temp___3227 Int)

(declare-const to_repqtresultqtunused2 Int)

(declare-const o37 us_rep3)

(declare-const getqtresultqtunused7 us_rep3)

(declare-const o38 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused10 us_split_fields4)

(declare-const rec__tree_model__path_type__aqtresultqtunused4 us_rep2)

(declare-const o39 Bool)

(declare-const int__contentqtresultqtunused15 Int)

(declare-const infix_eqqtresultqtunused3 Bool)

(declare-const o40 Bool)

(declare-const us_split_fields__contentqtresultqtunused13 us_split_fields2)

(declare-const temp___compl_3267 us_rep1)

(declare-const us_repqtmkqtresultqtunused13 us_rep1)

(declare-const o41 Int)

(declare-const range_check_qtresultqtunused3 Int)

(declare-const us_split_fields__contentqtresultqtunused14 us_split_fields2)

(declare-const temp___3269 us_rep1)

(declare-const us_repqtmkqtresultqtunused14 us_rep1)

(declare-const temp___3271 (Array Int us_rep3))

(declare-const modelqtresultqtunused5 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused16 Int)

(declare-const o42 us_rep3)

(declare-const getqtresultqtunused8 us_rep3)

(declare-const o43 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused11 us_split_fields4)

(declare-const rec__tree_model__path_type__kqtresultqtunused Bool)

(declare-const us_split_fields__contentqtresultqtunused15 us_split_fields2)

(declare-const temp___compl_3255 us_rep1)

(declare-const us_repqtmkqtresultqtunused15 us_rep1)

(declare-const temp___compl_3256 Int)

(declare-const us_split_fields__contentqtresultqtunused16 us_split_fields2)

(declare-const temp___3257 us_rep1)

(declare-const us_repqtmkqtresultqtunused16 us_rep1)

(declare-const temp___3259 (Array Int us_rep3))

(declare-const modelqtresultqtunused6 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused17 Int)

(declare-const o44 us_rep3)

(declare-const getqtresultqtunused9 us_rep3)

(declare-const o45 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused12 us_split_fields4)

(declare-const o46 Bool)

(declare-const rec__tree_model__path_type__kqtresultqtunused1 Bool)

(declare-const int__contentqtresultqtunused18 Int)

(declare-const us_split_fields__contentqtresultqtunused17 us_split_fields2)

(declare-const temp___compl_3261 us_rep1)

(declare-const us_repqtmkqtresultqtunused17 us_rep1)

(declare-const temp___compl_3262 Int)

(declare-const us_split_fields__contentqtresultqtunused18 us_split_fields2)

(declare-const temp___3263 us_rep1)

(declare-const us_repqtmkqtresultqtunused18 us_rep1)

(declare-const temp___3265 (Array Int us_rep3))

(declare-const modelqtresultqtunused7 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused19 Int)

(declare-const o47 us_rep3)

(declare-const getqtresultqtunused10 us_rep3)

(declare-const o48 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused13 us_split_fields4)

(declare-const o49 us_rep2)

(declare-const rec__tree_model__path_type__aqtresultqtunused5 us_rep2)

(declare-const o50 Int)

(declare-const lastqtresultqtunused Int)

(declare-const infix_eqqtresultqtunused4 Bool)

(declare-const o51 Bool)

(declare-const bool__contentqtresultqtunused1 Bool)

(declare-const us_split_fields__contentqtresultqtunused19 us_split_fields2)

(declare-const us_repqtmkqtresultqtunused19 us_rep1)

(declare-const us_split_fieldsqtresultqtunused14 us_split_fields2)

(declare-const temp___3201 (Array Int us_rep))

(declare-const rec__binary_trees__forest__cqtresultqtunused3 (Array Int us_rep))

(declare-const int__contentqtresultqtunused20 Int)

(declare-const o52 us_rep)

(declare-const getqtresultqtunused11 us_rep)

(declare-const o53 us_split_fields)

(declare-const us_split_fieldsqtresultqtunused15 us_split_fields)

(declare-const o54 position_type)

(declare-const rec__binary_trees__cell__positionqtresultqtunused1 position_type)

(declare-const o55 Int)

(declare-const to_repqtresultqtunused3 Int)

(declare-const range_check_qtresultqtunused4 Int)

(declare-const us_split_fields__contentqtresultqtunused20 us_split_fields2)

(declare-const temp___compl_3195 us_rep1)

(declare-const us_repqtmkqtresultqtunused20 us_rep1)

(declare-const temp___compl_3196 Int)

(declare-const us_split_fields__contentqtresultqtunused21 us_split_fields2)

(declare-const temp___3197 us_rep1)

(declare-const us_repqtmkqtresultqtunused21 us_rep1)

(declare-const temp___3199 (Array Int us_rep3))

(declare-const modelqtresultqtunused8 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused21 Int)

(declare-const o56 us_rep3)

(declare-const getqtresultqtunused12 us_rep3)

(declare-const o57 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused16 us_split_fields4)

(declare-const o58 us_rep2)

(declare-const rec__tree_model__path_type__aqtresultqtunused6 us_rep2)

(declare-const us_split_fields__contentqtresultqtunused22 us_split_fields2)

(declare-const temp___compl_3189 us_rep1)

(declare-const us_repqtmkqtresultqtunused22 us_rep1)

(declare-const temp___compl_3190 Int)

(declare-const us_split_fields__contentqtresultqtunused23 us_split_fields2)

(declare-const temp___3191 us_rep1)

(declare-const us_repqtmkqtresultqtunused23 us_rep1)

(declare-const temp___3193 (Array Int us_rep3))

(declare-const modelqtresultqtunused9 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused22 Int)

(declare-const o59 us_rep3)

(declare-const getqtresultqtunused13 us_rep3)

(declare-const o60 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused17 us_split_fields4)

(declare-const o61 us_rep2)

(declare-const rec__tree_model__path_type__aqtresultqtunused7 us_rep2)

(declare-const us_split_fields__contentqtresultqtunused24 us_split_fields2)

(declare-const temp___compl_3181 us_rep1)

(declare-const us_repqtmkqtresultqtunused24 us_rep1)

(declare-const temp___compl_3182 Int)

(declare-const us_split_fields__contentqtresultqtunused25 us_split_fields2)

(declare-const temp___3183 us_rep1)

(declare-const us_repqtmkqtresultqtunused25 us_rep1)

(declare-const temp___3185 (Array Int us_rep3))

(declare-const modelqtresultqtunused10 (Array Int us_rep3))

(declare-const us_split_fields__contentqtresultqtunused26 us_split_fields2)

(declare-const us_repqtmkqtresultqtunused26 us_rep1)

(declare-const us_split_fieldsqtresultqtunused18 us_split_fields2)

(declare-const temp___3187 (Array Int us_rep))

(declare-const rec__binary_trees__forest__cqtresultqtunused4 (Array Int us_rep))

(declare-const int__contentqtresultqtunused23 Int)

(declare-const o62 us_rep)

(declare-const getqtresultqtunused14 us_rep)

(declare-const o63 us_split_fields)

(declare-const us_split_fieldsqtresultqtunused19 us_split_fields)

(declare-const o64 extended_index_type)

(declare-const rec__binary_trees__cell__parentqtresultqtunused2 extended_index_type)

(declare-const temp___3188 Int)

(declare-const to_repqtresultqtunused4 Int)

(declare-const o65 us_rep3)

(declare-const getqtresultqtunused15 us_rep3)

(declare-const o66 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused20 us_split_fields4)

(declare-const rec__tree_model__path_type__aqtresultqtunused8 us_rep2)

(declare-const us_split_fields__contentqtresultqtunused27 us_split_fields2)

(declare-const temp___compl_3173 us_rep1)

(declare-const us_repqtmkqtresultqtunused27 us_rep1)

(declare-const temp___compl_3174 Int)

(declare-const us_split_fields__contentqtresultqtunused28 us_split_fields2)

(declare-const temp___3175 us_rep1)

(declare-const us_repqtmkqtresultqtunused28 us_rep1)

(declare-const temp___3177 (Array Int us_rep3))

(declare-const modelqtresultqtunused11 (Array Int us_rep3))

(declare-const us_split_fields__contentqtresultqtunused29 us_split_fields2)

(declare-const us_repqtmkqtresultqtunused29 us_rep1)

(declare-const us_split_fieldsqtresultqtunused21 us_split_fields2)

(declare-const temp___3179 (Array Int us_rep))

(declare-const rec__binary_trees__forest__cqtresultqtunused5 (Array Int us_rep))

(declare-const int__contentqtresultqtunused24 Int)

(declare-const o67 us_rep)

(declare-const getqtresultqtunused16 us_rep)

(declare-const o68 us_split_fields)

(declare-const us_split_fieldsqtresultqtunused22 us_split_fields)

(declare-const o69 extended_index_type)

(declare-const rec__binary_trees__cell__parentqtresultqtunused3 extended_index_type)

(declare-const temp___3180 Int)

(declare-const to_repqtresultqtunused5 Int)

(declare-const o70 us_rep3)

(declare-const getqtresultqtunused17 us_rep3)

(declare-const o71 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused23 us_split_fields4)

(declare-const rec__tree_model__path_type__aqtresultqtunused9 us_rep2)

(declare-const o72 Bool)

(declare-const us_split_fields__contentqtresultqtunused30 us_split_fields2)

(declare-const temp___compl_3214 us_rep1)

(declare-const us_repqtmkqtresultqtunused30 us_rep1)

(declare-const temp___compl_3215 Int)

(declare-const us_split_fields__contentqtresultqtunused31 us_split_fields2)

(declare-const temp___3216 us_rep1)

(declare-const us_repqtmkqtresultqtunused31 us_rep1)

(declare-const temp___3218 (Array Int us_rep3))

(declare-const modelqtresultqtunused12 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused25 Int)

(declare-const o73 us_rep3)

(declare-const getqtresultqtunused18 us_rep3)

(declare-const o74 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused24 us_split_fields4)

(declare-const rec__tree_model__path_type__kqtresultqtunused2 Bool)

(declare-const us_split_fields__contentqtresultqtunused32 us_split_fields2)

(declare-const temp___compl_3202 us_rep1)

(declare-const us_repqtmkqtresultqtunused32 us_rep1)

(declare-const temp___compl_3203 Int)

(declare-const us_split_fields__contentqtresultqtunused33 us_split_fields2)

(declare-const temp___3204 us_rep1)

(declare-const us_repqtmkqtresultqtunused33 us_rep1)

(declare-const temp___3206 (Array Int us_rep3))

(declare-const modelqtresultqtunused13 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused26 Int)

(declare-const o75 us_rep3)

(declare-const getqtresultqtunused19 us_rep3)

(declare-const o76 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused25 us_split_fields4)

(declare-const o77 Bool)

(declare-const rec__tree_model__path_type__kqtresultqtunused3 Bool)

(declare-const int__contentqtresultqtunused27 Int)

(declare-const us_split_fields__contentqtresultqtunused34 us_split_fields2)

(declare-const temp___compl_3208 us_rep1)

(declare-const us_repqtmkqtresultqtunused34 us_rep1)

(declare-const temp___compl_3209 Int)

(declare-const us_split_fields__contentqtresultqtunused35 us_split_fields2)

(declare-const temp___3210 us_rep1)

(declare-const us_repqtmkqtresultqtunused35 us_rep1)

(declare-const temp___3212 (Array Int us_rep3))

(declare-const modelqtresultqtunused14 (Array Int us_rep3))

(declare-const int__contentqtresultqtunused28 Int)

(declare-const o78 us_rep3)

(declare-const getqtresultqtunused20 us_rep3)

(declare-const o79 us_split_fields4)

(declare-const us_split_fieldsqtresultqtunused26 us_split_fields4)

(declare-const o80 us_rep2)

(declare-const rec__tree_model__path_type__aqtresultqtunused10 us_rep2)

(declare-const o81 Int)

(declare-const lastqtresultqtunused1 Int)

(declare-const infix_eqqtresultqtunused5 Bool)

(declare-const usf2 Int)

(declare-const anyqtresultqtunused5 Int)

(declare-const iqtunused2 Int)

(declare-const o82 Bool)

(declare-const infix_lseqqtresultqtunused10 Bool)

(declare-const o83 Bool)

(declare-const infix_lseqqtresultqtunused11 Bool)

(declare-const andbqtresultqtunused5 Bool)

;; "H"
(assert
  (ite (= o7 true)
    (and
      (and
        (= anyqtresultqtunused5 usf2)
        (and
          (= iqtunused2 usf2)
          (and
            (= o82 (ite (<= usf2 100) true false))
            (and
              (= infix_lseqqtresultqtunused10 o82)
              (and
                (= o83 (ite (<= 1 usf2) true false))
                (and
                  (= infix_lseqqtresultqtunused11 o83)
                  (= andbqtresultqtunused5 (and o83 o82))))))))
      (and
        (= usfqtunused1 (ite (forall ((i Int))
                               (=>
                                 (and (<= 1 i) (<= i 100))
                                 (=>
                                   (and
                                     (= (rec__tree_model__path_type__k
                                          (us_split_fields5
                                            (select (model1
                                                      (us_repqtmk1
                                                        f__split_fields)
                                                      root) i))) true)
                                     (<= (last
                                           (rec__tree_model__path_type__a
                                             (us_split_fields5
                                               (select (model1
                                                         (us_repqtmk1
                                                           f__split_fields)
                                                         root) i)))) (- 
                                     n2 1)))
                                   (=>
                                     (= (rec__tree_model__path_type__k
                                          (us_split_fields5
                                            (select (model1
                                                      (us_repqtmk1
                                                        f_old__split_fields)
                                                      root) i))) true)
                                     (= (oeq
                                          (rec__tree_model__path_type__a
                                            (us_split_fields5
                                              (select (model1
                                                        (us_repqtmk1
                                                          f__split_fields)
                                                        root) i)))
                                          (rec__tree_model__path_type__a
                                            (us_split_fields5
                                              (select (model1
                                                        (us_repqtmk1
                                                          f_old__split_fields)
                                                        root) i)))) true)))))
                          true
                          false))
        (and
          (= anyqtresultqtunused2 (ite (forall ((i Int))
                                         (=>
                                           (and (<= 1 i) (<= i 100))
                                           (=>
                                             (and
                                               (= (rec__tree_model__path_type__k
                                                    (us_split_fields5
                                                      (select (model1
                                                                (us_repqtmk1
                                                                  f__split_fields)
                                                                root) i))) true)
                                               (<= (last
                                                     (rec__tree_model__path_type__a
                                                       (us_split_fields5
                                                         (select (model1
                                                                   (us_repqtmk1
                                                                    f__split_fields)
                                                                   root) i)))) (- 
                                               n2 1)))
                                             (=>
                                               (= (rec__tree_model__path_type__k
                                                    (us_split_fields5
                                                      (select (model1
                                                                (us_repqtmk1
                                                                  f_old__split_fields)
                                                                root) i))) true)
                                               (= (oeq
                                                    (rec__tree_model__path_type__a
                                                      (us_split_fields5
                                                        (select (model1
                                                                  (us_repqtmk1
                                                                    f__split_fields)
                                                                  root) i)))
                                                    (rec__tree_model__path_type__a
                                                      (us_split_fields5
                                                        (select (model1
                                                                  (us_repqtmk1
                                                                    f_old__split_fields)
                                                                  root) i)))) true)))))
                                    true
                                    false))
          (and
            (= nqtunused3 (int__refqtmk n3))
            (and
              (and
                (forall ((i Int))
                  (=>
                    (and (<= 1 i) (<= i 100))
                    (=>
                      (and
                        (= (rec__tree_model__path_type__k
                             (us_split_fields5
                               (select (model1
                                         (us_repqtmk1 f__split_fields)
                                         root) i))) true)
                        (<= (last
                              (rec__tree_model__path_type__a
                                (us_split_fields5
                                  (select (model1
                                            (us_repqtmk1 f__split_fields)
                                            root) i)))) (- n3 1)))
                      (=>
                        (= (rec__tree_model__path_type__k
                             (us_split_fields5
                               (select (model1
                                         (us_repqtmk1 f_old__split_fields)
                                         root) i))) true)
                        (= (oeq
                             (rec__tree_model__path_type__a
                               (us_split_fields5
                                 (select (model1
                                           (us_repqtmk1 f__split_fields)
                                           root) i)))
                             (rec__tree_model__path_type__a
                               (us_split_fields5
                                 (select (model1
                                           (us_repqtmk1 f_old__split_fields)
                                           root) i)))) true)))))
                (forall ((i Int))
                  (=>
                    (and (<= 1 i) (<= i 100))
                    (=>
                      (and
                        (= (rec__tree_model__path_type__k
                             (us_split_fields5
                               (select (model1
                                         (us_repqtmk1 f__split_fields)
                                         root) i))) true)
                        (<= (last
                              (rec__tree_model__path_type__a
                                (us_split_fields5
                                  (select (model1
                                            (us_repqtmk1 f__split_fields)
                                            root) i)))) (- n3 1)))
                      (=>
                        (= (rec__tree_model__path_type__k
                             (us_split_fields5
                               (select (model1
                                         (us_repqtmk1 f_old__split_fields)
                                         v) i))) true)
                        (= (is_concat
                             (rec__tree_model__path_type__a
                               (us_split_fields5
                                 (select (model1
                                           (us_repqtmk1 f__split_fields)
                                           root) v)))
                             (rec__tree_model__path_type__a
                               (us_split_fields5
                                 (select (model1
                                           (us_repqtmk1 f_old__split_fields)
                                           v) i)))
                             (rec__tree_model__path_type__a
                               (us_split_fields5
                                 (select (model1
                                           (us_repqtmk1 f__split_fields)
                                           root) i)))) true))))))
              (and
                (and (<= 1 n3) (<= n3 100))
                (and
                  (and
                    (= kiqtunused (int__refqtmk ki))
                    (and
                      (= ki 1)
                      (and
                        (= int__contentqtresultqtunused6 ki)
                        (and
                          (= o8 (ite (<= ki 100) true false))
                          (and
                            (= infix_lseqqtresultqtunused6 o8)
                            (and
                              (= int__contentqtresultqtunused7 ki)
                              (and
                                (= o9 (ite (<= 1 ki) true false))
                                (and
                                  (= infix_lseqqtresultqtunused7 o9)
                                  (and
                                    (= o10 (and o9 o8))
                                    (and
                                      (= andbqtresultqtunused3 o10)
                                      (ite (= o10 true)
                                        (and
                                          (= spark__branchqtunused1 (bool__refqtmk
                                                                    spark__branch3))
                                          (and
                                            (and
                                              (and
                                                (and
                                                  (= us_split_fields__contentqtresultqtunused32 
                                                  f__split_fields)
                                                  (and
                                                    (= temp___compl_3202 
                                                    (us_repqtmk1
                                                      f__split_fields))
                                                    (and
                                                      (= us_repqtmkqtresultqtunused32 
                                                      temp___compl_3202)
                                                      (and
                                                        (= temp___compl_3203 
                                                        root)
                                                        (and
                                                          (= us_split_fields__contentqtresultqtunused33 
                                                          f__split_fields)
                                                          (and
                                                            (= temp___3204 
                                                            (us_repqtmk1
                                                              f__split_fields))
                                                            (and
                                                              (= us_repqtmkqtresultqtunused33 
                                                              temp___3204)
                                                              (and
                                                                (type_invariant
                                                                  temp___3204)
                                                                (and
                                                                  (= 
                                                                  temp___3206 
                                                                  (model1
                                                                    temp___compl_3202
                                                                    temp___compl_3203))
                                                                  (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3206
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 
                                                                    temp___compl_3203))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 
                                                                    temp___compl_3203)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3203))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3202
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 
                                                                    (parent
                                                                    temp___compl_3202
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3203)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 
                                                                    (parent
                                                                    temp___compl_3202
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3202
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3206 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused13 
                                                                    temp___3206)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused26 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o75 (select 
                                                                    temp___3206 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused19 
                                                                    o75)
                                                                    (and
                                                                    (= 
                                                                    o76 
                                                                    (us_split_fields5
                                                                    o75))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused25 
                                                                    o76)
                                                                    (and
                                                                    (= 
                                                                    o77 
                                                                    (rec__tree_model__path_type__k
                                                                    o76))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__kqtresultqtunused3 
                                                                    o77)
                                                                    (ite (= 
                                                                    o77 true)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused27 
                                                                    n3)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused34 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3208 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused34 
                                                                    temp___compl_3208)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3209 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused35 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3210 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused35 
                                                                    temp___3210)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3210)
                                                                    (and
                                                                    (= 
                                                                    temp___3212 
                                                                    (model1
                                                                    temp___compl_3208
                                                                    temp___compl_3209))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3212
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 
                                                                    temp___compl_3209))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 
                                                                    temp___compl_3209)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3209))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3208
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 
                                                                    (parent
                                                                    temp___compl_3208
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3209)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 
                                                                    (parent
                                                                    temp___compl_3208
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3208
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3212 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused14 
                                                                    temp___3212)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused28 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o78 (select 
                                                                    temp___3212 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused20 
                                                                    o78)
                                                                    (and
                                                                    (= 
                                                                    o79 
                                                                    (us_split_fields5
                                                                    o78))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused26 
                                                                    o79)
                                                                    (and
                                                                    (= 
                                                                    o80 
                                                                    (rec__tree_model__path_type__a
                                                                    o79))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused10 
                                                                    o80)
                                                                    (and
                                                                    (= 
                                                                    o81 
                                                                    (last
                                                                    o80))
                                                                    (and
                                                                    (in_range2
                                                                    o81)
                                                                    (and
                                                                    (= 
                                                                    lastqtresultqtunused1 
                                                                    o81)
                                                                    (and
                                                                    (= 
                                                                    o72 
                                                                    (ite (= 
                                                                    o81 
                                                                    n3)
                                                                    true
                                                                    false))
                                                                    (= 
                                                                    infix_eqqtresultqtunused5 
                                                                    o72))))))))))))))))))))))))
                                                                    (= 
                                                                    o72 false))))))))))))))))))))
                                                (ite (= o72 true)
                                                  (and
                                                    (= us_split_fields__contentqtresultqtunused30 
                                                    f_old__split_fields)
                                                    (and
                                                      (= temp___compl_3214 
                                                      (us_repqtmk1
                                                        f_old__split_fields))
                                                      (and
                                                        (= us_repqtmkqtresultqtunused30 
                                                        temp___compl_3214)
                                                        (and
                                                          (= temp___compl_3215 
                                                          root)
                                                          (and
                                                            (= us_split_fields__contentqtresultqtunused31 
                                                            f_old__split_fields)
                                                            (and
                                                              (= temp___3216 
                                                              (us_repqtmk1
                                                                f_old__split_fields))
                                                              (and
                                                                (= us_repqtmkqtresultqtunused31 
                                                                temp___3216)
                                                                (and
                                                                  (type_invariant
                                                                    temp___3216)
                                                                  (and
                                                                    (= 
                                                                    temp___3218 
                                                                    (model1
                                                                    temp___compl_3214
                                                                    temp___compl_3215))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3218
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 
                                                                    temp___compl_3215))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 
                                                                    temp___compl_3215)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3215))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3214
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 
                                                                    (parent
                                                                    temp___compl_3214
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3215)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 
                                                                    (parent
                                                                    temp___compl_3214
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3214
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3218 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused12 
                                                                    temp___3218)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused25 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o73 (select 
                                                                    temp___3218 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused18 
                                                                    o73)
                                                                    (and
                                                                    (= 
                                                                    o74 
                                                                    (us_split_fields5
                                                                    o73))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused24 
                                                                    o74)
                                                                    (and
                                                                    (= 
                                                                    o51 
                                                                    (rec__tree_model__path_type__k
                                                                    o74))
                                                                    (= 
                                                                    rec__tree_model__path_type__kqtresultqtunused2 
                                                                    o51))))))))))))))))))
                                                  (= o51 false)))
                                              (and
                                                (= spark__branch3 o51)
                                                (and
                                                  (= bool__contentqtresultqtunused1 
                                                  spark__branch3)
                                                  (=>
                                                    (= spark__branch3 true)
                                                    (and
                                                      (= us_split_fields__contentqtresultqtunused19 
                                                      f__split_fields)
                                                      (and
                                                        (= us_repqtmkqtresultqtunused19 
                                                        (us_repqtmk1
                                                          f__split_fields))
                                                        (and
                                                          (= us_split_fieldsqtresultqtunused14 
                                                          f__split_fields)
                                                          (and
                                                            (= temp___3201 
                                                            (rec__binary_trees__forest__c
                                                              f__split_fields))
                                                            (and
                                                              (= rec__binary_trees__forest__cqtresultqtunused3 
                                                              temp___3201)
                                                              (and
                                                                (= int__contentqtresultqtunused20 
                                                                ki)
                                                                (and
                                                                  (= 
                                                                  o52 (select 
                                                                  temp___3201 
                                                                  ki))
                                                                  (and
                                                                    (= 
                                                                    getqtresultqtunused11 
                                                                    o52)
                                                                    (and
                                                                    (= 
                                                                    o53 
                                                                    (us_split_fields1
                                                                    o52))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused15 
                                                                    o53)
                                                                    (and
                                                                    (= 
                                                                    o54 
                                                                    (rec__binary_trees__cell__position
                                                                    o53))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__cell__positionqtresultqtunused1 
                                                                    o54)
                                                                    (and
                                                                    (= 
                                                                    (to_rep1
                                                                    o54) 
                                                                    o55)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused3 
                                                                    o55)
                                                                    (and
                                                                    (in_range4
                                                                    o55)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused4 
                                                                    o55)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused20 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3195 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused20 
                                                                    temp___compl_3195)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3196 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused21 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3197 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused21 
                                                                    temp___3197)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3197)
                                                                    (and
                                                                    (= 
                                                                    temp___3199 
                                                                    (model1
                                                                    temp___compl_3195
                                                                    temp___compl_3196))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3199
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 
                                                                    temp___compl_3196))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 
                                                                    temp___compl_3196)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3196))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3195
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 
                                                                    (parent
                                                                    temp___compl_3195
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3196)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 
                                                                    (parent
                                                                    temp___compl_3195
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3195
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3199 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused8 
                                                                    temp___3199)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused21 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o56 (select 
                                                                    temp___3199 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused12 
                                                                    o56)
                                                                    (and
                                                                    (= 
                                                                    o57 
                                                                    (us_split_fields5
                                                                    o56))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused16 
                                                                    o57)
                                                                    (and
                                                                    (= 
                                                                    o58 
                                                                    (rec__tree_model__path_type__a
                                                                    o57))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused6 
                                                                    o58)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused22 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3189 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused22 
                                                                    temp___compl_3189)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3190 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused23 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3191 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused23 
                                                                    temp___3191)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3191)
                                                                    (and
                                                                    (= 
                                                                    temp___3193 
                                                                    (model1
                                                                    temp___compl_3189
                                                                    temp___compl_3190))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3193
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 
                                                                    temp___compl_3190))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 
                                                                    temp___compl_3190)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3190))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3189
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 
                                                                    (parent
                                                                    temp___compl_3189
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3190)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 
                                                                    (parent
                                                                    temp___compl_3189
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3189
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3193 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused9 
                                                                    temp___3193)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused22 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o59 (select 
                                                                    temp___3193 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused13 
                                                                    o59)
                                                                    (and
                                                                    (= 
                                                                    o60 
                                                                    (us_split_fields5
                                                                    o59))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused17 
                                                                    o60)
                                                                    (and
                                                                    (= 
                                                                    o61 
                                                                    (rec__tree_model__path_type__a
                                                                    o60))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused7 
                                                                    o61)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused24 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3181 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused24 
                                                                    temp___compl_3181)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3182 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused25 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3183 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused25 
                                                                    temp___3183)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3183)
                                                                    (and
                                                                    (= 
                                                                    temp___3185 
                                                                    (model1
                                                                    temp___compl_3181
                                                                    temp___compl_3182))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3185
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 
                                                                    temp___compl_3182))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 
                                                                    temp___compl_3182)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3182))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3181
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 
                                                                    (parent
                                                                    temp___compl_3181
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3182)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 
                                                                    (parent
                                                                    temp___compl_3181
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3181
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3185 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused10 
                                                                    temp___3185)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused26 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused26 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused18 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3187 
                                                                    (rec__binary_trees__forest__c
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__forest__cqtresultqtunused4 
                                                                    temp___3187)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused23 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o62 (select 
                                                                    temp___3187 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused14 
                                                                    o62)
                                                                    (and
                                                                    (= 
                                                                    o63 
                                                                    (us_split_fields1
                                                                    o62))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused19 
                                                                    o63)
                                                                    (and
                                                                    (= 
                                                                    o64 
                                                                    (rec__binary_trees__cell__parent
                                                                    o63))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__cell__parentqtresultqtunused2 
                                                                    o64)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o64) 
                                                                    temp___3188)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused4 
                                                                    temp___3188)
                                                                    (and
                                                                    (and
                                                                    (<= 1 
                                                                    temp___3188)
                                                                    (<= 
                                                                    temp___3188 100))
                                                                    (and
                                                                    (= 
                                                                    o65 (select 
                                                                    temp___3185 
                                                                    temp___3188))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused15 
                                                                    o65)
                                                                    (and
                                                                    (= 
                                                                    o66 
                                                                    (us_split_fields5
                                                                    o65))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused20 
                                                                    o66)
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused8 
                                                                    (rec__tree_model__path_type__a
                                                                    o66))
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused27 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3173 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused27 
                                                                    temp___compl_3173)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3174 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused28 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3175 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused28 
                                                                    temp___3175)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3175)
                                                                    (and
                                                                    (= 
                                                                    temp___3177 
                                                                    (model1
                                                                    temp___compl_3173
                                                                    temp___compl_3174))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3177
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 
                                                                    temp___compl_3174))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 
                                                                    temp___compl_3174)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3174))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3173
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 
                                                                    (parent
                                                                    temp___compl_3173
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3174)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 
                                                                    (parent
                                                                    temp___compl_3173
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3173
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3177 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused11 
                                                                    temp___3177)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused29 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused29 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused21 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3179 
                                                                    (rec__binary_trees__forest__c
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__forest__cqtresultqtunused5 
                                                                    temp___3179)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused24 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o67 (select 
                                                                    temp___3179 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused16 
                                                                    o67)
                                                                    (and
                                                                    (= 
                                                                    o68 
                                                                    (us_split_fields1
                                                                    o67))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused22 
                                                                    o68)
                                                                    (and
                                                                    (= 
                                                                    o69 
                                                                    (rec__binary_trees__cell__parent
                                                                    o68))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__cell__parentqtresultqtunused3 
                                                                    o69)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o69) 
                                                                    temp___3180)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused5 
                                                                    temp___3180)
                                                                    (and
                                                                    (and
                                                                    (<= 1 
                                                                    temp___3180)
                                                                    (<= 
                                                                    temp___3180 100))
                                                                    (and
                                                                    (= 
                                                                    o70 (select 
                                                                    temp___3177 
                                                                    temp___3180))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused17 
                                                                    o70)
                                                                    (and
                                                                    (= 
                                                                    o71 
                                                                    (us_split_fields5
                                                                    o70))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused23 
                                                                    o71)
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused9 
                                                                    (rec__tree_model__path_type__a
                                                                    o71))
                                                                    (= 
                                                                    (oeq
                                                                    o61
                                                                    o58) true)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
                                            (and
                                              (= spark__branchqtunused2 
                                              (bool__refqtmk
                                                spark__branch4))
                                              (and
                                                (and
                                                  (and
                                                    (and
                                                      (and
                                                        (= us_split_fields__contentqtresultqtunused15 
                                                        f__split_fields)
                                                        (and
                                                          (= temp___compl_3255 
                                                          (us_repqtmk1
                                                            f__split_fields))
                                                          (and
                                                            (= us_repqtmkqtresultqtunused15 
                                                            temp___compl_3255)
                                                            (and
                                                              (= temp___compl_3256 
                                                              root)
                                                              (and
                                                                (= us_split_fields__contentqtresultqtunused16 
                                                                f__split_fields)
                                                                (and
                                                                  (= 
                                                                  temp___3257 
                                                                  (us_repqtmk1
                                                                    f__split_fields))
                                                                  (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused16 
                                                                    temp___3257)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3257)
                                                                    (and
                                                                    (= 
                                                                    temp___3259 
                                                                    (model1
                                                                    temp___compl_3255
                                                                    temp___compl_3256))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3259
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 
                                                                    temp___compl_3256))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 
                                                                    temp___compl_3256)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3256))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3255
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 
                                                                    (parent
                                                                    temp___compl_3255
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3256)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 
                                                                    (parent
                                                                    temp___compl_3255
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3255
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3259 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused6 
                                                                    temp___3259)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused17 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o44 (select 
                                                                    temp___3259 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused9 
                                                                    o44)
                                                                    (and
                                                                    (= 
                                                                    o45 
                                                                    (us_split_fields5
                                                                    o44))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused12 
                                                                    o45)
                                                                    (and
                                                                    (= 
                                                                    o46 
                                                                    (rec__tree_model__path_type__k
                                                                    o45))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__kqtresultqtunused1 
                                                                    o46)
                                                                    (ite (= 
                                                                    o46 true)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused18 
                                                                    n3)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused17 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3261 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused17 
                                                                    temp___compl_3261)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3262 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused18 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3263 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused18 
                                                                    temp___3263)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3263)
                                                                    (and
                                                                    (= 
                                                                    temp___3265 
                                                                    (model1
                                                                    temp___compl_3261
                                                                    temp___compl_3262))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3265
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 
                                                                    temp___compl_3262))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 
                                                                    temp___compl_3262)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3262))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3261
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 
                                                                    (parent
                                                                    temp___compl_3261
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3262)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 
                                                                    (parent
                                                                    temp___compl_3261
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3261
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3265 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused7 
                                                                    temp___3265)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused19 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o47 (select 
                                                                    temp___3265 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused10 
                                                                    o47)
                                                                    (and
                                                                    (= 
                                                                    o48 
                                                                    (us_split_fields5
                                                                    o47))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused13 
                                                                    o48)
                                                                    (and
                                                                    (= 
                                                                    o49 
                                                                    (rec__tree_model__path_type__a
                                                                    o48))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused5 
                                                                    o49)
                                                                    (and
                                                                    (= 
                                                                    o50 
                                                                    (last
                                                                    o49))
                                                                    (and
                                                                    (in_range2
                                                                    o50)
                                                                    (and
                                                                    (= 
                                                                    lastqtresultqtunused 
                                                                    o50)
                                                                    (and
                                                                    (= 
                                                                    o40 
                                                                    (ite (= 
                                                                    o50 
                                                                    n3)
                                                                    true
                                                                    false))
                                                                    (= 
                                                                    infix_eqqtresultqtunused4 
                                                                    o40))))))))))))))))))))))))
                                                                    (= 
                                                                    o40 false))))))))))))))))))))
                                                      (ite (= o40 true)
                                                        (and
                                                          (= us_split_fields__contentqtresultqtunused13 
                                                          f_old__split_fields)
                                                          (and
                                                            (= temp___compl_3267 
                                                            (us_repqtmk1
                                                              f_old__split_fields))
                                                            (and
                                                              (= us_repqtmkqtresultqtunused13 
                                                              temp___compl_3267)
                                                              (and
                                                                (= o41 
                                                                v)
                                                                (and
                                                                  (in_range3
                                                                    o41)
                                                                  (and
                                                                    (= 
                                                                    range_check_qtresultqtunused3 
                                                                    o41)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused14 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3269 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused14 
                                                                    temp___3269)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3269)
                                                                    (and
                                                                    (= 
                                                                    temp___3271 
                                                                    (model1
                                                                    temp___compl_3267
                                                                    o41))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3271
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 
                                                                    o41))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 
                                                                    o41)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    o41))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3267
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 
                                                                    (parent
                                                                    temp___compl_3267
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    o41)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 
                                                                    (parent
                                                                    temp___compl_3267
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3267
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3271 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused5 
                                                                    temp___3271)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused16 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o42 (select 
                                                                    temp___3271 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused8 
                                                                    o42)
                                                                    (and
                                                                    (= 
                                                                    o43 
                                                                    (us_split_fields5
                                                                    o42))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused11 
                                                                    o43)
                                                                    (and
                                                                    (= 
                                                                    o39 
                                                                    (rec__tree_model__path_type__k
                                                                    o43))
                                                                    (= 
                                                                    rec__tree_model__path_type__kqtresultqtunused 
                                                                    o39))))))))))))))))))))
                                                        (= o39 false)))
                                                    (ite (= o39 true)
                                                      (and
                                                        (= int__contentqtresultqtunused15 
                                                        ki)
                                                        (and
                                                          (= infix_eqqtresultqtunused3 
                                                          (ite (= ki 
                                                            v)
                                                            true
                                                            false))
                                                          (= o13 (ite (= 
                                                                   ki 
                                                                   v)
                                                                   false
                                                                   true))))
                                                      (= o13 false)))
                                                  (and
                                                    (= spark__branch4 
                                                    o13)
                                                    (and
                                                      (= bool__contentqtresultqtunused 
                                                      spark__branch4)
                                                      (=>
                                                        (= spark__branch4 true)
                                                        (and
                                                          (= us_split_fields__contentqtresultqtunused 
                                                          f__split_fields)
                                                          (and
                                                            (= us_repqtmkqtresultqtunused 
                                                            (us_repqtmk1
                                                              f__split_fields))
                                                            (and
                                                              (= us_split_fieldsqtresultqtunused 
                                                              f__split_fields)
                                                              (and
                                                                (= temp___3254 
                                                                (rec__binary_trees__forest__c
                                                                  f__split_fields))
                                                                (and
                                                                  (= 
                                                                  rec__binary_trees__forest__cqtresultqtunused 
                                                                  temp___3254)
                                                                  (and
                                                                    (= 
                                                                    int__contentqtresultqtunused10 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o14 (select 
                                                                    temp___3254 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused 
                                                                    o14)
                                                                    (and
                                                                    (= 
                                                                    o15 
                                                                    (us_split_fields1
                                                                    o14))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused1 
                                                                    o15)
                                                                    (and
                                                                    (= 
                                                                    o16 
                                                                    (rec__binary_trees__cell__position
                                                                    o15))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__cell__positionqtresultqtunused 
                                                                    o16)
                                                                    (and
                                                                    (= 
                                                                    (to_rep1
                                                                    o16) 
                                                                    o17)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused 
                                                                    o17)
                                                                    (and
                                                                    (in_range4
                                                                    o17)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused 
                                                                    o17)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused1 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3248 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused1 
                                                                    temp___compl_3248)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3249 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused2 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3250 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused2 
                                                                    temp___3250)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3250)
                                                                    (and
                                                                    (= 
                                                                    temp___3252 
                                                                    (model1
                                                                    temp___compl_3248
                                                                    temp___compl_3249))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3252
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 
                                                                    temp___compl_3249))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 
                                                                    temp___compl_3249)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3249))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3248
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 
                                                                    (parent
                                                                    temp___compl_3248
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3249)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 
                                                                    (parent
                                                                    temp___compl_3248
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3248
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3252 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused 
                                                                    temp___3252)
                                                                    (and
                                                                    (and
                                                                    (<= 1 
                                                                    v)
                                                                    (<= 
                                                                    v 100))
                                                                    (and
                                                                    (= 
                                                                    o18 (select 
                                                                    temp___3252 
                                                                    v))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused1 
                                                                    o18)
                                                                    (and
                                                                    (= 
                                                                    o19 
                                                                    (us_split_fields5
                                                                    o18))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused2 
                                                                    o19)
                                                                    (and
                                                                    (= 
                                                                    o20 
                                                                    (rec__tree_model__path_type__a
                                                                    o19))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused 
                                                                    o20)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused3 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3242 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused3 
                                                                    temp___compl_3242)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3243 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused4 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3244 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused4 
                                                                    temp___3244)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3244)
                                                                    (and
                                                                    (= 
                                                                    temp___3246 
                                                                    (model1
                                                                    temp___compl_3242
                                                                    temp___compl_3243))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3246
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 
                                                                    temp___compl_3243))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 
                                                                    temp___compl_3243)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3243))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3242
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 
                                                                    (parent
                                                                    temp___compl_3242
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3243)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 
                                                                    (parent
                                                                    temp___compl_3242
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3242
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3246 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused1 
                                                                    temp___3246)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused11 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o21 (select 
                                                                    temp___3246 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused2 
                                                                    o21)
                                                                    (and
                                                                    (= 
                                                                    o22 
                                                                    (us_split_fields5
                                                                    o21))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused3 
                                                                    o22)
                                                                    (and
                                                                    (= 
                                                                    o23 
                                                                    (rec__tree_model__path_type__a
                                                                    o22))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused1 
                                                                    o23)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused5 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3236 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused5 
                                                                    temp___compl_3236)
                                                                    (and
                                                                    (= 
                                                                    o24 
                                                                    v)
                                                                    (and
                                                                    (in_range3
                                                                    o24)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused1 
                                                                    o24)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused6 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3238 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused6 
                                                                    temp___3238)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3238)
                                                                    (and
                                                                    (= 
                                                                    temp___3240 
                                                                    (model1
                                                                    temp___compl_3236
                                                                    o24))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3240
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 
                                                                    o24))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 
                                                                    o24)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    o24))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3236
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 
                                                                    (parent
                                                                    temp___compl_3236
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    o24)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 
                                                                    (parent
                                                                    temp___compl_3236
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3236
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3240 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused2 
                                                                    temp___3240)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused12 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o25 (select 
                                                                    temp___3240 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused3 
                                                                    o25)
                                                                    (and
                                                                    (= 
                                                                    o26 
                                                                    (us_split_fields5
                                                                    o25))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused4 
                                                                    o26)
                                                                    (and
                                                                    (= 
                                                                    o27 
                                                                    (rec__tree_model__path_type__a
                                                                    o26))
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused2 
                                                                    o27)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused7 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3228 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused7 
                                                                    temp___compl_3228)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3229 
                                                                    root)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused8 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3230 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused8 
                                                                    temp___3230)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3230)
                                                                    (and
                                                                    (= 
                                                                    temp___3232 
                                                                    (model1
                                                                    temp___compl_3228
                                                                    temp___compl_3229))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3232
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 
                                                                    temp___compl_3229))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 
                                                                    temp___compl_3229)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3229))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3228
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 
                                                                    (parent
                                                                    temp___compl_3228
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    temp___compl_3229)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 
                                                                    (parent
                                                                    temp___compl_3228
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3228
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3232 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused3 
                                                                    temp___3232)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused9 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused9 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused5 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3234 
                                                                    (rec__binary_trees__forest__c
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__forest__cqtresultqtunused1 
                                                                    temp___3234)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused13 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o28 (select 
                                                                    temp___3234 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused4 
                                                                    o28)
                                                                    (and
                                                                    (= 
                                                                    o29 
                                                                    (us_split_fields1
                                                                    o28))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused6 
                                                                    o29)
                                                                    (and
                                                                    (= 
                                                                    o30 
                                                                    (rec__binary_trees__cell__parent
                                                                    o29))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__cell__parentqtresultqtunused 
                                                                    o30)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o30) 
                                                                    temp___3235)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused1 
                                                                    temp___3235)
                                                                    (and
                                                                    (and
                                                                    (<= 1 
                                                                    temp___3235)
                                                                    (<= 
                                                                    temp___3235 100))
                                                                    (and
                                                                    (= 
                                                                    o31 (select 
                                                                    temp___3232 
                                                                    temp___3235))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused5 
                                                                    o31)
                                                                    (and
                                                                    (= 
                                                                    o32 
                                                                    (us_split_fields5
                                                                    o31))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused7 
                                                                    o32)
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused3 
                                                                    (rec__tree_model__path_type__a
                                                                    o32))
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused10 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___compl_3220 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused10 
                                                                    temp___compl_3220)
                                                                    (and
                                                                    (= 
                                                                    o33 
                                                                    v)
                                                                    (and
                                                                    (in_range3
                                                                    o33)
                                                                    (and
                                                                    (= 
                                                                    range_check_qtresultqtunused2 
                                                                    o33)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused11 
                                                                    f_old__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3222 
                                                                    (us_repqtmk1
                                                                    f_old__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused11 
                                                                    temp___3222)
                                                                    (and
                                                                    (type_invariant
                                                                    temp___3222)
                                                                    (and
                                                                    (= 
                                                                    temp___3224 
                                                                    (model1
                                                                    temp___compl_3220
                                                                    o33))
                                                                    (and
                                                                    (and
                                                                    (dynamic_invariant
                                                                    temp___3224
                                                                    true
                                                                    false
                                                                    true
                                                                    true)
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 
                                                                    o33))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 
                                                                    o33)))) 0))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (not
                                                                    (= i 
                                                                    o33))
                                                                    (ite 
                                                                    (and
                                                                    (not
                                                                    (= 
                                                                    (parent
                                                                    temp___compl_3220
                                                                    i) 0))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 
                                                                    (parent
                                                                    temp___compl_3220
                                                                    i)))) true))
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 i))) true)
                                                                    (not
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 i))) true)))))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (ite 
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 i))) true)
                                                                    (not
                                                                    (= i 
                                                                    o33)))
                                                                    (= 
                                                                    (is_add
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 
                                                                    (parent
                                                                    temp___compl_3220
                                                                    i))))
                                                                    (position
                                                                    temp___compl_3220
                                                                    i)
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 i)))) true)
                                                                    (= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 i)))) 0)))))
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 100))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 i))) true)
                                                                    (forall 
                                                                    ((j Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 j)
                                                                    (<= j 100))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 j))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    temp___3224 j)))) true))
                                                                    (= j i)))))))))
                                                                    (and
                                                                    (= 
                                                                    modelqtresultqtunused4 
                                                                    temp___3224)
                                                                    (and
                                                                    (= 
                                                                    us_split_fields__contentqtresultqtunused12 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    us_repqtmkqtresultqtunused12 
                                                                    (us_repqtmk1
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused8 
                                                                    f__split_fields)
                                                                    (and
                                                                    (= 
                                                                    temp___3226 
                                                                    (rec__binary_trees__forest__c
                                                                    f__split_fields))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__forest__cqtresultqtunused2 
                                                                    temp___3226)
                                                                    (and
                                                                    (= 
                                                                    int__contentqtresultqtunused14 
                                                                    ki)
                                                                    (and
                                                                    (= 
                                                                    o34 (select 
                                                                    temp___3226 
                                                                    ki))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused6 
                                                                    o34)
                                                                    (and
                                                                    (= 
                                                                    o35 
                                                                    (us_split_fields1
                                                                    o34))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused9 
                                                                    o35)
                                                                    (and
                                                                    (= 
                                                                    o36 
                                                                    (rec__binary_trees__cell__parent
                                                                    o35))
                                                                    (and
                                                                    (= 
                                                                    rec__binary_trees__cell__parentqtresultqtunused1 
                                                                    o36)
                                                                    (and
                                                                    (= 
                                                                    (to_rep
                                                                    o36) 
                                                                    temp___3227)
                                                                    (and
                                                                    (= 
                                                                    to_repqtresultqtunused2 
                                                                    temp___3227)
                                                                    (and
                                                                    (and
                                                                    (<= 1 
                                                                    temp___3227)
                                                                    (<= 
                                                                    temp___3227 100))
                                                                    (and
                                                                    (= 
                                                                    o37 (select 
                                                                    temp___3224 
                                                                    temp___3227))
                                                                    (and
                                                                    (= 
                                                                    getqtresultqtunused7 
                                                                    o37)
                                                                    (and
                                                                    (= 
                                                                    o38 
                                                                    (us_split_fields5
                                                                    o37))
                                                                    (and
                                                                    (= 
                                                                    us_split_fieldsqtresultqtunused10 
                                                                    o38)
                                                                    (and
                                                                    (= 
                                                                    rec__tree_model__path_type__aqtresultqtunused4 
                                                                    (rec__tree_model__path_type__a
                                                                    o38))
                                                                    (= 
                                                                    (is_concat
                                                                    o20
                                                                    o27
                                                                    o23) true)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
                                                (and
                                                  (and
                                                    (= anyqtresultqtunused4 
                                                    usf1)
                                                    (and
                                                      (= iqtunused1 usf1)
                                                      (and
                                                        (= int__contentqtresultqtunused9 
                                                        ki)
                                                        (and
                                                          (= o11 (ite (<= 
                                                                   usf1 
                                                                   ki)
                                                                   true
                                                                   false))
                                                          (and
                                                            (= infix_lseqqtresultqtunused8 
                                                            o11)
                                                            (and
                                                              (= o12 
                                                              (ite (<= 1 
                                                                usf1)
                                                                true
                                                                false))
                                                              (and
                                                                (= infix_lseqqtresultqtunused9 
                                                                o12)
                                                                (= andbqtresultqtunused4 (and 
                                                                o12 o11)))))))))
                                                  (and
                                                    (= usfqtunused2 (ite 
                                                                    (forall 
                                                                    ((i Int))
                                                                    (=>
                                                                    (and
                                                                    (<= 1 i)
                                                                    (<= i 
                                                                    ki))
                                                                    (=>
                                                                    (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i))) true)
                                                                    (<= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))) 
                                                                    n3))
                                                                    (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f_old__split_fields)
                                                                    root) i))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f_old__split_fields)
                                                                    root) i)))) true)))))
                                                                    true
                                                                    false))
                                                    (and
                                                      (= anyqtresultqtunused3 
                                                      (ite (forall ((i Int))
                                                             (=>
                                                               (and
                                                                 (<= 1 i)
                                                                 (<= i 
                                                                 ki))
                                                               (=>
                                                                 (and
                                                                   (= 
                                                                   (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i))) true)
                                                                   (<= 
                                                                   (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))) 
                                                                   n3))
                                                                 (=>
                                                                   (= 
                                                                   (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f_old__split_fields)
                                                                    root) i))) true)
                                                                   (= 
                                                                   (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f_old__split_fields)
                                                                    root) i)))) true)))))
                                                        true
                                                        false))
                                                      (and
                                                        (= kiqtunused1 
                                                        (int__refqtmk
                                                          ki1))
                                                        (and
                                                          (and
                                                            (forall ((i Int))
                                                              (=>
                                                                (and
                                                                  (<= 1 i)
                                                                  (<= i 
                                                                  ki1))
                                                                (=>
                                                                  (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i))) true)
                                                                    (<= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))) 
                                                                    n3))
                                                                  (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f_old__split_fields)
                                                                    root) i))) true)
                                                                    (= 
                                                                    (oeq
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f_old__split_fields)
                                                                    root) i)))) true)))))
                                                            (forall ((i Int))
                                                              (=>
                                                                (and
                                                                  (<= 1 i)
                                                                  (<= i 
                                                                  ki1))
                                                                (=>
                                                                  (and
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i))) true)
                                                                    (<= 
                                                                    (last
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))) 
                                                                    n3))
                                                                  (=>
                                                                    (= 
                                                                    (rec__tree_model__path_type__k
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f_old__split_fields)
                                                                    v) i))) true)
                                                                    (= 
                                                                    (is_concat
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) 
                                                                    v)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f_old__split_fields)
                                                                    v) i)))
                                                                    (rec__tree_model__path_type__a
                                                                    (us_split_fields5
                                                                    (select 
                                                                    (model1
                                                                    (us_repqtmk1
                                                                    f__split_fields)
                                                                    root) i)))) true))))))
                                                          (and
                                                            (and
                                                              (<= 1 ki1)
                                                              (<= ki1 100))
                                                            (and
                                                              (= int__contentqtresultqtunused8 
                                                              ki1)
                                                              (and
                                                                (= infix_eqqtresultqtunused2 
                                                                (ite (= 
                                                                  ki1 100)
                                                                  true
                                                                  false))
                                                                (= ki1 100)))))))))))))
                                        (= spark__branch1 spark__branch2))))))))))))
                  (and
                    (= int__contentqtresultqtunused5 n3)
                    (and
                      (= infix_eqqtresultqtunused1 (ite (= n3 100)
                                                     true
                                                     false))
                      (= n3 100))))))))))
    (= spark__branch1 spark__branch)))

;; "__split_fields__content'result'unused"
(define-fun us_split_fields__contentqtresultqtunused36 () us_split_fields2
  f_old__split_fields)

;; "__rep'mk'result'unused"
(define-fun us_repqtmkqtresultqtunused36 () us_rep1
  (us_repqtmk1 f_old__split_fields))

;; "__split_fields'result'unused"
(define-fun us_split_fieldsqtresultqtunused27 () us_split_fields2
  f_old__split_fields)

;; "o"
(define-fun o84 () extended_index_type
  (rec__binary_trees__forest__s f_old__split_fields))

;; "rec__binary_trees__forest__s'result'unused"
(define-fun rec__binary_trees__forest__sqtresultqtunused () extended_index_type
  o84)

(declare-const binary_trees__plug__prove_post__L_17__R266b__assume Int)

;; "Ensures"
(assert (= (to_rep o84) binary_trees__plug__prove_post__L_17__R266b__assume))

;; "to_rep'result'unused"
(define-fun to_repqtresultqtunused6 () Int
  binary_trees__plug__prove_post__L_17__R266b__assume)

;; "Assume"
(assert (= binary_trees__plug__prove_post__L_17__R266b__assume r266b))

;; "Assume"
(assert (in_range r266b))

;; "temp___3454"
(define-fun temp___3454 () Int
  r266b)

(declare-const r Int)

;; "r'unused"
(define-fun rqtunused () int__ref
  (int__refqtmk r))

;; "H"
(assert (= r 1))

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused29 () Int
  r)

;; "o"
(define-fun o85 () Bool
  (ite (<= r temp___3454) true false))

;; "infix <='result'unused"
(define-fun infix_lseqqtresultqtunused12 () Bool
  o85)

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused30 () Int
  r)

;; "o"
(define-fun o86 () Bool
  (ite (<= 1 r) true false))

;; "infix <='result'unused"
(define-fun infix_lseqqtresultqtunused13 () Bool
  o86)

;; "o"
(define-fun o87 () Bool
  (and o86 o85))

;; "andb'result'unused"
(define-fun andbqtresultqtunused6 () Bool
  o87)

;; "H"
(assert (= o87 true))

(declare-const spark__branch5 Bool)

;; "spark__branch'unused"
(define-fun spark__branchqtunused3 () bool__ref
  (bool__refqtmk spark__branch5))

(declare-const us_split_fields__contentqtresultqtunused37 us_split_fields2)

;; "H"
(assert (= us_split_fields__contentqtresultqtunused37 f_old__split_fields))

(declare-const us_repqtmkqtresultqtunused37 us_rep1)

;; "H"
(assert (= us_repqtmkqtresultqtunused37 (us_repqtmk1 f_old__split_fields)))

(declare-const us_split_fieldsqtresultqtunused28 us_split_fields2)

;; "H"
(assert (= us_split_fieldsqtresultqtunused28 f_old__split_fields))

(declare-const temp___3465 (Array Int us_rep))

;; "H"
(assert (= temp___3465 (rec__binary_trees__forest__c f_old__split_fields)))

(declare-const rec__binary_trees__forest__cqtresultqtunused6 (Array Int us_rep))

;; "H"
(assert (= rec__binary_trees__forest__cqtresultqtunused6 temp___3465))

(declare-const int__contentqtresultqtunused31 Int)

;; "H"
(assert (= int__contentqtresultqtunused31 r))

(declare-const o88 us_rep)

;; "H"
(assert (= o88 (select temp___3465 r)))

(declare-const getqtresultqtunused21 us_rep)

;; "H"
(assert (= getqtresultqtunused21 o88))

(declare-const o89 us_split_fields)

;; "H"
(assert (= o89 (us_split_fields1 o88)))

(declare-const us_split_fieldsqtresultqtunused29 us_split_fields)

;; "H"
(assert (= us_split_fieldsqtresultqtunused29 o89))

(declare-const o90 position_type)

;; "H"
(assert (= o90 (rec__binary_trees__cell__position o89)))

(declare-const rec__binary_trees__cell__positionqtresultqtunused2 position_type)

;; "H"
(assert (= rec__binary_trees__cell__positionqtresultqtunused2 o90))

(declare-const o91 Int)

;; "Ensures"
(assert (= (to_rep1 o90) o91))

(declare-const to_repqtresultqtunused7 Int)

;; "H"
(assert (= to_repqtresultqtunused7 o91))

(declare-const o92 Bool)

;; "H"
(assert (= o92 (ite (= o91 2) true false)))

(declare-const infix_eqqtresultqtunused6 Bool)

;; "H"
(assert (= infix_eqqtresultqtunused6 o92))

(declare-const int__contentqtresultqtunused32 Int)

;; "H"
(assert (= int__contentqtresultqtunused32 r))

(declare-const infix_eqqtresultqtunused7 Bool)

;; "H"
(assert (= infix_eqqtresultqtunused7 (ite (= r v) true false)))

(declare-const int__contentqtresultqtunused33 Int)

;; "H"
(assert (= int__contentqtresultqtunused33 r))

(declare-const infix_eqqtresultqtunused8 Bool)

;; "H"
(assert (= infix_eqqtresultqtunused8 (ite (= r root) true false)))

(declare-const o93 Bool)

;; "H"
(assert (= o93 (and (ite (= r root) false true) (ite (= r v) false true))))

(declare-const andbqtresultqtunused7 Bool)

;; "H"
(assert (= andbqtresultqtunused7 o93))

(declare-const o94 Bool)

;; "H"
(assert (= o94 (and o93 o92)))

(declare-const andbqtresultqtunused8 Bool)

;; "H"
(assert (= andbqtresultqtunused8 o94))

;; "H"
(assert (= spark__branch5 o94))

(declare-const bool__contentqtresultqtunused2 Bool)

;; "H"
(assert (= bool__contentqtresultqtunused2 spark__branch5))

(declare-const us_split_fields__contentqtresultqtunused38 us_split_fields2)

(declare-const temp___compl_3455 us_rep1)

(declare-const us_repqtmkqtresultqtunused38 us_rep1)

(declare-const int__contentqtresultqtunused34 Int)

(declare-const us_split_fields__contentqtresultqtunused39 us_split_fields2)

(declare-const temp___3458 us_rep1)

(declare-const us_repqtmkqtresultqtunused39 us_rep1)

(declare-const us_split_fields__contentqtresultqtunused40 us_split_fields2)

(declare-const temp___compl_3460 us_rep1)

(declare-const us_repqtmkqtresultqtunused40 us_rep1)

(declare-const o95 Int)

(declare-const range_check_qtresultqtunused5 Int)

(declare-const int__contentqtresultqtunused35 Int)

(declare-const us_split_fields__contentqtresultqtunused41 us_split_fields2)

(declare-const temp___3463 us_rep1)

(declare-const us_repqtmkqtresultqtunused41 us_rep1)

(declare-const int__contentqtresultqtunused36 Int)

(declare-const us_split_fields__contentqtresultqtunused42 us_split_fields2)

(declare-const o96 us_rep1)

(declare-const us_repqtmkqtresultqtunused42 us_rep1)

(declare-const us_split_fields__contentqtresultqtunused43 us_split_fields2)

(declare-const o97 us_rep1)

(declare-const us_repqtmkqtresultqtunused43 us_rep1)

;; "H"
(assert
  (=>
    (= spark__branch5 true)
    (and
      (= us_split_fields__contentqtresultqtunused38 f_old__split_fields)
      (and
        (= temp___compl_3455 (us_repqtmk1 f_old__split_fields))
        (and
          (= us_repqtmkqtresultqtunused38 temp___compl_3455)
          (and
            (= int__contentqtresultqtunused34 r)
            (and
              (= us_split_fields__contentqtresultqtunused39 f_old__split_fields)
              (and
                (= temp___3458 (us_repqtmk1 f_old__split_fields))
                (and
                  (= us_repqtmkqtresultqtunused39 temp___3458)
                  (and
                    (type_invariant temp___3458)
                    (and
                      (forall ((i Int))
                        (=>
                          (and (<= 1 i) (<= i 100))
                          (or
                            (not
                              (= (rec__tree_model__path_type__k
                                   (us_split_fields5
                                     (select (model1 temp___compl_3455 root) i))) true))
                            (not
                              (= (rec__tree_model__path_type__k
                                   (us_split_fields5
                                     (select (model1 temp___compl_3455 r) i))) true)))))
                      (and
                        (= us_split_fields__contentqtresultqtunused40 
                        f_old__split_fields)
                        (and
                          (= temp___compl_3460 (us_repqtmk1
                                                 f_old__split_fields))
                          (and
                            (= us_repqtmkqtresultqtunused40 temp___compl_3460)
                            (and
                              (= o95 v)
                              (and
                                (in_range3 o95)
                                (and
                                  (= range_check_qtresultqtunused5 o95)
                                  (and
                                    (= int__contentqtresultqtunused35 
                                    r)
                                    (and
                                      (= us_split_fields__contentqtresultqtunused41 
                                      f_old__split_fields)
                                      (and
                                        (= temp___3463 (us_repqtmk1
                                                         f_old__split_fields))
                                        (and
                                          (= us_repqtmkqtresultqtunused41 
                                          temp___3463)
                                          (and
                                            (type_invariant temp___3463)
                                            (and
                                              (forall ((i Int))
                                                (=>
                                                  (and (<= 1 i) (<= i 100))
                                                  (or
                                                    (not
                                                      (= (rec__tree_model__path_type__k
                                                           (us_split_fields5
                                                             (select 
                                                             (model1
                                                               temp___compl_3460
                                                               o95) i))) true))
                                                    (not
                                                      (= (rec__tree_model__path_type__k
                                                           (us_split_fields5
                                                             (select 
                                                             (model1
                                                               temp___compl_3460
                                                               r) i))) true)))))
                                              (and
                                                (= int__contentqtresultqtunused36 
                                                r)
                                                (and
                                                  (= us_split_fields__contentqtresultqtunused42 
                                                  f__split_fields)
                                                  (and
                                                    (= o96 (us_repqtmk1
                                                             f__split_fields))
                                                    (and
                                                      (= us_repqtmkqtresultqtunused42 
                                                      o96)
                                                      (and
                                                        (= us_split_fields__contentqtresultqtunused43 
                                                        f_old__split_fields)
                                                        (and
                                                          (= o97 (us_repqtmk1
                                                                   f_old__split_fields))
                                                          (and
                                                            (= us_repqtmkqtresultqtunused43 
                                                            o97)
                                                            (and
                                                              (= (to_rep1
                                                                   (rec__binary_trees__cell__position
                                                                    (us_split_fields1
                                                                    (select 
                                                                    (rec__binary_trees__forest__c
                                                                    f__split_fields) 
                                                                    r)))) 2)
                                                              (= (oeq1
                                                                   (model1
                                                                    o97
                                                                    r)
                                                                   (model1
                                                                    o96
                                                                    r)) true))))))))))))))))))))))))))))))))

(declare-const usf3 Int)

(declare-const anyqtresultqtunused6 Int)

;; "H"
(assert (= anyqtresultqtunused6 usf3))

(declare-const pqtunused Int)

;; "H"
(assert (= pqtunused usf3))

(declare-const int__contentqtresultqtunused37 Int)

;; "H"
(assert (= int__contentqtresultqtunused37 r))

(declare-const o98 Bool)

;; "H"
(assert (= o98 (ite (<= usf3 r) true false)))

(declare-const infix_lseqqtresultqtunused14 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused14 o98))

(declare-const o99 Bool)

;; "H"
(assert (= o99 (ite (<= 1 usf3) true false)))

(declare-const infix_lseqqtresultqtunused15 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused15 o99))

(declare-const andbqtresultqtunused9 Bool)

;; "H"
(assert (= andbqtresultqtunused9 (and o99 o98)))

;; "_f'unused"
(define-fun usfqtunused3 () Bool
  (ite (forall ((p Int))
         (=>
           (and (<= 1 p) (<= p r))
           (=>
             (and
               (and (not (= p root)) (not (= p v)))
               (= (to_rep1
                    (rec__binary_trees__cell__position
                      (us_split_fields1
                        (select (rec__binary_trees__forest__c
                                  f_old__split_fields) p)))) 2))
             (= (oeq1
                  (model1 (us_repqtmk1 f__split_fields) p)
                  (model1 (us_repqtmk1 f_old__split_fields) p)) true))))
    true
    false))

;; "any'result'unused"
(define-fun anyqtresultqtunused7 () Bool
  (ite (forall ((p Int))
         (=>
           (and (<= 1 p) (<= p r))
           (=>
             (and
               (and (not (= p root)) (not (= p v)))
               (= (to_rep1
                    (rec__binary_trees__cell__position
                      (us_split_fields1
                        (select (rec__binary_trees__forest__c
                                  f_old__split_fields) p)))) 2))
             (= (oeq1
                  (model1 (us_repqtmk1 f__split_fields) p)
                  (model1 (us_repqtmk1 f_old__split_fields) p)) true))))
    true
    false))

(declare-const r1 Int)

;; "r'unused"
(define-fun rqtunused1 () int__ref
  (int__refqtmk r1))

;; "LoopInvariant"
(assert
  (forall ((p Int))
    (=>
      (and (<= 1 p) (<= p r1))
      (=>
        (and
          (and (not (= p root)) (not (= p v)))
          (= (to_rep1
               (rec__binary_trees__cell__position
                 (us_split_fields1
                   (select (rec__binary_trees__forest__c f_old__split_fields) p)))) 2))
        (= (oeq1
             (model1 (us_repqtmk1 f__split_fields) p)
             (model1 (us_repqtmk1 f_old__split_fields) p)) true)))))

;; "Assume"
(assert (and (<= 1 r1) (<= r1 r266b)))

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused38 () Int
  r1)

;; "infix ='result'unused"
(define-fun infix_eqqtresultqtunused9 () Bool
  (ite (= r1 temp___3454) true false))

;; "H"
(assert (not (= r1 temp___3454)))

;; "int__content'result'unused"
(define-fun int__contentqtresultqtunused39 () Int
  r1)

;; "o"
(define-fun o100 () Int
  (+ r1 1))

;; "infix +'result'unused"
(define-fun infix_plqtresultqtunused () Int
  o100)

(declare-const r2 Int)

;; "r'unused"
(define-fun rqtunused2 () int__ref
  (int__refqtmk r2))

;; "H"
(assert (= r2 o100))

(declare-const us_split_fields__contentqtresultqtunused44 us_split_fields2)

;; "H"
(assert (= us_split_fields__contentqtresultqtunused44 f_old__split_fields))

(declare-const us_repqtmkqtresultqtunused44 us_rep1)

;; "H"
(assert (= us_repqtmkqtresultqtunused44 (us_repqtmk1 f_old__split_fields)))

(declare-const us_split_fieldsqtresultqtunused30 us_split_fields2)

;; "H"
(assert (= us_split_fieldsqtresultqtunused30 f_old__split_fields))

(declare-const temp___34651 (Array Int us_rep))

;; "H"
(assert (= temp___34651 (rec__binary_trees__forest__c f_old__split_fields)))

(declare-const rec__binary_trees__forest__cqtresultqtunused7 (Array Int us_rep))

;; "H"
(assert (= rec__binary_trees__forest__cqtresultqtunused7 temp___34651))

(declare-const int__contentqtresultqtunused40 Int)

;; "H"
(assert (= int__contentqtresultqtunused40 r2))

(declare-const o101 us_rep)

;; "H"
(assert (= o101 (select temp___34651 r2)))

(declare-const getqtresultqtunused22 us_rep)

;; "H"
(assert (= getqtresultqtunused22 o101))

(declare-const o102 us_split_fields)

;; "H"
(assert (= o102 (us_split_fields1 o101)))

(declare-const us_split_fieldsqtresultqtunused31 us_split_fields)

;; "H"
(assert (= us_split_fieldsqtresultqtunused31 o102))

(declare-const o103 position_type)

;; "H"
(assert (= o103 (rec__binary_trees__cell__position o102)))

(declare-const rec__binary_trees__cell__positionqtresultqtunused3 position_type)

;; "H"
(assert (= rec__binary_trees__cell__positionqtresultqtunused3 o103))

(declare-const o104 Int)

;; "Ensures"
(assert (= (to_rep1 o103) o104))

(declare-const to_repqtresultqtunused8 Int)

;; "H"
(assert (= to_repqtresultqtunused8 o104))

(declare-const o105 Bool)

;; "H"
(assert (= o105 (ite (= o104 2) true false)))

(declare-const infix_eqqtresultqtunused10 Bool)

;; "H"
(assert (= infix_eqqtresultqtunused10 o105))

(declare-const int__contentqtresultqtunused41 Int)

;; "H"
(assert (= int__contentqtresultqtunused41 r2))

(declare-const infix_eqqtresultqtunused11 Bool)

;; "H"
(assert (= infix_eqqtresultqtunused11 (ite (= r2 v) true false)))

(declare-const int__contentqtresultqtunused42 Int)

;; "H"
(assert (= int__contentqtresultqtunused42 r2))

(declare-const infix_eqqtresultqtunused12 Bool)

;; "H"
(assert (= infix_eqqtresultqtunused12 (ite (= r2 root) true false)))

(declare-const o106 Bool)

;; "H"
(assert
  (= o106 (and (ite (= r2 root) false true) (ite (= r2 v) false true))))

(declare-const andbqtresultqtunused10 Bool)

;; "H"
(assert (= andbqtresultqtunused10 o106))

(declare-const o107 Bool)

;; "H"
(assert (= o107 (and o106 o105)))

(declare-const andbqtresultqtunused11 Bool)

;; "H"
(assert (= andbqtresultqtunused11 o107))

(declare-const spark__branch6 Bool)

(declare-const spark__branchqtunused4 bool__ref)

;; "H"
(assert (= spark__branchqtunused4 (bool__refqtmk spark__branch6)))

;; "H"
(assert (= spark__branch6 o107))

(declare-const bool__contentqtresultqtunused3 Bool)

;; "H"
(assert (= bool__contentqtresultqtunused3 spark__branch6))

(declare-const us_split_fields__contentqtresultqtunused45 us_split_fields2)

(declare-const temp___compl_34551 us_rep1)

(declare-const us_repqtmkqtresultqtunused45 us_rep1)

(declare-const int__contentqtresultqtunused43 Int)

(declare-const us_split_fields__contentqtresultqtunused46 us_split_fields2)

(declare-const temp___34581 us_rep1)

(declare-const us_repqtmkqtresultqtunused46 us_rep1)

(declare-const us_split_fields__contentqtresultqtunused47 us_split_fields2)

(declare-const temp___compl_34601 us_rep1)

(declare-const us_repqtmkqtresultqtunused47 us_rep1)

(declare-const o108 Int)

(declare-const range_check_qtresultqtunused6 Int)

(declare-const int__contentqtresultqtunused44 Int)

(declare-const us_split_fields__contentqtresultqtunused48 us_split_fields2)

(declare-const temp___34631 us_rep1)

(declare-const us_repqtmkqtresultqtunused48 us_rep1)

(declare-const int__contentqtresultqtunused45 Int)

(declare-const us_split_fields__contentqtresultqtunused49 us_split_fields2)

(declare-const o109 us_rep1)

(declare-const us_repqtmkqtresultqtunused49 us_rep1)

(declare-const us_split_fields__contentqtresultqtunused50 us_split_fields2)

(declare-const o110 us_rep1)

(declare-const us_repqtmkqtresultqtunused50 us_rep1)

;; "H"
(assert
  (=>
    (= spark__branch6 true)
    (and
      (= us_split_fields__contentqtresultqtunused45 f_old__split_fields)
      (and
        (= temp___compl_34551 (us_repqtmk1 f_old__split_fields))
        (and
          (= us_repqtmkqtresultqtunused45 temp___compl_34551)
          (and
            (= int__contentqtresultqtunused43 r2)
            (and
              (= us_split_fields__contentqtresultqtunused46 f_old__split_fields)
              (and
                (= temp___34581 (us_repqtmk1 f_old__split_fields))
                (and
                  (= us_repqtmkqtresultqtunused46 temp___34581)
                  (and
                    (type_invariant temp___34581)
                    (and
                      (forall ((i Int))
                        (=>
                          (and (<= 1 i) (<= i 100))
                          (or
                            (not
                              (= (rec__tree_model__path_type__k
                                   (us_split_fields5
                                     (select (model1 temp___compl_34551 root) i))) true))
                            (not
                              (= (rec__tree_model__path_type__k
                                   (us_split_fields5
                                     (select (model1 temp___compl_34551 r2) i))) true)))))
                      (and
                        (= us_split_fields__contentqtresultqtunused47 
                        f_old__split_fields)
                        (and
                          (= temp___compl_34601 (us_repqtmk1
                                                  f_old__split_fields))
                          (and
                            (= us_repqtmkqtresultqtunused47 temp___compl_34601)
                            (and
                              (= o108 v)
                              (and
                                (in_range3 o108)
                                (and
                                  (= range_check_qtresultqtunused6 o108)
                                  (and
                                    (= int__contentqtresultqtunused44 
                                    r2)
                                    (and
                                      (= us_split_fields__contentqtresultqtunused48 
                                      f_old__split_fields)
                                      (and
                                        (= temp___34631 (us_repqtmk1
                                                          f_old__split_fields))
                                        (and
                                          (= us_repqtmkqtresultqtunused48 
                                          temp___34631)
                                          (and
                                            (type_invariant temp___34631)
                                            (and
                                              (forall ((i Int))
                                                (=>
                                                  (and (<= 1 i) (<= i 100))
                                                  (or
                                                    (not
                                                      (= (rec__tree_model__path_type__k
                                                           (us_split_fields5
                                                             (select 
                                                             (model1
                                                               temp___compl_34601
                                                               o108) i))) true))
                                                    (not
                                                      (= (rec__tree_model__path_type__k
                                                           (us_split_fields5
                                                             (select 
                                                             (model1
                                                               temp___compl_34601
                                                               r2) i))) true)))))
                                              (and
                                                (= int__contentqtresultqtunused45 
                                                r2)
                                                (and
                                                  (= us_split_fields__contentqtresultqtunused49 
                                                  f__split_fields)
                                                  (and
                                                    (= o109 (us_repqtmk1
                                                              f__split_fields))
                                                    (and
                                                      (= us_repqtmkqtresultqtunused49 
                                                      o109)
                                                      (and
                                                        (= us_split_fields__contentqtresultqtunused50 
                                                        f_old__split_fields)
                                                        (and
                                                          (= o110 (us_repqtmk1
                                                                    f_old__split_fields))
                                                          (and
                                                            (= us_repqtmkqtresultqtunused50 
                                                            o110)
                                                            (and
                                                              (= (to_rep1
                                                                   (rec__binary_trees__cell__position
                                                                    (us_split_fields1
                                                                    (select 
                                                                    (rec__binary_trees__forest__c
                                                                    f__split_fields) 
                                                                    r2)))) 2)
                                                              (= (oeq1
                                                                   (model1
                                                                    o110
                                                                    r2)
                                                                   (model1
                                                                    o109
                                                                    r2)) true))))))))))))))))))))))))))))))))

(declare-const usf4 Int)

(declare-const anyqtresultqtunused8 Int)

;; "H"
(assert (= anyqtresultqtunused8 usf4))

(declare-const pqtunused1 Int)

;; "H"
(assert (= pqtunused1 usf4))

(declare-const int__contentqtresultqtunused46 Int)

;; "H"
(assert (= int__contentqtresultqtunused46 r2))

(declare-const o111 Bool)

;; "H"
(assert (= o111 (ite (<= usf4 r2) true false)))

(declare-const infix_lseqqtresultqtunused16 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused16 o111))

(declare-const o112 Bool)

;; "H"
(assert (= o112 (ite (<= 1 usf4) true false)))

(declare-const infix_lseqqtresultqtunused17 Bool)

;; "H"
(assert (= infix_lseqqtresultqtunused17 o112))

(declare-const andbqtresultqtunused12 Bool)

;; "H"
(assert (= andbqtresultqtunused12 (and o112 o111)))

;; "_f'unused"
(define-fun usfqtunused4 () Bool
  (ite (forall ((p Int))
         (=>
           (and (<= 1 p) (<= p r2))
           (=>
             (and
               (and (not (= p root)) (not (= p v)))
               (= (to_rep1
                    (rec__binary_trees__cell__position
                      (us_split_fields1
                        (select (rec__binary_trees__forest__c
                                  f_old__split_fields) p)))) 2))
             (= (oeq1
                  (model1 (us_repqtmk1 f__split_fields) p)
                  (model1 (us_repqtmk1 f_old__split_fields) p)) true))))
    true
    false))

;; "any'result'unused"
(define-fun anyqtresultqtunused9 () Bool
  (ite (forall ((p Int))
         (=>
           (and (<= 1 p) (<= p r2))
           (=>
             (and
               (and (not (= p root)) (not (= p v)))
               (= (to_rep1
                    (rec__binary_trees__cell__position
                      (us_split_fields1
                        (select (rec__binary_trees__forest__c
                                  f_old__split_fields) p)))) 2))
             (= (oeq1
                  (model1 (us_repqtmk1 f__split_fields) p)
                  (model1 (us_repqtmk1 f_old__split_fields) p)) true))))
    true
    false))

(declare-const p Int)

;; "H"
(assert (<= 1 p))

;; "H"
(assert (<= p r2))

;; "H"
(assert (not (= p root)))

;; "H"
(assert (not (= p v)))

;; "H"
(assert
  (= (to_rep1
       (rec__binary_trees__cell__position
         (us_split_fields1
           (select (rec__binary_trees__forest__c f_old__split_fields) 
           p)))) 2))

;; "dummy"
(define-fun dummy5 () Bool
  true)

(declare-const i Int)

;; "H"
(assert (<= 1 i))

;; "H"
(assert (<= i 100))

;; "H"
(assert
  (= (rec__tree_model__path_type__k
       (us_split_fields5 (select (model1 (us_repqtmk1 f__split_fields) p) i))) true))

;; Goal "def'vc"
;; File "binary_trees.adb", line 665, characters 0-0
(assert
  (not
  (= (rec__tree_model__path_type__k
       (us_split_fields5
         (select (model1 (us_repqtmk1 f_old__split_fields) p) i))) true)))

(check-sat)
