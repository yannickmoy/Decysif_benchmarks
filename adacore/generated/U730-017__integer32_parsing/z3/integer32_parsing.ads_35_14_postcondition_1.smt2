...
(assert
  (forall ((i Int))
    (=>
      (and (<= first2 i) (<= i (last s2)))
      (let ((temp___481 (to_rep (select (to_array s2) i))))
        (and (<= 48 temp___481) (<= temp___481 57))))))

;; "H"
(assert (valid_int_32__function_guard (valid_int_32 s2) s2))

;; Goal "def'vc"
;; File "integer32_parsing.ads", line 31, characters 0-0
(assert
  (not (= (valid_int_32 s2) true)))
