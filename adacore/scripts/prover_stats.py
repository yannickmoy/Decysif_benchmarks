...
class Colibri(Prover):

    limit_reg = re.compile("Steps : (\d*)")  # noqa
    name = "colibri"

    def __init__(self, executable_name="colibri"):
        Prover.__init__(self)
        self.executable_name = executable_name
        self.version_arg = "--version"
...
def compute_file_list(files, prover, limit):
    ...
    for fn in files:
        ...
        if os.path.isdir(fn):
            result += glob.glob(os.path.join(fn, default_pattern))
            if prover.pattern != default_pattern:
...
def main():
    provers = [Z3(), CVC4(), Altergo(), CVC5(), Colibri()]
